...
contains
  ...
  subroutine read_input_GPU(file, config, driver_config, ncol, nlev, &
       &          single_level, thermodynamics, &
       &          gas, cloud, aerosol, lacc)
    ...
    if (config%do_clouds) then
      ...
      call single_level%init_seed_simple_GPU(1,ncol,lacc=.false.)
      ...
      if (driver_config%overlap_decorr_length_override > 0.0_jprb) then
        ...
        call cloud%set_overlap_param_GPU(thermodynamics, &
             &    driver_config%overlap_decorr_length_override, lacc=.false.)
        ...
        call cloud%set_overlap_param_GPU(thermodynamics, decorr_length_default, lacc=.false.)
      ...
      if (driver_config%fractional_std_override >= 0.0_jprb) then
        ...
        call cloud%create_fractional_std_GPU(ncol, nlev, &
             &  driver_config%fractional_std_override, lacc=.false.)
      else if (.not. allocated(cloud%fractional_std)) then
        call cloud%create_fractional_std_GPU(ncol, nlev, 0.0_jprb, lacc=.false.)
        if (driver_config%iverbose >= 1) then
      ...
      if (config%i_solver_sw == ISolverSPARTACUS &
        ...
        if (driver_config%low_inv_effective_size_override >= 0.0_jprb &
          ...
          call cloud%create_inv_cloud_effective_size_eta_GPU(ncol, nlev, &
               &  thermodynamics%pressure_hl, &
               &  driver_config%low_inv_effective_size_override, &
               &  driver_config%middle_inv_effective_size_override, &
               &  driver_config%high_inv_effective_size_override, 0.8_jprb, 0.45_jprb, lacc=.false.)
          ...
          call cloud%param_cloud_effective_separation_eta_GPU(ncol, nlev, &
               &  thermodynamics%pressure_hl, &
               &  driver_config%cloud_separation_scale_surface, &
               &  driver_config%cloud_separation_scale_toa, &
               &  driver_config%cloud_separation_scale_power, &
               &  driver_config%cloud_inhom_separation_factor, lacc=.false.)
    ...
    call gas%allocate_GPU(ncol, nlev, lacc=.false.)
    ...
    do jgas = 1,NMaxGases
      if (jgas == IH2O) then
        ...
        if (irank == 0) then
          ...
          call gas%put_well_mixed_GPU(jgas, IVolumeMixingRatio, well_mixed_gas_vmr, lacc=.false.)
        else if (irank == 2) then
          call file%get(trim(gas_var_name), gas_mr)
          call gas%put_GPU(jgas, IVolumeMixingRatio, gas_mr, lacc=.false.)
    ...
    call gas%scale_GPU(IH2O,    driver_config%h2o_scaling,    driver_config%iverbose >= 2, lacc=.false.)
    call gas%scale_GPU(ICO2,    driver_config%co2_scaling,    driver_config%iverbose >= 2, lacc=.false.)
    call gas%scale_GPU(IO3,     driver_config%o3_scaling,     driver_config%iverbose >= 2, lacc=.false.)
    call gas%scale_GPU(IN2O,    driver_config%n2o_scaling,    driver_config%iverbose >= 2, lacc=.false.)
    call gas%scale_GPU(ICO,     driver_config%co_scaling,     driver_config%iverbose >= 2, lacc=.false.)
    call gas%scale_GPU(ICH4,    driver_config%ch4_scaling,    driver_config%iverbose >= 2, lacc=.false.)
    call gas%scale_GPU(IO2,     driver_config%o2_scaling,     driver_config%iverbose >= 2, lacc=.false.)
    call gas%scale_GPU(ICFC11,  driver_config%cfc11_scaling,  driver_config%iverbose >= 2, lacc=.false.)
    call gas%scale_GPU(ICFC12,  driver_config%cfc12_scaling,  driver_config%iverbose >= 2, lacc=.false.)
    call gas%scale_GPU(IHCFC22, driver_config%hcfc22_scaling, driver_config%iverbose >= 2, lacc=.false.)
    call gas%scale_GPU(ICCL4,   driver_config%ccl4_scaling,   driver_config%iverbose >= 2, lacc=.false.)
    call gas%scale_GPU(INO2,    driver_config%no2_scaling,    driver_config%iverbose >= 2, lacc=.false.)

  end subroutine read_input_GPU

  subroutine read_input_CPU(file, config, driver_config, ncol, nlev, &
       &          single_level, thermodynamics, &
       &          gas, cloud, aerosol)
    ...
    if (config%do_clouds) then
      ...
      ! Simple initialization of the seeds for the Monte Carlo scheme
      call single_level%init_seed_simple_CPU(1,ncol,lacc=.false.)
      ! Overwrite with user-specified values if available
      if (file%exists('iseed')) then
        call file%get('iseed', single_level%iseed)
      end if
      ...
      if (driver_config%overlap_decorr_length_override > 0.0_jprb) then
        ! Convert overlap decorrelation length to overlap parameter between
        ! adjacent layers, stored in cloud%overlap_param
        call cloud%set_overlap_param_CPU(thermodynamics, &
        ...
        call cloud%set_overlap_param_CPU(thermodynamics, decorr_length_default)
      else if (driver_config%overlap_decorr_length_scaling > 0.0_jprb) then
        ! Scale the overlap decorrelation length by taking the overlap
        ! parameter to a power
        !    where (cloud%overlap_param > 0.99_jprb) cloud%overlap_param = 0.99_jprb
      ...
      if (driver_config%fractional_std_override >= 0.0_jprb) then
        ...
        call cloud%create_fractional_std_CPU(ncol, nlev, &
             &  driver_config%fractional_std_override, lacc=.false.)
      else if (.not. allocated(cloud%fractional_std)) then
        call cloud%create_fractional_std_CPU(ncol, nlev, 0.0_jprb, lacc=.false.)
        if (driver_config%iverbose >= 1) then
          write(nulout,'(a)') 'Warning: cloud optical depth fractional standard deviation set to zero'
        end if
      end if
      ...
      if (config%i_solver_sw == ISolverSPARTACUS &
        ...
        if (driver_config%low_inv_effective_size_override >= 0.0_jprb &
          ...
          call cloud%create_inv_cloud_effective_size_eta_CPU(ncol, nlev, &
               &  thermodynamics%pressure_hl, &
               &  driver_config%low_inv_effective_size_override, &
               &  driver_config%middle_inv_effective_size_override, &
          ...
          call cloud%param_cloud_effective_separation_eta_CPU(ncol, nlev, &
               &  thermodynamics%pressure_hl, &
               &  driver_config%cloud_separation_scale_surface, &
               &  driver_config%cloud_separation_scale_toa, &
               &  driver_config%cloud_separation_scale_power, &
    ...
    call gas%allocate_CPU(ncol, nlev)
    ...
    do jgas = 1,NMaxGases
      if (jgas == IH2O) then
        if (file%exists('q')) then
          call file%get('q', gas_mr)
          call gas%put_CPU(IH2O, IMassMixingRatio, gas_mr)
        else if (file%exists('h2o_mmr')) then
          call file%get('h2o_mmr', gas_mr)
          call gas%put_CPU(IH2O, IMassMixingRatio, gas_mr)
        else
          call file%get('h2o' // trim(driver_config%vmr_suffix_str), gas_mr);
          call gas%put_CPU(IH2O, IVolumeMixingRatio, gas_mr)
        end if
      else if (jgas == IO3) then
        if (file%exists('o3_mmr')) then
          call file%get('o3_mmr', gas_mr)
          call gas%put_CPU(IO3, IMassMixingRatio, gas_mr)
        else
          call file%get('o3' // trim(driver_config%vmr_suffix_str), gas_mr)
          call gas%put_CPU(IO3, IVolumeMixingRatio, gas_mr)
        end if
      else
        ! Find number of dimensions of the variable holding gas "jgas" in
        ! the input file, where the following function returns -1 if the
        ...
        if (irank == 0) then
          ! Store this as a well-mixed gas
          call file%get(trim(gas_var_name), well_mixed_gas_vmr)
          call gas%put_well_mixed_CPU(jgas, IVolumeMixingRatio, well_mixed_gas_vmr)
        else if (irank == 2) then
          call file%get(trim(gas_var_name), gas_mr)
          call gas%put_CPU(jgas, IVolumeMixingRatio, gas_mr)
        else if (irank > 0) then
          write(nulout,'(a,a,a)')  '***  Error: ', trim(gas_var_name), ' does not have 0 or 2 dimensions'
          stop
        end if
    ...
    ! Scale gas concentrations if needed
    call gas%scale_CPU(IH2O,    driver_config%h2o_scaling,    driver_config%iverbose >= 2)
    call gas%scale_CPU(ICO2,    driver_config%co2_scaling,    driver_config%iverbose >= 2)
    call gas%scale_CPU(IO3,     driver_config%o3_scaling,     driver_config%iverbose >= 2)
    call gas%scale_CPU(IN2O,    driver_config%n2o_scaling,    driver_config%iverbose >= 2)
    call gas%scale_CPU(ICO,     driver_config%co_scaling,     driver_config%iverbose >= 2)
    call gas%scale_CPU(ICH4,    driver_config%ch4_scaling,    driver_config%iverbose >= 2)
    call gas%scale_CPU(IO2,     driver_config%o2_scaling,     driver_config%iverbose >= 2)
    call gas%scale_CPU(ICFC11,  driver_config%cfc11_scaling,  driver_config%iverbose >= 2)
    call gas%scale_CPU(ICFC12,  driver_config%cfc12_scaling,  driver_config%iverbose >= 2)
    call gas%scale_CPU(IHCFC22, driver_config%hcfc22_scaling, driver_config%iverbose >= 2)
    call gas%scale_CPU(ICCL4,   driver_config%ccl4_scaling,   driver_config%iverbose >= 2)
    call gas%scale_CPU(INO2,    driver_config%no2_scaling,    driver_config%iverbose >= 2)

  end subroutine read_input_CPU
