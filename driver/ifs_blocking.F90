...
module ifs_blocking
  ...
  use parkind1 ! Working precision, integer type
...
integer(kind=jpim) function indrad_GPU(knext,kflds,lduse, lacc)
...
end function indrad_GPU

subroutine ifs_setup_indices_GPU (driver_config, ifs_config, yradiation, nlev, lacc)
  ...
  ifs_config%igi    =indrad_GPU(inext,1,lldebug, lacc)
  ifs_config%imu0   =indrad_GPU(inext,1,.true., lacc)
  ifs_config%iamu0  =indrad_GPU(inext,1,.true., lacc)
  ifs_config%iemiss =indrad_GPU(inext,yradiation%yrerad%nlwemiss,.true., lacc)
  ifs_config%its    =indrad_GPU(inext,1,.true., lacc)
  ifs_config%islm   =indrad_GPU(inext,1,.true., lacc)
  ifs_config%iccnl  =indrad_GPU(inext,1,.true., lacc)
  ifs_config%iccno  =indrad_GPU(inext,1,.true., lacc)
  ifs_config%ibas   =indrad_GPU(inext,1,.true., lacc)
  ifs_config%itop   =indrad_GPU(inext,1,.true., lacc)
  ifs_config%igelam =indrad_GPU(inext,1,.true., lacc)
  ifs_config%igemu  =indrad_GPU(inext,1,.true., lacc)
  ifs_config%iclon  =indrad_GPU(inext,1,.true., lacc)
  ifs_config%islon  =indrad_GPU(inext,1,.true., lacc)
  ifs_config%iald   =indrad_GPU(inext,yradiation%yrerad%nsw,.true., lacc)
  ifs_config%ialp   =indrad_GPU(inext,yradiation%yrerad%nsw,.true., lacc)
  ifs_config%iti    =indrad_GPU(inext,nlev,.true., lacc)
  ifs_config%ipr    =indrad_GPU(inext,nlev,.true., lacc)
  ifs_config%iqs    =indrad_GPU(inext,nlev,.true., lacc)
  ifs_config%iwv    =indrad_GPU(inext,nlev,.true., lacc)
  ifs_config%iclc   =indrad_GPU(inext,nlev,.true., lacc)
  ifs_config%ilwa   =indrad_GPU(inext,nlev,.true., lacc)
  ifs_config%iiwa   =indrad_GPU(inext,nlev,.true., lacc)
  ifs_config%iswa   =indrad_GPU(inext,nlev,.true., lacc)
  ifs_config%irwa   =indrad_GPU(inext,nlev,.true., lacc)
  ifs_config%irra   =indrad_GPU(inext,nlev,.true., lacc)
  ifs_config%idp    =indrad_GPU(inext,nlev,.true., lacc)
  ifs_config%ioz    =indrad_GPU(inext,nlev,.false., lacc)
  ifs_config%iecpo3 =indrad_GPU(inext,nlev ,.false., lacc)
  ifs_config%ihpr   =indrad_GPU(inext,nlev+1,.true., lacc) ! not used in ecrad
  ifs_config%iaprs  =indrad_GPU(inext,nlev+1,.true., lacc)
  ifs_config%ihti   =indrad_GPU(inext,nlev+1,.true., lacc)
  ifs_config%iaero  =indrad_GPU(inext,yradiation%rad_config%n_aerosol_types*nlev,&
                          & llactaero .and. yradiation%yrerad%naermacc==0, lacc)
  ...
  if (yradiation%yrerad%naermacc == 1) then
    ifs_config%iaero = indrad_GPU(inext,yradiation%rad_config%n_aerosol_types*nlev,&
                            & yradiation%yrerad%ldiagforcing, lacc)
  endif
  ifs_config%ifrsod =indrad_GPU(inext,1,.true., lacc)
  ifs_config%ifrted =indrad_GPU(inext,yradiation%yrerad%nlwout,.true., lacc)
  ifs_config%ifrsodc=indrad_GPU(inext,1,.true., lacc)
  ifs_config%ifrtedc=indrad_GPU(inext,1,.true., lacc)
  ifs_config%iemit  =indrad_GPU(inext,1,.true., lacc)
  ifs_config%isudu  =indrad_GPU(inext,1,.true., lacc)
  ifs_config%iuvdf  =indrad_GPU(inext,1,.true., lacc)
  ifs_config%iparf  =indrad_GPU(inext,1,.true., lacc)
  ifs_config%iparcf =indrad_GPU(inext,1,.true., lacc)
  ifs_config%itincf =indrad_GPU(inext,1,.true., lacc)
  ifs_config%ifdir  =indrad_GPU(inext,1,.true., lacc)
  ifs_config%ifdif  =indrad_GPU(inext,1,.true., lacc)
  ifs_config%icdir  =indrad_GPU(inext,1,.true., lacc)
  ifs_config%ilwderivative =indrad_GPU(inext,nlev+1, yradiation%yrerad%lapproxlwupdate, lacc)
  ifs_config%iswdirectband =indrad_GPU(inext,yradiation%yrerad%nsw,yradiation%yrerad%lapproxswupdate, lacc)
  ifs_config%iswdiffuseband=indrad_GPU(inext,yradiation%yrerad%nsw,yradiation%yrerad%lapproxswupdate, lacc)
  ifs_config%ifrso  =indrad_GPU(inext,nlev+1,.true., lacc)
  ifs_config%iswfc  =indrad_GPU(inext,nlev+1,.true., lacc)
  ifs_config%ifrth  =indrad_GPU(inext,nlev+1,.true., lacc)
  ifs_config%ilwfc  =indrad_GPU(inext,nlev+1,.true., lacc)
  ifs_config%iaer   =indrad_GPU(inext,6*nlev,yradiation%yrerad%ldiagforcing, lacc)
  ifs_config%ioz    =indrad_GPU(inext,nlev,yradiation%yrerad%ldiagforcing, lacc)
  ifs_config%iico2  =indrad_GPU(inext,nlev,yradiation%yrerad%ldiagforcing, lacc)
  ifs_config%iich4  =indrad_GPU(inext,nlev,yradiation%yrerad%ldiagforcing, lacc)
  ifs_config%iin2o  =indrad_GPU(inext,nlev,yradiation%yrerad%ldiagforcing, lacc)
  ifs_config%ino2   =indrad_GPU(inext,nlev,yradiation%yrerad%ldiagforcing, lacc)
  ifs_config%ic11   =indrad_GPU(inext,nlev,yradiation%yrerad%ldiagforcing, lacc)
  ifs_config%ic12   =indrad_GPU(inext,nlev,yradiation%yrerad%ldiagforcing, lacc)
  ifs_config%ic22   =indrad_GPU(inext,nlev,yradiation%yrerad%ldiagforcing, lacc)
  ifs_config%icl4   =indrad_GPU(inext,nlev,yradiation%yrerad%ldiagforcing, lacc)
  ifs_config%igix   =indrad_GPU(inext,1,lldebug, lacc)
  ...
  if(.not.yradiation%yrerad%ldiagforcing) then
    if (yradiation%rad_config%n_aerosol_types == 0 .or. yradiation%yrerad%naermacc == 1) then
      ifs_config%iaero = indrad_GPU(inext,yradiation%rad_config%n_aerosol_types*nlev,.true., lacc)
    endif
    ifs_config%iaer   =indrad_GPU(inext,nlev*6,.true., lacc)
    ifs_config%ioz    =indrad_GPU(inext,nlev,.true., lacc)
    ifs_config%iico2  =indrad_GPU(inext,nlev,.true., lacc)
    ifs_config%iich4  =indrad_GPU(inext,nlev,.true., lacc)
    ifs_config%iin2o  =indrad_GPU(inext,nlev,.true., lacc)
    ifs_config%ino2   =indrad_GPU(inext,nlev,.true., lacc)
    ifs_config%ic11   =indrad_GPU(inext,nlev,.true., lacc)
    ifs_config%ic12   =indrad_GPU(inext,nlev,.true., lacc)
    ifs_config%ic22   =indrad_GPU(inext,nlev,.true., lacc)
    ifs_config%icl4   =indrad_GPU(inext,nlev,.true., lacc)
  endif
  ...
  ifs_config%ire_liq =indrad_GPU(inext,nlev,.true., lacc)
  ifs_config%ire_ice =indrad_GPU(inext,nlev,.true., lacc)
  ifs_config%ioverlap =indrad_GPU(inext,nlev-1,.true., lacc)
...
end subroutine ifs_setup_indices_GPU

subroutine ifs_copy_inputs_to_blocked_GPU ( &
  ...
  & zrgp, thermodynamics_out, iseed, lacc)
  ...
  associate(yderad=>yradiation%yrerad, rad_config=>yradiation%rad_config)
    ...
    do jrl=1,ncol,nproma
      ...
      call gas%get_GPU(ICO2, IMassMixingRatio, zrgp(1:il,ifs_config%iico2:ifs_config%iico2+nlev-1,ib), istartcol=ibeg, lacc=lacc)
      call gas%get_GPU(ICH4, IMassMixingRatio, zrgp(1:il,ifs_config%iich4:ifs_config%iich4+nlev-1,ib), istartcol=ibeg, lacc=lacc)
      call gas%get_GPU(IN2O, IMassMixingRatio, zrgp(1:il,ifs_config%iin2o:ifs_config%iin2o+nlev-1,ib), istartcol=ibeg, lacc=lacc)
      call gas%get_GPU(ICFC11, IMassMixingRatio, zrgp(1:il,ifs_config%ic11:ifs_config%ic11+nlev-1,ib), istartcol=ibeg, lacc=lacc)
      call gas%get_GPU(ICFC12, IMassMixingRatio, zrgp(1:il,ifs_config%ic12:ifs_config%ic12+nlev-1,ib), istartcol=ibeg, lacc=lacc)
      call gas%get_GPU(IHCFC22,IMassMixingRatio, zrgp(1:il,ifs_config%ic22:ifs_config%ic22+nlev-1,ib), istartcol=ibeg, lacc=lacc)
      call gas%get_GPU(ICCL4,  IMassMixingRatio, zrgp(1:il,ifs_config%icl4:ifs_config%icl4+nlev-1,ib), istartcol=ibeg, lacc=lacc)
      call gas%get_GPU(IO3, IMassMixingRatio, zrgp(1:il,ifs_config%ioz:ifs_config%ioz+nlev-1,ib), istartcol=ibeg, lacc=lacc)
...
end subroutine ifs_copy_inputs_to_blocked_GPU

subroutine ifs_copy_fluxes_from_blocked_GPU(&
    & driver_config, ifs_config, yradiation, ncol, nlev,&
    & zrgp, flux, flux_sw_direct_normal, flux_uv, flux_par, flux_par_clear,&
    & emissivity_out, flux_diffuse_band, flux_direct_band, lacc)
  use ecrad_driver_config
  use radiation_setup
  use radiation_flux
...
end subroutine ifs_copy_fluxes_from_blocked_GPU

integer(kind=jpim) function indrad_CPU(knext,kflds,lduse)
...
end function indrad_CPU

subroutine ifs_setup_indices_CPU (driver_config, ifs_config, yradiation, nlev)
  ...
  inext  =1
  iinbeg =1                        ! start of input variables
  ifs_config%igi    =indrad_CPU(inext,1,lldebug)
  ifs_config%imu0   =indrad_CPU(inext,1,.true.)
  ifs_config%iamu0  =indrad_CPU(inext,1,.true.)
  ifs_config%iemiss =indrad_CPU(inext,yradiation%yrerad%nlwemiss,.true.)
  ifs_config%its    =indrad_CPU(inext,1,.true.)
  ifs_config%islm   =indrad_CPU(inext,1,.true.)
  ifs_config%iccnl  =indrad_CPU(inext,1,.true.)
  ifs_config%iccno  =indrad_CPU(inext,1,.true.)
  ifs_config%ibas   =indrad_CPU(inext,1,.true.)
  ifs_config%itop   =indrad_CPU(inext,1,.true.)
  ifs_config%igelam =indrad_CPU(inext,1,.true.)
  ifs_config%igemu  =indrad_CPU(inext,1,.true.)
  ifs_config%iclon  =indrad_CPU(inext,1,.true.)
  ifs_config%islon  =indrad_CPU(inext,1,.true.)
  ifs_config%iald   =indrad_CPU(inext,yradiation%yrerad%nsw,.true.)
  ifs_config%ialp   =indrad_CPU(inext,yradiation%yrerad%nsw,.true.)
  ifs_config%iti    =indrad_CPU(inext,nlev,.true.)
  ifs_config%ipr    =indrad_CPU(inext,nlev,.true.)
  ifs_config%iqs    =indrad_CPU(inext,nlev,.true.)
  ifs_config%iwv    =indrad_CPU(inext,nlev,.true.)
  ifs_config%iclc   =indrad_CPU(inext,nlev,.true.)
  ifs_config%ilwa   =indrad_CPU(inext,nlev,.true.)
  ifs_config%iiwa   =indrad_CPU(inext,nlev,.true.)
  ifs_config%iswa   =indrad_CPU(inext,nlev,.true.)
  ifs_config%irwa   =indrad_CPU(inext,nlev,.true.)
  ifs_config%irra   =indrad_CPU(inext,nlev,.true.)
  ifs_config%idp    =indrad_CPU(inext,nlev,.true.)
  ifs_config%ioz    =indrad_CPU(inext,nlev,.false.)
  ifs_config%iecpo3 =indrad_CPU(inext,nlev ,.false.)
  ifs_config%ihpr   =indrad_CPU(inext,nlev+1,.true.) ! not used in ecrad
  ifs_config%iaprs  =indrad_CPU(inext,nlev+1,.true.)
  ifs_config%ihti   =indrad_CPU(inext,nlev+1,.true.)
  ifs_config%iaero  =indrad_CPU(inext,yradiation%rad_config%n_aerosol_types*nlev,&
  ...
  ifs_config%ifrsod =indrad_CPU(inext,1,.true.)
  ifs_config%ifrted =indrad_CPU(inext,yradiation%yrerad%nlwout,.true.)
  ifs_config%ifrsodc=indrad_CPU(inext,1,.true.)
  ifs_config%ifrtedc=indrad_CPU(inext,1,.true.)
  ifs_config%iemit  =indrad_CPU(inext,1,.true.)
  ifs_config%isudu  =indrad_CPU(inext,1,.true.)
  ifs_config%iuvdf  =indrad_CPU(inext,1,.true.)
  ifs_config%iparf  =indrad_CPU(inext,1,.true.)
  ifs_config%iparcf =indrad_CPU(inext,1,.true.)
  ifs_config%itincf =indrad_CPU(inext,1,.true.)
  ifs_config%ifdir  =indrad_CPU(inext,1,.true.)
  ifs_config%ifdif  =indrad_CPU(inext,1,.true.)
  ifs_config%icdir  =indrad_CPU(inext,1,.true.)
  ifs_config%ilwderivative =indrad_CPU(inext,nlev+1, yradiation%yrerad%lapproxlwupdate)
  ifs_config%iswdirectband =indrad_CPU(inext,yradiation%yrerad%nsw,yradiation%yrerad%lapproxswupdate)
  ifs_config%iswdiffuseband=indrad_CPU(inext,yradiation%yrerad%nsw,yradiation%yrerad%lapproxswupdate)
  ifs_config%ifrso  =indrad_CPU(inext,nlev+1,.true.)
  ifs_config%iswfc  =indrad_CPU(inext,nlev+1,.true.)
  ifs_config%ifrth  =indrad_CPU(inext,nlev+1,.true.)
  ifs_config%ilwfc  =indrad_CPU(inext,nlev+1,.true.)
  ifs_config%iaer   =indrad_CPU(inext,6*nlev,yradiation%yrerad%ldiagforcing)
  ifs_config%ioz    =indrad_CPU(inext,nlev,yradiation%yrerad%ldiagforcing)
  ifs_config%iico2  =indrad_CPU(inext,nlev,yradiation%yrerad%ldiagforcing)
  ifs_config%iich4  =indrad_CPU(inext,nlev,yradiation%yrerad%ldiagforcing)
  ifs_config%iin2o  =indrad_CPU(inext,nlev,yradiation%yrerad%ldiagforcing)
  ifs_config%ino2   =indrad_CPU(inext,nlev,yradiation%yrerad%ldiagforcing)
  ifs_config%ic11   =indrad_CPU(inext,nlev,yradiation%yrerad%ldiagforcing)
  ifs_config%ic12   =indrad_CPU(inext,nlev,yradiation%yrerad%ldiagforcing)
  ifs_config%ic22   =indrad_CPU(inext,nlev,yradiation%yrerad%ldiagforcing)
  ifs_config%icl4   =indrad_CPU(inext,nlev,yradiation%yrerad%ldiagforcing)
  ifs_config%igix   =indrad_CPU(inext,1,lldebug)
  ...
  if(.not.yradiation%yrerad%ldiagforcing) then
    if (yradiation%rad_config%n_aerosol_types == 0 .or. yradiation%yrerad%naermacc == 1) then
      ifs_config%iaero = indrad_CPU(inext,yradiation%rad_config%n_aerosol_types*nlev,.true.)
    endif
    ifs_config%iaer   =indrad_CPU(inext,nlev*6,.true.)
    ifs_config%ioz    =indrad_CPU(inext,nlev,.true.)
    ifs_config%iico2  =indrad_CPU(inext,nlev,.true.)
    ifs_config%iich4  =indrad_CPU(inext,nlev,.true.)
    ifs_config%iin2o  =indrad_CPU(inext,nlev,.true.)
    ifs_config%ino2   =indrad_CPU(inext,nlev,.true.)
    ifs_config%ic11   =indrad_CPU(inext,nlev,.true.)
    ifs_config%ic12   =indrad_CPU(inext,nlev,.true.)
    ifs_config%ic22   =indrad_CPU(inext,nlev,.true.)
    ifs_config%icl4   =indrad_CPU(inext,nlev,.true.)
  endif
                                ! end of local variables
  ...
  ! To validate results against standalone ecrad, we overwrite effective
  ! radii, cloud overlap and seed with input values
  ifs_config%ire_liq =indrad_CPU(inext,nlev,.true.)
  ifs_config%ire_ice =indrad_CPU(inext,nlev,.true.)
  ifs_config%ioverlap =indrad_CPU(inext,nlev-1,.true.)
                                  ! end of standalone inputs workaround variables
...
end subroutine ifs_setup_indices_CPU

subroutine ifs_copy_inputs_to_blocked_CPU ( &
  & driver_config, ifs_config, yradiation, ncol, nlev, &
  & single_level, thermodynamics, gas, cloud, aerosol, &
  & sin_latitude, longitude_rad, land_frac, pressure_fl, temperature_fl, &
  ...
  associate(yderad=>yradiation%yrerad, rad_config=>yradiation%rad_config)
    ...
    do jrl=1,ncol,nproma
      ...
      ! -- by default, globally averaged concentrations (mmr)
      call gas%get_CPU(ICO2, IMassMixingRatio, zrgp(1:il,ifs_config%iico2:ifs_config%iico2+nlev-1,ib), istartcol=ibeg)
      call gas%get_CPU(ICH4, IMassMixingRatio, zrgp(1:il,ifs_config%iich4:ifs_config%iich4+nlev-1,ib), istartcol=ibeg)
      call gas%get_CPU(IN2O, IMassMixingRatio, zrgp(1:il,ifs_config%iin2o:ifs_config%iin2o+nlev-1,ib), istartcol=ibeg)
      call gas%get_CPU(ICFC11, IMassMixingRatio, zrgp(1:il,ifs_config%ic11:ifs_config%ic11+nlev-1,ib), istartcol=ibeg)
      call gas%get_CPU(ICFC12, IMassMixingRatio, zrgp(1:il,ifs_config%ic12:ifs_config%ic12+nlev-1,ib), istartcol=ibeg)
      call gas%get_CPU(IHCFC22,IMassMixingRatio, zrgp(1:il,ifs_config%ic22:ifs_config%ic22+nlev-1,ib), istartcol=ibeg)
      call gas%get_CPU(ICCL4,  IMassMixingRatio, zrgp(1:il,ifs_config%icl4:ifs_config%icl4+nlev-1,ib), istartcol=ibeg)
      call gas%get_CPU(IO3, IMassMixingRatio, zrgp(1:il,ifs_config%ioz:ifs_config%ioz+nlev-1,ib), istartcol=ibeg)
      ! convert ozone kg/kg to Pa*kg/kg
      ! do jlev=1,nlev
      !   zrgp(1:il,ifs_config%ioz+jlev-1,ib)  = zrgp(1:il,ifs_config%ioz+jlev-1,ib) &
      !         &                       * (thermodynamics%pressure_hl(ibeg:iend,jlev+1) &
...
end subroutine ifs_copy_inputs_to_blocked_CPU

subroutine ifs_copy_fluxes_from_blocked_CPU(&
    & driver_config, ifs_config, yradiation, ncol, nlev,&
    & zrgp, flux, flux_sw_direct_normal, flux_uv, flux_par, flux_par_clear,&
...
end subroutine ifs_copy_fluxes_from_blocked_CPU
