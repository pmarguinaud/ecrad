SUBROUTINE LIQUID_EFFECTIVE_RADIUS_GPU &
...
CASE(0)
  ...
  !$ACC KERNELS DEFAULT(NONE) PRESENT(PRE_UM, PPRESSURE) IF(lacc)
...
CASE(1)
  ...
  ! and Rossow
  !$ACC PARALLEL DEFAULT(NONE) PRESENT(PRE_UM, PLAND_FRAC) ASYNC(1) IF(lacc)
...
CASE(2)
  ...
  !$ACC     PQ_LIQ, PQ_RAIN, PPRESSURE, PTEMPERATURE, PCLOUD_FRAC, PRE_UM) ASYNC(1) IF(lacc)
...
END SUBROUTINE LIQUID_EFFECTIVE_RADIUS_GPU


SUBROUTINE LIQUID_EFFECTIVE_RADIUS_CPU &
     & (YDERAD,KIDIA, KFDIA, KLON, KLEV, &
     &  PPRESSURE, PTEMPERATURE, PCLOUD_FRAC, PQ_LIQ, PQ_RAIN, &
     &  PLAND_FRAC, PCCN_LAND, PCCN_SEA, &
     &  PRE_UM) !, PPERT)
...
END SUBROUTINE LIQUID_EFFECTIVE_RADIUS_CPU
