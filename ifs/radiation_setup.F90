...
CONTAINS
  ...
  SUBROUTINE SETUP_RADIATION_SCHEME_GPU(PRADIATION,LDOUTPUT,FILE_NAME, lacc)
...
!!      CALL ABOR1('RADIATION_SETUP: error interpreting NICEOPT')   !db fix
    ...
    CALL SETUP_RADIATION_GPU(RAD_CONFIG, lacc=lacc)
  ...
  SUBROUTINE SETUP_RADIATION_SCHEME_CPU(PRADIATION,LDOUTPUT,FILE_NAME)
...
!!      CALL ABOR1('RADIATION_SETUP: error interpreting NICEOPT')   !db fix
    ...
    ! Use configuration data to set-up radiation scheme, including
    ! reading scattering datafiles
    CALL SETUP_RADIATION_CPU(RAD_CONFIG)
