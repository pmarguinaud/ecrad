SUBROUTINE RRTM_TAUMOL1_GPU (KIDIA,KFDIA,KLEV,taug,PAVEL,&
...
REAL(KIND=JPRB) :: taufor,tauself,corradj,pp,scalen2, taun2
    ...
    !$ACC             INDFOR, INDMINOR, SCALEMINORN2, COLBRD) IF(lacc)
...
!$NEC unroll(NG1)
          do ig = 1, ng1
            ...
            taug(jl,ig,lay) = corradj * (colh2o(jl,lay) * &
                 ...
                 + tauself + taufor + taun2) + p_tauaerl(jl,lay,1)
...
!$NEC unroll(NG1)
            ...
            do ig = 1, ng1
              ...
              taug(jl,ig,lay) = corradj * (colh2o(jl,lay) * &
                  ...
                  fac01(jl,lay) * absb(ind1,ig) + &
                  fac11(jl,lay) * absb(ind1+1,ig)) &
                  + taufor + taun2) + p_tauaerl(jl,lay,1)
              fracs(jl,ig,lay) = fracrefb(ig)
            enddo
           endif
          enddo
...
END SUBROUTINE RRTM_TAUMOL1_GPU


SUBROUTINE RRTM_TAUMOL1_CPU (KIDIA,KFDIA,KLEV,taug,PAVEL,&
 & P_TAUAERL,FAC00,FAC01,FAC10,FAC11,FORFAC,FORFRAC,INDFOR,JP,JT,jt1,&
 & COLH2O,LAYTROP,SELFFAC,SELFFRAC,INDSELF,fracs,MINORFRAC,INDMINOR,&
 & SCALEMINORN2,COLBRD)
...
END SUBROUTINE RRTM_TAUMOL1_CPU
