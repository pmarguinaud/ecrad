!----------------------------------------------------------------------------
SUBROUTINE RRTM_TAUMOL12_GPU (KIDIA,KFDIA,KLEV,taug,&
...
REAL(KIND=JPRB) :: refrat_planck_a
    ...
    !$ACC             rat_h2oco2, rat_h2oco2_1, indfor, forfrac, forfac) IF(lacc)
...
!$NEC unroll(NG12)
          do ig = 1, ng12
            ...
            taug(jl,ngs11+ig,lay) = tau_major(ig) + tau_major1(ig) &
                 + tauself + taufor + p_tauaerl(jl,lay,12)
...
END SUBROUTINE RRTM_TAUMOL12_GPU


!----------------------------------------------------------------------------
SUBROUTINE RRTM_TAUMOL12_CPU (KIDIA,KFDIA,KLEV,taug,&
 & P_TAUAERL,fac00,fac01,fac10,fac11,forfac,forfrac,indfor,jp,jt,jt1,oneminus,&
 & colh2o,colco2,laytrop,selffac,selffrac,indself,fracs, &
...
END SUBROUTINE RRTM_TAUMOL12_CPU
