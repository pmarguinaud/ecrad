!----------------------------------------------------------------------------
SUBROUTINE RRTM_TAUMOL16_GPU (KIDIA,KFDIA,KLEV,taug,&
...
& fpl, specmult_planck, specparm_planck,speccomb_planck
    ...
    !$ACC             fracs, rat_h2och4, rat_h2och4_1, indfor, forfac, forfrac) IF(lacc)
...
!$NEC unroll(NG16)
          do ig = 1, ng16
            ...
            taug(jl,ngs15+ig,lay) = tau_major(ig) + tau_major1(ig) &
                 + tauself + taufor + p_tauaerl(jl,lay,16)
...
!$NEC unroll(NG16)
          do ig = 1, ng16
            taug(jl,ngs15+ig,lay) = colch4(jl,lay) * &
                 ...
                 fac11(jl,lay) * absb(ind1+1,ig))  + p_tauaerl(jl,lay,16)
...
END SUBROUTINE RRTM_TAUMOL16_GPU


!----------------------------------------------------------------------------
SUBROUTINE RRTM_TAUMOL16_CPU (KIDIA,KFDIA,KLEV,taug,&
 & P_TAUAERL,fac00,fac01,fac10,fac11,forfac,forfrac,indfor,jp,jt,jt1,oneminus,&
 & colh2o,colch4,laytrop,selffac,selffrac,indself,fracs, &
...
END SUBROUTINE RRTM_TAUMOL16_CPU
