!----------------------------------------------------------------------------
SUBROUTINE RRTM_TAUMOL2_GPU (KIDIA,KFDIA,KLEV,taug,PAVEL,coldry,&
 & P_TAUAERL,fac00,fac01,fac10,fac11,forfac,forfrac,indfor,jp,jt,jt1,&
 & colh2o,laytrop,selffac,selffrac,indself,fracs, lacc)
...
REAL(KIND=JPRB) :: taufor,tauself,corradj,pp
    ...
    !$ACC             selffac, selffrac, indself, indfor, fracs) IF(lacc)
...
!$NEC unroll(NG2)
          do ig = 1, ng2
            ...
            taug(jl,ngs1+ig,lay) = corradj * (colh2o(jl,lay) * &
                 ...
                 + tauself + taufor) + p_tauaerl(jl,lay,2)
...
END SUBROUTINE RRTM_TAUMOL2_GPU


!----------------------------------------------------------------------------
SUBROUTINE RRTM_TAUMOL2_CPU (KIDIA,KFDIA,KLEV,taug,PAVEL,coldry,&
 & P_TAUAERL,fac00,fac01,fac10,fac11,forfac,forfrac,indfor,jp,jt,jt1,&
...
END SUBROUTINE RRTM_TAUMOL2_CPU
