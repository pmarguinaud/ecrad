...
#include "radiation_delta_eddington.h"
  ...
  subroutine setup_aerosol_optics_GPU(config, lacc)
    ...
    if (config%n_aerosol_types > 0) then
      ...
      if (config%use_general_aerosol_optics) then
        ...
        call setup_general_aerosol_optics_GPU(config, lacc=lacc)
      else
        ! Read file containing optical properties already in the bands
        ! of the gas-optics scheme
        call config%aerosol_optics%setup_GPU(trim(config%aerosol_optics_file_name), &
             &                           iverbose=config%iverbosesetup, lacc=lacc)
      end if

      call config%aerosol_optics%initialize_types_GPU(config%n_aerosol_types, lacc=lacc)
      ...
      call config%aerosol_optics%set_types_GPU(config%i_aerosol_type_map(1:config%n_aerosol_types), lacc=lacc)
    end if

    if (config%iverbosesetup >= 1) then
      call config%aerosol_optics%print_description_GPU(config%i_aerosol_type_map(1:config%n_aerosol_types), lacc=lacc)
    end if
  ...
  subroutine setup_general_aerosol_optics_GPU(config, lacc)
    ...
    associate(ao => config%aerosol_optics)
      ...
      if (.not. file%exists('wavenumber')) then
        ...
        call setup_general_aerosol_optics_legacy_GPU(config, trim(config%aerosol_optics_file_name), lacc=lacc)
      ...
      call ao%allocate_GPU(n_type_phobic, n_type_philic, nrh, nspeclw, nspecsw, nmono, lacc=lacc)

      if (config%do_sw) then
        call config%gas_optics_sw%spectral_def%calc_mapping_GPU(wavenumber, mapping, &
            &          use_bands=(.not. config%do_cloud_aerosol_per_sw_g_point), lacc=lacc)
      ...
      if (config%do_lw) then
        call config%gas_optics_lw%spectral_def%calc_mapping_GPU(wavenumber, mapping, &
            &          use_bands=(.not. config%do_cloud_aerosol_per_lw_g_point), lacc=lacc)
  ...
  subroutine setup_general_aerosol_optics_legacy_GPU(config, file_name, lacc)
    ...
    call ao_legacy%setup_GPU(file_name, iverbose=config%iverbosesetup, lacc=lacc)

    ! Copy over scalars and coordinate variables
    call ao%allocate_GPU(ao_legacy%n_type_phobic, ao_legacy%n_type_philic, ao_legacy%nrh, &
         &           config%n_bands_lw, config%n_bands_sw, ao_legacy%n_mono_wl, lacc=lacc)
    ...
    if (config%do_sw) then
      call config%gas_optics_sw%spectral_def%calc_mapping_from_wavenumber_bands_GPU( &
    ...
    if (config%do_lw) then
      ...
      call config%gas_optics_lw%spectral_def%calc_mapping_from_wavenumber_bands_GPU( &
  ...
  subroutine add_aerosol_optics_GPU(ng_sw_in, ng_lw_in, nbnd_sw_in, nbnd_lw_in, &
    ...
    ! spectrum
    real(jprb), dimension(nbnd_sw_in,nlev,istartcol:iendcol) &
         &  :: od_sw_aerosol, scat_sw_aerosol, scat_g_sw_aerosol
    real(jprb), dimension(nbnd_lw_in,nlev,istartcol:iendcol) &
         &  :: od_lw_aerosol
    ...
    if (aerosol%is_direct) then
      ...
      call add_aerosol_optics_direct_GPU(nlev,istartcol,iendcol, &
           &  config, aerosol, &
      ...
      call get_gas_GPU(gas, IH2O, IMassMixingRatio, h2o_mmr, istartcol=istartcol, lacc=lacc)
      ...
      do jcol = istartcol,iendcol
        ...
        do jlev = istartlev,iendlev
          ...
          irhs(jlev,jcol) = calc_rh_index_GPU(config%aerosol_optics, rh)
        ...
        if (.not. config%do_sw_delta_scaling_with_gases) then
          ...
          ! the solver
          call delta_eddington_extensive_vec(nbnd_sw_in*nlev, od_sw_aerosol(:,:,jcol), &
        ...
        if (config%do_lw_aerosol_scattering) then
          ...
          call delta_eddington_extensive_vec(nbnd_lw_in*nlev, od_lw_aerosol(:,:,jcol), &
  ...
  subroutine add_aerosol_optics_direct_GPU(nlev,istartcol,iendcol, &
    ...
    !$ACC DATA PRESENT(config, aerosol) IF(lacc)
    ...
    if (config%do_sw) then
      ...
      !$ACC DATA PRESENT(od_sw, ssa_sw, g_sw) IF(lacc)
      ...
      !$ACC   VECTOR_LENGTH(((config%n_g_sw-1)/32+1)*32) ASYNC(1) IF(lacc)
...
!NEC$ forced_collapse
        ...
        !$ACC   NUM_GANGS(iendcol-istartcol+1) &
        !$ACC   VECTOR_LENGTH(((config%n_g_lw-1)/32+1)*32) ASYNC(1) IF(lacc)
...
!NEC$ forced_collapse
  ...
  function dry_aerosol_mass_extinction_GPU(config, itype, wavelength, lacc)
    ...
    if (ao%iclass(itype) == IAerosolClassHydrophobic) then
      dry_aerosol_mass_extinction_GPU = ao%mass_ext_mono_phobic(imono,ao%itype(itype))
    else if (ao%iclass(itype) == IAerosolClassHydrophilic) then
      ! Take the value at the first relative-humidity bin for the
      ! "dry" aerosol value
      dry_aerosol_mass_extinction_GPU = ao%mass_ext_mono_philic(imono,1,ao%itype(itype))
    else
      dry_aerosol_mass_extinction_GPU = 0.0_jprb
    end if
  ...
  subroutine aerosol_extinction_GPU(ncol,istartcol,iendcol, &
    ...
    do jcol = istartcol,iendcol
      ...
      irh = ao%calc_rh_index_GPU(relative_humidity(jcol))
  ...
  subroutine setup_aerosol_optics_CPU(config)
    ...
    if (config%n_aerosol_types > 0) then
      ...
      if (config%use_general_aerosol_optics) then
        ...
        ! properties and average to the spectral intervals of the
        ! current gas-optics scheme
        call setup_general_aerosol_optics_CPU(config)
      else
        ! Read file containing optical properties already in the bands
        ! of the gas-optics scheme
        call config%aerosol_optics%setup_CPU(trim(config%aerosol_optics_file_name), &
             &                           iverbose=config%iverbosesetup)
      end if

      call config%aerosol_optics%initialize_types_CPU(config%n_aerosol_types)
      ...
      ! Map aerosol types to those loaded from the data file
      call config%aerosol_optics%set_types_CPU(config%i_aerosol_type_map(1:config%n_aerosol_types))
    end if

    if (config%iverbosesetup >= 1) then
      call config%aerosol_optics%print_description_CPU(config%i_aerosol_type_map(1:config%n_aerosol_types))
    end if
  ...
  subroutine setup_general_aerosol_optics_CPU(config)
    ...
    associate(ao => config%aerosol_optics)
      ...
      if (.not. file%exists('wavenumber')) then
        ...
        call setup_general_aerosol_optics_legacy_CPU(config, trim(config%aerosol_optics_file_name))
        if (lhook) call dr_hook('radiation_aerosol_optics:setup_general_aerosol_optics',1,hook_handle)
        return
      end if
      ...
      call ao%allocate_CPU(n_type_phobic, n_type_philic, nrh, nspeclw, nspecsw, nmono)

      if (config%do_sw) then
        call config%gas_optics_sw%spectral_def%calc_mapping_CPU(wavenumber, mapping, &
      ...
      if (config%do_lw) then
        call config%gas_optics_lw%spectral_def%calc_mapping_CPU(wavenumber, mapping, &
  ...
  subroutine setup_general_aerosol_optics_legacy_CPU(config, file_name)
    ...
    ! Load file into a local structure
    call ao_legacy%setup_CPU(file_name, iverbose=config%iverbosesetup)

    ! Copy over scalars and coordinate variables
    call ao%allocate_CPU(ao_legacy%n_type_phobic, ao_legacy%n_type_philic, ao_legacy%nrh, &
    ...
    if (config%do_sw) then
      call config%gas_optics_sw%spectral_def%calc_mapping_from_wavenumber_bands_CPU( &
           &  ao_legacy%wavenumber1_sw, ao_legacy%wavenumber2_sw, mapping_transp, &
           &  use_bands=(.not. config%do_cloud_aerosol_per_sw_g_point))
      if (allocated(mapping)) then
        deallocate(mapping)
    ...
    if (config%do_lw) then
      ...
      call config%gas_optics_lw%spectral_def%calc_mapping_from_wavenumber_bands_CPU( &
           &  ao_legacy%wavenumber1_lw, ao_legacy%wavenumber2_lw, mapping_transp, &
           &  use_bands=(.not. config%do_cloud_aerosol_per_lw_g_point))
      if (allocated(mapping)) then
        deallocate(mapping)
  ...
  subroutine add_aerosol_optics_CPU(ng_sw_in, ng_lw_in, nbnd_sw_in, nbnd_lw_in, &
    ...
    if (aerosol%is_direct) then
      ! Aerosol optical properties have been provided in each band
      ! directly by the user
      call add_aerosol_optics_direct_CPU(nlev,istartcol,iendcol, &
           &  config, aerosol, &
           &  od_lw, ssa_lw, g_lw, od_sw, ssa_sw, g_sw)
    else
      ! Aerosol mixing ratios have been provided
      ...
      !call gas%get(IH2O, IMassMixingRatio, h2o_mmr, istartcol=istartcol)
      call get_gas_CPU(gas, IH2O, IMassMixingRatio, h2o_mmr, istartcol=istartcol)
      ...
      do jcol = istartcol,iendcol
        ...
        do jlev = istartlev,iendlev
          ...
          ! hydrophilic-aerosol data
          rh  = h2o_mmr(jcol,jlev) / thermodynamics%h2o_sat_liq(jcol,jlev)
          irhs(jlev,jcol) = calc_rh_index_CPU(config%aerosol_optics, rh)
          ! irhs(jlev,jcol) = config%aerosol_optics%calc_rh_index(rh)
...
!NEC$ forced_collapse
  ...
  function dry_aerosol_mass_extinction_CPU(config, itype, wavelength)
    ...
    if (ao%iclass(itype) == IAerosolClassHydrophobic) then
      dry_aerosol_mass_extinction_CPU = ao%mass_ext_mono_phobic(imono,ao%itype(itype))
    else if (ao%iclass(itype) == IAerosolClassHydrophilic) then
      ! Take the value at the first relative-humidity bin for the
      ! "dry" aerosol value
      dry_aerosol_mass_extinction_CPU = ao%mass_ext_mono_philic(imono,1,ao%itype(itype))
    else
      dry_aerosol_mass_extinction_CPU = 0.0_jprb
    end if
  ...
  subroutine aerosol_extinction_CPU(ncol,istartcol,iendcol, &
    ...
    do jcol = istartcol,iendcol
      ext = 0.0_jprb
      ! Get relative-humidity index
      irh = ao%calc_rh_index_CPU(relative_humidity(jcol))
      ! Add extinction coefficients from each aerosol type
      do jtype = 1,config%n_aerosol_types
        if (ao%iclass(jtype) == IAerosolClassHydrophobic) then
          ext = ext + mixing_ratio(jcol,jtype) &
