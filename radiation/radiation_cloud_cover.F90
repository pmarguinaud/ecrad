  ...
  function cloud_cover_GPU(nlev, i_overlap_scheme, frac, overlap_param, &
    ...
    if (i_overlap_scheme == IOverlapExponentialRandom) then
      call cum_cloud_cover_exp_ran_GPU(nlev, frac, overlap_param, &
           &   cum_cloud_cover, pair_cloud_cover, is_beta_overlap, lacc=lacc)
    else if (i_overlap_scheme == IOverlapExponential) then
      call cum_cloud_cover_exp_exp_GPU(nlev, frac, overlap_param, &
           &   cum_cloud_cover, pair_cloud_cover, is_beta_overlap, lacc=lacc)
    else
      call cum_cloud_cover_max_ran_GPU(nlev, frac, cum_cloud_cover, &
           &                       pair_cloud_cover, lacc=lacc)
    end if

    cloud_cover_GPU = cum_cloud_cover(nlev)
  ...
  subroutine cum_cloud_cover_exp_ran_GPU(nlev, frac, overlap_param, &
    ...
    do jlev = 1,nlev-1
      ...
      if (do_overlap_conversion) then
        overlap_alpha = beta2alpha_GPU(overlap_param(jlev), &
...
! Added for DWD (2020)
  ...
  subroutine cum_cloud_cover_exp_exp_GPU(nlev, frac, overlap_param, &
    ...
    if (nobj > 0) then
      ...
      if (.not. do_overlap_conversion) then
        ...
        overlap_alpha = beta2alpha_GPU(overlap_param, &
  ...
  function cloud_cover_CPU(nlev, i_overlap_scheme, frac, overlap_param, &
    ...
    if (i_overlap_scheme == IOverlapExponentialRandom) then
      ...
      call cum_cloud_cover_max_ran_CPU(nlev, frac, cum_cloud_cover, &
           &                       pair_cloud_cover)
    end if

    cloud_cover_CPU = cum_cloud_cover(nlev)
  ...
  subroutine cum_cloud_cover_exp_ran_CPU(nlev, frac, overlap_param, &
    ...
    do jlev = 1,nlev-1
      ! Convert to "alpha" overlap parameter if necessary
      if (do_overlap_conversion) then
        overlap_alpha = beta2alpha_CPU(overlap_param(jlev), &
             &                     frac(jlev), frac(jlev+1))
      else
        overlap_alpha = overlap_param(jlev)
      end if
  ...
  subroutine cum_cloud_cover_exp_exp_CPU(nlev, frac, overlap_param, &
    ...
    if (nobj > 0) then
      ...
      if (.not. do_overlap_conversion) then
        ...
        ! Convert Shonk et al overlap parameter to Hogan and
        ! Illingworth definition
        overlap_alpha = beta2alpha_CPU(overlap_param, &
             &                     frac(1:nlev-1), frac(2:nlev))
        ! Compute the combined cloud cover of pairs of layers
        do jlev = 1,nlev-1
          pair_cloud_cover(jlev) &
