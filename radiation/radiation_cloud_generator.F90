...
contains
  ...
  subroutine cloud_generator_GPU(ng, nlev, i_overlap_scheme, &
    ...
    if (i_overlap_scheme == IOverlapExponentialRandom) then
      call cum_cloud_cover_exp_ran_GPU(nlev, frac, overlap_param, &
           &   cum_cloud_cover, pair_cloud_cover, use_beta_overlap, lacc=lacc)
    else if (i_overlap_scheme == IOverlapMaximumRandom) then
      call cum_cloud_cover_max_ran_GPU(nlev, frac, &
           &   cum_cloud_cover, pair_cloud_cover, lacc=lacc)
    else if (i_overlap_scheme == IOverlapExponential) then
      call cum_cloud_cover_exp_exp_GPU(nlev, frac, overlap_param, &
           &   cum_cloud_cover, pair_cloud_cover, use_beta_overlap, lacc=lacc)
    else
      ...
      if (.not. use_vec_gen) then
        ...
        do jg = 1,ng
          ...
          if (i_overlap_scheme /= IOverlapExponential) then
            call generate_column_exp_ran_GPU(ng, nlev, jg, random_stream, pdf_sampler, &
                 &  frac, pair_cloud_cover, &
                 &  cum_cloud_cover, overhang, fractional_std, overlap_param_inhom, &
                 &  itrigger, iend, od_scaling, lacc=lacc)
          else
            call generate_column_exp_exp_GPU(ng, nlev, jg, random_stream, pdf_sampler, &
                 &  frac, pair_cloud_cover, &
                 &  cum_cloud_cover, overhang, fractional_std, overlap_param_inhom, &
                 &  itrigger, iend, od_scaling, lacc=lacc)
          end if
        ...
        call generate_columns_exp_ran_GPU(ng, nlev, iseed, pdf_sampler, &
             &  total_cloud_cover, frac_threshold, frac, pair_cloud_cover, &
             &  cum_cloud_cover, overhang, fractional_std, overlap_param_inhom, &
             &  ibegin, iend, od_scaling, lacc=lacc)
  ...
  subroutine generate_column_exp_ran_GPU(ng, nlev, ig, random_stream, pdf_sampler, &
    ...
    do jlev = itrigger+1,iend+1
      ...
      if (do_fill_od_scaling) then
        ...
        call pdf_sampler%sample_GPU(fractional_std(jlev-n_layers_to_scale:jlev-1), &
             & rand_inhom1(1:n_layers_to_scale), od_scaling(ig,jlev-n_layers_to_scale:jlev-1), lacc=lacc)
  ...
  subroutine generate_columns_exp_ran_GPU(ng, nlev, iseed, pdf_sampler, &
    ...
    logical :: found_cloud(ng)
    logical, intent (in) :: lacc ! Cloud found in this column counting down from top?
    ...
    call random_number_generator%initialize_GPU(IRngMinstdVector, iseed=iseed, &
         &                                  nmaxstreams=ng, lacc=lacc)

    ! Random numbers to use to locate cloud top
    call random_number_generator%uniform_distribution_GPU(trigger, lacc=lacc)
    ...
    ! cloudy
    call random_number_generator%uniform_distribution_GPU(rand_cloud, is_any_cloud, lacc=lacc)

    ! Random numbers to generate sub-grid cloud structure
    call random_number_generator%uniform_distribution_GPU(rand_inhom, lacc=lacc)
    call random_number_generator%uniform_distribution_GPU(rand_inhom2, is_any_cloud, lacc=lacc)
...
!NEC$ shortloop
    ...
    call pdf_sampler%masked_block_sample_GPU(iend-ibegin+1, ng, &
         &  fractional_std(ibegin:iend), &
         &  rand_inhom(:,ibegin:iend), od_scaling(:,ibegin:iend), &
         &  is_any_cloud, lacc=lacc)
  ...
  subroutine cloud_generator_CPU(ng, nlev, i_overlap_scheme, &
    ...
    if (i_overlap_scheme == IOverlapExponentialRandom) then
      call cum_cloud_cover_exp_ran_CPU(nlev, frac, overlap_param, &
           &   cum_cloud_cover, pair_cloud_cover, use_beta_overlap)
    else if (i_overlap_scheme == IOverlapMaximumRandom) then
      call cum_cloud_cover_max_ran_CPU(nlev, frac, &
           &   cum_cloud_cover, pair_cloud_cover)
    else if (i_overlap_scheme == IOverlapExponential) then
      call cum_cloud_cover_exp_exp_CPU(nlev, frac, overlap_param, &
           &   cum_cloud_cover, pair_cloud_cover, use_beta_overlap)
    else
      write(nulerr,'(a)') '*** Error: cloud overlap scheme not recognised'
      call radiation_abort()
    ...
    if (total_cloud_cover < frac_threshold) then
      ...
      if (.not. use_vec_gen) then
        ...
        do jg = 1,ng
          ...
          if (i_overlap_scheme /= IOverlapExponential) then
            call generate_column_exp_ran_CPU(ng, nlev, jg, random_stream, pdf_sampler, &
                 &  frac, pair_cloud_cover, &
                 &  cum_cloud_cover, overhang, fractional_std, overlap_param_inhom, &
                 &  itrigger, iend, od_scaling)
          else
            call generate_column_exp_exp_CPU(ng, nlev, jg, random_stream, pdf_sampler, &
                 &  frac, pair_cloud_cover, &
                 &  cum_cloud_cover, overhang, fractional_std, overlap_param_inhom, &
                 &  itrigger, iend, od_scaling)
          end if
        ...
        call generate_columns_exp_ran_CPU(ng, nlev, iseed, pdf_sampler, &
             &  total_cloud_cover, frac_threshold, frac, pair_cloud_cover, &
             &  cum_cloud_cover, overhang, fractional_std, overlap_param_inhom, &
  ...
  subroutine generate_column_exp_ran_CPU(ng, nlev, ig, random_stream, pdf_sampler, &
    ...
    do jlev = itrigger+1,iend+1
      ...
      if (do_fill_od_scaling) then
        ...
        ! Sample from a lognormal or gamma distribution to obtain
        ! the optical depth scalings
        call pdf_sampler%sample_CPU(fractional_std(jlev-n_layers_to_scale:jlev-1), &
  ...
  subroutine generate_column_exp_exp_CPU(ng, nlev, ig, random_stream, pdf_sampler, &
    ...
    ! Masked version assuming values outside the range itrigger:iend
    ! are already zero:
    call pdf_sampler%masked_sample_CPU(n_layers_to_scale, &
         &  fractional_std(itrigger:iend), &
         &  rand_inhom1(1:n_layers_to_scale), od_scaling(ig,itrigger:iend), &
  ...
  subroutine generate_columns_exp_ran_CPU(ng, nlev, iseed, pdf_sampler, &
    ...
    ! that random numbers will be requested in blocks of length the
    ! number of spectral intervals ng.
    call random_number_generator%initialize_CPU(IRngMinstdVector, iseed=iseed, &
         &                                  nmaxstreams=ng)

    ! Random numbers to use to locate cloud top
    call random_number_generator%uniform_distribution_CPU(trigger)

    ! Random numbers to work out whether to transition vertically from
    ! clear to cloudy, cloudy to clear, clear to clear or cloudy to
    ! cloudy
    call random_number_generator%uniform_distribution_CPU(rand_cloud, is_any_cloud)

    ! Random numbers to generate sub-grid cloud structure
    call random_number_generator%uniform_distribution_CPU(rand_inhom)
    call random_number_generator%uniform_distribution_CPU(rand_inhom2, is_any_cloud)
...
!NEC$ shortloop
    ...
    ! optical depth scalings, calling the faster masked version and
    ! assuming values outside the range ibegin:iend are already zero
    call pdf_sampler%masked_block_sample_CPU(iend-ibegin+1, ng, &
         &  fractional_std(ibegin:iend), &
         &  rand_inhom(:,ibegin:iend), od_scaling(:,ibegin:iend), &
         &  is_any_cloud)
