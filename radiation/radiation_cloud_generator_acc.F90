  ...
  subroutine cloud_generator_acc_GPU(ng, nlev, &
    ...
    if (total_cloud_cover >= frac_threshold) then
      ...
      do jg = 1,ng
        ...
        rand_top = uniform_distribution_acc_GPU(istate)
        ...
        do jlev = itrigger+1,iend+1
          ...
          if (jlev <= iend) then
            rand_cloud = uniform_distribution_acc_GPU(istate)
          ...
          if (do_fill_od_scaling) then
            ...
            rand_inhom1 = uniform_distribution_acc_GPU(istate)
            ...
            do jcloud = jlev-n_layers_to_scale,jlev-1
              ...
              rand_inhom2 = uniform_distribution_acc_GPU(istate)
  ...
  subroutine cloud_generator_acc_CPU(ng, nlev, &
    ...
    if (total_cloud_cover >= frac_threshold) then
      ...
      do jg = 1,ng
        ...
        istate = mod(IMinstdA * istate, IMinstdM)
        ! end manuel inline of istate = initialize_acc(iseed, jg)
        rand_top = uniform_distribution_acc_CPU(istate)
        ...
        do jlev = itrigger+1,iend+1
          do_fill_od_scaling = .false.
          if (jlev <= iend) then
            rand_cloud = uniform_distribution_acc_CPU(istate)
            if (n_layers_to_scale > 0) then
              ! There is a cloud above, in which case the probability
              ! of cloud in the layer below is as follows
              if (rand_cloud*frac(jlev-1) &
          ...
          if (do_fill_od_scaling) then
            ...
            rand_inhom1 = uniform_distribution_acc_CPU(istate)
            ! Loop through the sequence of cloudy layers
            
            
            do jcloud = jlev-n_layers_to_scale,jlev-1

              rand_inhom2 = uniform_distribution_acc_CPU(istate)
