...
#include "radiation_delta_eddington.h"
  ...
  subroutine setup_cloud_optics_GPU(config, lacc)
    ...
    call config%cloud_optics%setup_GPU(trim(config%liq_optics_file_name), &
         &     trim(config%ice_optics_file_name), iverbose=config%iverbosesetup, lacc=lacc)
          ...
          if (cloud%fraction(jcol,jlev) > 0.0_jprb) then
            ...
            if (lwp_in_cloud > 0.0_jprb) then
              if (config%i_liq_model == ILiquidModelSOCRATES) then
                ...
                call calc_liq_optics_socrates_GPU(config%n_bands_lw, &
                    &  config%cloud_optics%liq_coeff_lw, &
                    &  lwp_in_cloud, cloud%re_liq(jcol,jlev), &
                    &  od_lw_liq, scat_od_lw_liq, g_lw_liq)
                ! Compute shortwave properties
                call calc_liq_optics_socrates_GPU(config%n_bands_sw, &
                    &  config%cloud_optics%liq_coeff_sw, &
                    &  lwp_in_cloud, cloud%re_liq(jcol,jlev), &
                    &  od_sw_liq, scat_od_sw_liq, g_sw_liq)
              else if (config%i_liq_model == ILiquidModelSlingo) then
                ! Compute longwave properties
                call calc_liq_optics_lindner_li_GPU(config%n_bands_lw, &
                    &  config%cloud_optics%liq_coeff_lw, &
                    &  lwp_in_cloud, cloud%re_liq(jcol,jlev), &
                    &  od_lw_liq, scat_od_lw_liq, g_lw_liq)
                ! Compute shortwave properties
                call calc_liq_optics_slingo_GPU(config%n_bands_sw, &
                    &  config%cloud_optics%liq_coeff_sw, &
                    &  lwp_in_cloud, cloud%re_liq(jcol,jlev), &
                    &  od_sw_liq, scat_od_sw_liq, g_sw_liq)
              end if
            ...
            if (iwp_in_cloud > 0.0_jprb) then
                ! Compute longwave properties
                call calc_ice_optics_fu_lw_GPU(config%n_bands_lw, &
                ...
                ! Compute shortwave properties
                call calc_ice_optics_fu_sw_GPU(config%n_bands_sw, &
  ...
  subroutine setup_cloud_optics_CPU(config)
    ...
    call config%cloud_optics%setup_CPU(trim(config%liq_optics_file_name), &
        ...
        do jcol = istartcol,iendcol
          ...
          if (cloud%fraction(jcol,jlev) > 0.0_jprb) then
            ...
            if (lwp_in_cloud > 0.0_jprb) then
              if (config%i_liq_model == ILiquidModelSOCRATES) then
                ! Compute longwave properties
                call calc_liq_optics_socrates_CPU(config%n_bands_lw, &
                    &  config%cloud_optics%liq_coeff_lw, &
                    &  lwp_in_cloud, cloud%re_liq(jcol,jlev), &
                    &  od_lw_liq, scat_od_lw_liq, g_lw_liq)
                ! Compute shortwave properties
                call calc_liq_optics_socrates_CPU(config%n_bands_sw, &
                    &  config%cloud_optics%liq_coeff_sw, &
                    &  lwp_in_cloud, cloud%re_liq(jcol,jlev), &
                    &  od_sw_liq, scat_od_sw_liq, g_sw_liq)
              else if (config%i_liq_model == ILiquidModelSlingo) then
                ! Compute longwave properties
                call calc_liq_optics_lindner_li_CPU(config%n_bands_lw, &
                    &  config%cloud_optics%liq_coeff_lw, &
                    &  lwp_in_cloud, cloud%re_liq(jcol,jlev), &
                    &  od_lw_liq, scat_od_lw_liq, g_lw_liq)
                ! Compute shortwave properties
                call calc_liq_optics_slingo_CPU(config%n_bands_sw, &
                    &  config%cloud_optics%liq_coeff_sw, &
                    &  lwp_in_cloud, cloud%re_liq(jcol,jlev), &
                    &  od_sw_liq, scat_od_sw_liq, g_sw_liq)
              else
            ...
            if (iwp_in_cloud > 0.0_jprb) then
              if (config%i_ice_model == IIceModelBaran) then
                ! Compute longwave properties
                call calc_ice_optics_baran_CPU(config%n_bands_lw, &
                    &  config%cloud_optics%ice_coeff_lw, &
                    &  iwp_in_cloud, cloud%q_ice(jcol,jlev), &
                    &  od_lw_ice, scat_od_lw_ice, g_lw_ice)
                ! Compute shortwave properties
                call calc_ice_optics_baran_CPU(config%n_bands_sw, &
                    &  config%cloud_optics%ice_coeff_sw, &
                    &  iwp_in_cloud, cloud%q_ice(jcol,jlev), &
                    &  od_sw_ice, scat_od_sw_ice, g_sw_ice)
                ...
                ! Compute longwave properties
                call calc_ice_optics_baran2016_CPU(config%n_bands_lw, &
                    &  config%cloud_optics%ice_coeff_lw, &
                    &  iwp_in_cloud, cloud%q_ice(jcol,jlev), &
                    &  temperature, &
                    &  od_lw_ice, scat_od_lw_ice, g_lw_ice)
                ! Compute shortwave properties
                call calc_ice_optics_baran2016_CPU(config%n_bands_sw, &
                    &  config%cloud_optics%ice_coeff_sw, &
                    &  iwp_in_cloud, cloud%q_ice(jcol,jlev), &
                    &  temperature, &
                    &  od_sw_ice, scat_od_sw_ice, g_sw_ice)
                ...
                temperature = 0.5_jprb * (thermodynamics%temperature_hl(jcol,jlev) &
                    &                   +thermodynamics%temperature_hl(jcol,jlev+1))
                ! Compute longwave properties
                call calc_ice_optics_baran2017_CPU(config%n_bands_lw, &
                    ...
                    &  od_lw_ice, scat_od_lw_ice, g_lw_ice)
                ! Compute shortwave properties
                call calc_ice_optics_baran2017_CPU(config%n_bands_sw, &
                    &  config%cloud_optics%ice_coeff_gen, &
                    &  config%cloud_optics%ice_coeff_sw, &
                    &  iwp_in_cloud, cloud%q_ice(jcol,jlev), &
                    &  temperature, &
                ...
                ! Compute longwave properties
                call calc_ice_optics_fu_lw_CPU(config%n_bands_lw, &
                    &  config%cloud_optics%ice_coeff_lw, &
                    &  iwp_in_cloud, cloud%re_ice(jcol,jlev), &
                    &  od_lw_ice, scat_od_lw_ice, g_lw_ice)
                if (config%do_fu_lw_ice_optics_bug) then
                ...
                ! Compute shortwave properties
                call calc_ice_optics_fu_sw_CPU(config%n_bands_sw, &
                    &  config%cloud_optics%ice_coeff_sw, &
                    &  iwp_in_cloud, cloud%re_ice(jcol,jlev), &
                    &  od_sw_ice, scat_od_sw_ice, g_sw_ice)
              else if (config%i_ice_model == IIceModelYi) then
                ! Compute longwave properties
                call calc_ice_optics_yi_lw_CPU(config%n_bands_lw, &
                    &  config%cloud_optics%ice_coeff_lw, &
                    &  iwp_in_cloud, cloud%re_ice(jcol,jlev), &
                    &  od_lw_ice, scat_od_lw_ice, g_lw_ice)
                ! Compute shortwave properties
                call calc_ice_optics_yi_sw_CPU(config%n_bands_sw, &
                    &  config%cloud_optics%ice_coeff_sw, &
                    &  iwp_in_cloud, cloud%re_ice(jcol,jlev), &
                    &  od_sw_ice, scat_od_sw_ice, g_sw_ice)
              else
