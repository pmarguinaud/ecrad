...
contains
  ...
  subroutine read_config_from_namelist_GPU(this, file_name, unit, is_success, lacc)
    ...
    call get_enum_code_GPU(liquid_model_name, LiquidModelName, &
         &            'liquid_model_name', this%i_liq_model, lacc=lacc)

    ! Determine ice optics model
    call get_enum_code_GPU(ice_model_name, IceModelName, &
         &            'ice_model_name', this%i_ice_model, lacc=lacc)

    ! Determine gas optics model(s) - firstly try the generic gas_model_name
    i_gas_model = -1
    call get_enum_code_GPU(gas_model_name, GasModelName, &
         &            'gas_model_name', i_gas_model, lacc=lacc)
    if (i_gas_model > -1) then
      this%i_gas_model_sw = i_gas_model
      this%i_gas_model_lw = i_gas_model
    end if
    ! ...then the band-specific values
    call get_enum_code_GPU(sw_gas_model_name, GasModelName, &
         &            'sw_gas_model_name', this%i_gas_model_sw, lacc=lacc)
    call get_enum_code_GPU(lw_gas_model_name, GasModelName, &
         &            'lw_gas_model_name', this%i_gas_model_lw, lacc=lacc)

    ! Determine solvers
    call get_enum_code_GPU(sw_solver_name, SolverName, &
         &            'sw_solver_name', this%i_solver_sw, lacc=lacc)
    call get_enum_code_GPU(lw_solver_name, SolverName, &
         &            'lw_solver_name', this%i_solver_lw, lacc=lacc)

    if (len_trim(sw_encroachment_name) > 1) then
      call get_enum_code_GPU(sw_encroachment_name, EncroachmentName, &
           &             'sw_encroachment_name', this%i_3d_sw_entrapment, lacc=lacc)
      write(nulout, '(a)') 'Warning: radiation namelist string "sw_encroachment_name" is deprecated: use "sw_entrapment_name"'
    else
      call get_enum_code_GPU(sw_entrapment_name, EntrapmentName, &
           &             'sw_entrapment_name', this%i_3d_sw_entrapment, lacc=lacc)
    end if

    ! Determine overlap scheme
    call get_enum_code_GPU(overlap_scheme_name, OverlapName, &
         &             'overlap_scheme_name', this%i_overlap_scheme, lacc=lacc)

    ! Determine cloud PDF shape
    call get_enum_code_GPU(cloud_pdf_shape_name, PdfShapeName, &
         &             'cloud_pdf_shape_name', this%i_cloud_pdf_shape, lacc=lacc)
  ...
  subroutine print_config_GPU(this, iverbose, lacc)
    ...
    if (i_local_verbose >= 2) then
      ...
      call print_logical_GPU('  Clear-sky calculations are', 'do_clear', this%do_clear, lacc=lacc)
      call print_logical_GPU('  Saving intermediate radiative properties', &
           &   'do_save_radiative_properties', this%do_save_radiative_properties, lacc=lacc)
      call print_logical_GPU('  Saving spectral flux profiles', &
           &   'do_save_spectral_flux', this%do_save_spectral_flux, lacc=lacc)
      call print_enum_GPU('  Shortwave gas model is', GasModelName, 'i_gas_model_sw', &
           &          this%i_gas_model_sw, lacc=lacc)
      call print_enum_GPU('  Longwave gas model is', GasModelName, 'i_gas_model_lw', &
           &          this%i_gas_model_lw, lacc=lacc)
      call print_logical_GPU('  Aerosols are', 'use_aerosols', this%use_aerosols, lacc=lacc)
      if (this%use_aerosols) then
        call print_logical_GPU('  General aerosol optics', &
             &             'use_general_aerosol_optics', this%use_general_aerosol_optics, lacc=lacc)
      end if
      ...
      if (this%do_sw) then
        call print_logical_GPU('  Do cloud/aerosol/surface SW properties per g-point', &
             &  'do_cloud_aerosol_per_sw_g_point', this%do_cloud_aerosol_per_sw_g_point, lacc=lacc)
      end if
      if (this%do_lw) then
        call print_logical_GPU('  Do cloud/aerosol/surface LW properties per g-point', &
             &  'do_cloud_aerosol_per_lw_g_point', this%do_cloud_aerosol_per_lw_g_point, lacc=lacc)
      end if
      if (this%do_sw) then
        call print_logical_GPU('  Represent solar cycle in spectral irradiance', &
             &  'use_spectral_solar_cycle', this%use_spectral_solar_cycle, lacc=lacc)
        call print_logical_GPU('  Scale spectral solar irradiance', &
             &  'use_spectral_solar_scaling', this%use_spectral_solar_scaling, lacc=lacc)
      end if

      !---------------------------------------------------------------------
      write(nulout, '(a)') 'Surface and top-of-atmosphere settings:'
      call print_logical_GPU('  Saving top-of-atmosphere spectral fluxes', &
           &   'do_toa_spectral_flux', this%do_toa_spectral_flux, lacc=lacc)
      if (this%do_sw) then
        call print_logical_GPU('  Saving surface shortwave spectral fluxes', &
             &   'do_surface_sw_spectral_flux', this%do_surface_sw_spectral_flux, lacc=lacc)
        call print_logical_GPU('  Saving surface shortwave fluxes in abledo bands', &
             &   'do_canopy_fluxes_sw', this%do_canopy_fluxes_sw, lacc=lacc)
      end if
      if (this%do_lw) then
        call print_logical_GPU('  Saving surface longwave fluxes in emissivity bands', &
             &   'do_canopy_fluxes_lw', this%do_canopy_fluxes_lw, lacc=lacc)
        call print_logical_GPU('  Longwave derivative calculation is', &
             &   'do_lw_derivatives',this%do_lw_derivatives, lacc=lacc)
      end if
      if (this%do_sw) then
        call print_logical_GPU('  Nearest-neighbour spectral albedo mapping', &
             &   'do_nearest_spectral_sw_albedo', this%do_nearest_spectral_sw_albedo, lacc=lacc)
      end if
      if (this%do_lw) then
        call print_logical_GPU('  Nearest-neighbour spectral emissivity mapping', &
             &   'do_nearest_spectral_lw_emiss', this%do_nearest_spectral_lw_emiss, lacc=lacc)
      end if
      call print_logical_GPU('  Planck-weighted surface albedo/emiss mapping', &
           &   'do_weighted_surface_mapping', this%do_weighted_surface_mapping, lacc=lacc)

      !---------------------------------------------------------------------
      if (this%do_clouds) then
        write(nulout, '(a)') 'Cloud settings:'
        call print_real_GPU('  Cloud fraction threshold', &
             &   'cloud_fraction_threshold', this%cloud_fraction_threshold, lacc=lacc)
        call print_real_GPU('  Cloud mixing-ratio threshold', &
             &   'cloud_mixing_ratio_threshold', this%cloud_mixing_ratio_threshold, lacc=lacc)
        call print_logical_GPU('  General cloud optics', &
             &             'use_general_cloud_optics', this%use_general_cloud_optics, lacc=lacc)
        if (.not. this%use_general_cloud_optics) then
          call print_enum_GPU('  Liquid optics scheme is', LiquidModelName, &
               &          'i_liq_model',this%i_liq_model, lacc=lacc)
          call print_enum_GPU('  Ice optics scheme is', IceModelName, &
               &          'i_ice_model',this%i_ice_model, lacc=lacc)
          if (this%i_ice_model == IIceModelFu) then
            call print_logical_GPU('  Longwave ice optics bug in Fu scheme is', &
                 &   'do_fu_lw_ice_optics_bug',this%do_fu_lw_ice_optics_bug, lacc=lacc)
          end if
        end if
        call print_enum_GPU('  Cloud overlap scheme is', OverlapName, &
             &          'i_overlap_scheme',this%i_overlap_scheme, lacc=lacc)
        call print_logical_GPU('  Use "beta" overlap parameter is', &
             &   'use_beta_overlap', this%use_beta_overlap, lacc=lacc)
        call print_enum_GPU('  Cloud PDF shape is', PdfShapeName, &
             &          'i_cloud_pdf_shape',this%i_cloud_pdf_shape, lacc=lacc)
        call print_real_GPU('  Cloud inhom decorrelation scaling', &
             &   'cloud_inhom_decorr_scaling', this%cloud_inhom_decorr_scaling, lacc=lacc)
      end if

      !---------------------------------------------------------------------
      write(nulout, '(a)') 'Solver settings:'
      if (this%do_sw) then
        call print_enum_GPU('  Shortwave solver is', SolverName, &
             &          'i_solver_sw', this%i_solver_sw, lacc=lacc)

        if (this%i_gas_model_sw == IGasModelMonochromatic) then
          call print_real_GPU('  Shortwave atmospheric optical depth', &
               &   'mono_sw_total_od', this%mono_sw_total_od, lacc=lacc)
          call print_real_GPU('  Shortwave particulate single-scattering albedo', &
               &   'mono_sw_single_scattering_albedo', &
               &   this%mono_sw_single_scattering_albedo, lacc=lacc)
          call print_real_GPU('  Shortwave particulate asymmetry factor', &
               &   'mono_sw_asymmetry_factor', &
               &   this%mono_sw_asymmetry_factor, lacc=lacc)
        end if
        call print_logical_GPU('  Shortwave delta scaling after merge with gases', &
             &   'do_sw_delta_scaling_with_gases', &
             &   this%do_sw_delta_scaling_with_gases, lacc=lacc)
      else
        call print_logical_GPU('  Shortwave calculations are','do_sw',this%do_sw, lacc=lacc)
      end if

      if (this%do_lw) then
        call print_enum_GPU('  Longwave solver is', SolverName, 'i_solver_lw', &
             &          this%i_solver_lw, lacc=lacc)

        if (this%i_gas_model_lw == IGasModelMonochromatic) then
          if (this%mono_lw_wavelength > 0.0_jprb) then
            call print_real_GPU('  Longwave effective wavelength (m)', &
                 &   'mono_lw_wavelength', this%mono_lw_wavelength, lacc=lacc)
          else
            write(nulout,'(a)') '  Longwave fluxes are broadband                              (mono_lw_wavelength<=0)'
          end if
          call print_real_GPU('  Longwave atmospheric optical depth', &
               &   'mono_lw_total_od', this%mono_lw_total_od, lacc=lacc)
          call print_real_GPU('  Longwave particulate single-scattering albedo', &
               &   'mono_lw_single_scattering_albedo', &
               &   this%mono_lw_single_scattering_albedo, lacc=lacc)
          call print_real_GPU('  Longwave particulate asymmetry factor', &
               &   'mono_lw_asymmetry_factor', &
               &   this%mono_lw_asymmetry_factor, lacc=lacc)
        end if
        call print_logical_GPU('  Longwave cloud scattering is', &
             &   'do_lw_cloud_scattering',this%do_lw_cloud_scattering, lacc=lacc)
        call print_logical_GPU('  Longwave aerosol scattering is', &
             &   'do_lw_aerosol_scattering',this%do_lw_aerosol_scattering, lacc=lacc)
      else
        call print_logical_GPU('  Longwave calculations are','do_lw', this%do_lw, lacc=lacc)
      end if

      if (this%i_solver_sw == ISolverSpartacus &
           &  .or. this%i_solver_lw == ISolverSpartacus) then
        write(nulout, '(a)') '  SPARTACUS options:'
        call print_integer_GPU('    Number of regions', 'n_regions', this%nregions, lacc=lacc)
        call print_real_GPU('    Max cloud optical depth per layer (LW)', &
             &   'max_cloud_od_lw', this%max_cloud_od_lw, lacc=lacc)
        call print_real_GPU('    Max cloud optical depth per layer (SW)', &
             &   'max_cloud_od_sw', this%max_cloud_od_sw, lacc=lacc)
        call print_enum_GPU('    Shortwave entrapment is', EntrapmentName, &
             &          'i_3d_sw_entrapment', this%i_3d_sw_entrapment, lacc=lacc)
        call print_logical_GPU('    Multilayer longwave horizontal transport is', &
             'do_3d_lw_multilayer_effects', this%do_3d_lw_multilayer_effects, lacc=lacc)
        call print_logical_GPU('    Use matrix exponential everywhere is', &
             &   'use_expm_everywhere', this%use_expm_everywhere, lacc=lacc)
        call print_logical_GPU('    3D effects are', 'do_3d_effects', &
             &             this%do_3d_effects, lacc=lacc)

        if (this%do_3d_effects) then
          call print_logical_GPU('    Longwave side emissivity parameterization is', &
               &  'do_lw_side_emissivity', this%do_lw_side_emissivity, lacc=lacc)
          call print_real_GPU('    Clear-to-thick edge fraction is', &
               &  'clear_to_thick_fraction', this%clear_to_thick_fraction, lacc=lacc)
          call print_real_GPU('    Overhead sun factor is', &
               &  'overhead_sun_factor', this%overhead_sun_factor, lacc=lacc)
          call print_real_GPU('    Max gas optical depth for 3D effects', &
               &   'max_gas_od_3d', this%max_gas_od_3d, lacc=lacc)
          call print_real_GPU('    Max 3D transfer rate', &
               &   'max_3d_transfer_rate', this%max_3d_transfer_rate, lacc=lacc)
          call print_real_GPU('    Min cloud effective size (m)', &
               &   'min_cloud_effective_size', this%min_cloud_effective_size, lacc=lacc)
          call print_real_GPU('    Overhang factor', &
               &   'overhang_factor', this%overhang_factor, lacc=lacc)
        end if
        ...
        call print_logical_GPU('  Use vectorizable McICA cloud generator', &
             &   'use_vectorizable_generator', this%use_vectorizable_generator, lacc=lacc)
      end if
  ...
  subroutine get_sw_weights_GPU(this, wavelength1, wavelength2, &
    ...
    integer :: jband
    logical, intent (in) :: lacc ! Loop index for spectral band
    ...
    call this%gas_optics_sw%spectral_def%calc_mapping_from_bands_GPU( &
         &  [wavelength1, wavelength2], [1, 2, 3], mapping, &
         &  use_bands=(.not. this%do_cloud_aerosol_per_sw_g_point), use_fluxes=.true., lacc=lacc)
  ...
  subroutine get_sw_mapping_GPU(this, wavelength_bound, mapping, weighting_name, lacc)
    ...
    integer :: jint
    logical, intent (in) :: lacc  ! Loop for interval
    ...
    call this%gas_optics_sw%spectral_def%calc_mapping_from_bands_GPU( &
         ...
         &  use_bands=(.not. this%do_cloud_aerosol_per_sw_g_point), use_fluxes=.false., lacc=lacc)
  ...
  subroutine define_sw_albedo_intervals_GPU(this, ninterval, wavelength_bound, &
    ...
    if (this%is_consolidated) then
      call this%consolidate_sw_albedo_intervals_GPU(lacc=lacc)
    end if
  ...
  subroutine define_lw_emiss_intervals_GPU(this, ninterval, wavelength_bound, &
    ...
    if (this%is_consolidated) then
      call this%consolidate_lw_emiss_intervals_GPU(lacc=lacc)
    end if
  ...
  subroutine consolidate_sw_albedo_intervals_GPU(this, lacc)
    ...
    if (this%do_weighted_surface_mapping) then
      call this%gas_optics_sw%spectral_def%calc_mapping_from_bands_GPU( &
           ...
           &  this%sw_albedo_weights, use_bands=(.not. this%do_cloud_aerosol_per_sw_g_point), lacc=lacc)
    else
    ...
    if (this%iverbosesetup >= 2) then
      ...
      if (.not. this%do_nearest_spectral_sw_albedo) then
        call this%gas_optics_sw%spectral_def%print_mapping_from_bands_GPU(this%sw_albedo_weights, &
             &       use_bands=(.not. this%do_cloud_aerosol_per_sw_g_point), lacc=lacc)
  ...
  subroutine consolidate_lw_emiss_intervals_GPU(this, lacc)
    ...
    if (this%do_weighted_surface_mapping) then
      call this%gas_optics_lw%spectral_def%calc_mapping_from_bands_GPU( &
           &  this%lw_emiss_wavelength_bound(1:ninterval-1), this%i_lw_emiss_index(1:ninterval), &
           &  this%lw_emiss_weights, use_bands=(.not. this%do_cloud_aerosol_per_lw_g_point), lacc=lacc)
    else
      ! Weight each wavenumber equally as in IFS Cycles 48 and earlier
      call this%gas_optics_lw%spectral_def%calc_mapping_from_bands_GPU( &
           &  this%lw_emiss_wavelength_bound(1:ninterval-1), this%i_lw_emiss_index(1:ninterval), &
    ...
    if (this%iverbosesetup >= 2) then
      ...
      if (.not. this%do_nearest_spectral_lw_emiss) then
        call this%gas_optics_lw%spectral_def%print_mapping_from_bands_GPU(this%lw_emiss_weights, &
             &                          use_bands=(.not. this%do_cloud_aerosol_per_lw_g_point), lacc=lacc)
      else if (ninterval <= 1) then
  ...
  subroutine create_device_GPU(this, lacc)
    ...
    !$ACC ENTER DATA COPYIN(this%g_frac_sw) IF((allocated(this%g_frac_sw)) .AND. lacc) ASYNC(1)
    !$ACC ENTER DATA COPYIN(this%g_frac_lw) IF((allocated(this%g_frac_lw)) .AND. lacc) ASYNC(1)
    !$ACC ENTER DATA COPYIN(this%i_albedo_from_band_sw) IF((allocated(this%i_albedo_from_band_sw)) .AND. lacc) ASYNC(1)
    !$ACC ENTER DATA COPYIN(this%i_emiss_from_band_lw) IF((allocated(this%i_emiss_from_band_lw)) .AND. lacc) ASYNC(1)
    !$ACC ENTER DATA COPYIN(this%sw_albedo_weights) IF((allocated(this%sw_albedo_weights)) .AND. lacc) ASYNC(1)
    !$ACC ENTER DATA COPYIN(this%lw_emiss_weights) IF((allocated(this%lw_emiss_weights)) .AND. lacc) ASYNC(1)
    !$ACC ENTER DATA COPYIN(this%i_band_from_g_lw) IF((allocated(this%i_band_from_g_lw)) .AND. lacc) ASYNC(1)
    !$ACC ENTER DATA COPYIN(this%i_band_from_g_sw) IF((allocated(this%i_band_from_g_sw)) .AND. lacc) ASYNC(1)
    !$ACC ENTER DATA COPYIN(this%i_band_from_reordered_g_lw) IF((allocated(this%i_band_from_reordered_g_lw)) .AND. lacc) ASYNC(1)
    !$ACC ENTER DATA COPYIN(this%i_band_from_reordered_g_sw) IF((allocated(this%i_band_from_reordered_g_sw)) .AND. lacc) ASYNC(1)
    !$ACC ENTER DATA COPYIN(this%i_spec_from_reordered_g_lw) IF((allocated(this%i_spec_from_reordered_g_lw)) .AND. lacc) ASYNC(1)
    !$ACC ENTER DATA COPYIN(this%i_spec_from_reordered_g_sw) IF((allocated(this%i_spec_from_reordered_g_sw)) .AND. lacc) ASYNC(1)

    ! NB: ckd_model_type not yet implemented

    !$ACC ENTER DATA COPYIN(this%cloud_optics) ASYNC(1) IF(lacc)
    call this%cloud_optics%create_device_GPU(lacc=lacc)

    ! NB: general_cloud_optics_type not yet implemented

    !$ACC ENTER DATA COPYIN(this%aerosol_optics) ASYNC(1) IF(lacc)
    call this%aerosol_optics%create_device_GPU(lacc=lacc)

    !$ACC ENTER DATA COPYIN(this%pdf_sampler) ASYNC(1) IF(lacc)
    call this%pdf_sampler%create_device_GPU(lacc=lacc)
  ...
  subroutine update_host_GPU(this, lacc)
    ...
    !$ACC UPDATE HOST(this%g_frac_sw) IF((allocated(this%g_frac_sw)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE HOST(this%g_frac_lw) IF((allocated(this%g_frac_lw)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE HOST(this%i_albedo_from_band_sw) IF((allocated(this%i_albedo_from_band_sw)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE HOST(this%i_emiss_from_band_lw) IF((allocated(this%i_emiss_from_band_lw)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE HOST(this%sw_albedo_weights) IF((allocated(this%sw_albedo_weights)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE HOST(this%lw_emiss_weights) IF((allocated(this%lw_emiss_weights)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE HOST(this%i_band_from_g_lw) IF((allocated(this%i_band_from_g_lw)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE HOST(this%i_band_from_g_sw) IF((allocated(this%i_band_from_g_sw)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE HOST(this%i_band_from_reordered_g_lw) IF((allocated(this%i_band_from_reordered_g_lw)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE HOST(this%i_band_from_reordered_g_sw) IF((allocated(this%i_band_from_reordered_g_sw)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE HOST(this%i_spec_from_reordered_g_lw) IF((allocated(this%i_spec_from_reordered_g_lw)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE HOST(this%i_spec_from_reordered_g_sw) IF((allocated(this%i_spec_from_reordered_g_sw)) .AND. lacc) ASYNC(1)

    ! NB: ckd_model_type not yet implemented

    !$ACC UPDATE HOST(this%cloud_optics) ASYNC(1) IF(lacc)
    call this%cloud_optics%update_host_GPU(lacc=lacc)

    ! NB: general_cloud_optics_type not yet implemented

    !$ACC UPDATE HOST(this%aerosol_optics) ASYNC(1) IF(lacc)
    call this%aerosol_optics%update_host_GPU(lacc=lacc)

    !$ACC UPDATE HOST(this%pdf_sampler) ASYNC(1) IF(lacc)
    call this%pdf_sampler%update_host_GPU(lacc=lacc)
  ...
  subroutine update_device_GPU(this, lacc)
    ...
    !$ACC UPDATE DEVICE(this%g_frac_sw) IF((allocated(this%g_frac_sw)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE DEVICE(this%g_frac_lw) IF((allocated(this%g_frac_lw)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE DEVICE(this%i_albedo_from_band_sw) IF((allocated(this%i_albedo_from_band_sw)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE DEVICE(this%i_emiss_from_band_lw) IF((allocated(this%i_emiss_from_band_lw)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE DEVICE(this%sw_albedo_weights) IF((allocated(this%sw_albedo_weights)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE DEVICE(this%lw_emiss_weights) IF((allocated(this%lw_emiss_weights)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE DEVICE(this%i_band_from_g_lw) IF((allocated(this%i_band_from_g_lw)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE DEVICE(this%i_band_from_g_sw) IF((allocated(this%i_band_from_g_sw)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE DEVICE(this%i_band_from_reordered_g_lw) IF((allocated(this%i_band_from_reordered_g_lw)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE DEVICE(this%i_band_from_reordered_g_sw) IF((allocated(this%i_band_from_reordered_g_sw)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE DEVICE(this%i_spec_from_reordered_g_lw) IF((allocated(this%i_spec_from_reordered_g_lw)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE DEVICE(this%i_spec_from_reordered_g_sw) IF((allocated(this%i_spec_from_reordered_g_sw)) .AND. lacc) ASYNC(1)

    ! NB: ckd_model_type not yet implemented

    !$ACC UPDATE DEVICE(this%cloud_optics) IF(lacc)
    call this%cloud_optics%update_device_GPU(lacc=lacc)

    ! NB: general_cloud_optics_type not yet implemented

    !$ACC UPDATE DEVICE(this%aerosol_optics) IF(lacc)
    call this%aerosol_optics%update_device_GPU(lacc=lacc)

    !$ACC UPDATE DEVICE(this%pdf_sampler) IF(lacc)
    call this%pdf_sampler%update_device_GPU(lacc=lacc)
  ...
  subroutine delete_device_GPU(this, lacc)
    ...
    !$ACC EXIT DATA DELETE(this%g_frac_sw) IF((allocated(this%g_frac_sw)) .AND. lacc) ASYNC(1)
    !$ACC EXIT DATA DELETE(this%g_frac_lw) IF((allocated(this%g_frac_lw)) .AND. lacc) ASYNC(1)
    !$ACC EXIT DATA DELETE(this%i_albedo_from_band_sw) IF((allocated(this%i_albedo_from_band_sw)) .AND. lacc) ASYNC(1)
    !$ACC EXIT DATA DELETE(this%i_emiss_from_band_lw) IF((allocated(this%i_emiss_from_band_lw)) .AND. lacc) ASYNC(1)
    !$ACC EXIT DATA DELETE(this%sw_albedo_weights) IF((allocated(this%sw_albedo_weights)) .AND. lacc) ASYNC(1)
    !$ACC EXIT DATA DELETE(this%lw_emiss_weights) IF((allocated(this%lw_emiss_weights)) .AND. lacc) ASYNC(1)
    !$ACC EXIT DATA DELETE(this%i_band_from_g_lw) IF((allocated(this%i_band_from_g_lw)) .AND. lacc) ASYNC(1)
    !$ACC EXIT DATA DELETE(this%i_band_from_g_sw) IF((allocated(this%i_band_from_g_sw)) .AND. lacc) ASYNC(1)
    !$ACC EXIT DATA DELETE(this%i_band_from_reordered_g_lw) IF((allocated(this%i_band_from_reordered_g_lw)) .AND. lacc) ASYNC(1)
    !$ACC EXIT DATA DELETE(this%i_band_from_reordered_g_sw) IF((allocated(this%i_band_from_reordered_g_sw)) .AND. lacc) ASYNC(1)
    !$ACC EXIT DATA DELETE(this%i_spec_from_reordered_g_lw) IF((allocated(this%i_spec_from_reordered_g_lw)) .AND. lacc) ASYNC(1)
    !$ACC EXIT DATA DELETE(this%i_spec_from_reordered_g_sw) IF((allocated(this%i_spec_from_reordered_g_sw)) .AND. lacc) ASYNC(1)

    ! NB: ckd_model_type not yet implemented

    !$ACC EXIT DATA DELETE(this%cloud_optics) ASYNC(1) IF(lacc)
    call this%cloud_optics%delete_device_GPU(lacc=lacc)

    ! NB: general_cloud_optics_type not yet implemented

    !$ACC EXIT DATA DELETE(this%aerosol_optics) ASYNC(1) IF(lacc)
    call this%aerosol_optics%delete_device_GPU(lacc=lacc)

    !$ACC EXIT DATA DELETE(this%pdf_sampler) ASYNC(1) IF(lacc)
    call this%pdf_sampler%delete_device_GPU(lacc=lacc)
  ...
  subroutine read_config_from_namelist_CPU(this, file_name, unit, is_success)
    ...
    ! Determine liquid optics model
    call get_enum_code_CPU(liquid_model_name, LiquidModelName, &
         &            'liquid_model_name', this%i_liq_model)

    ! Determine ice optics model
    call get_enum_code_CPU(ice_model_name, IceModelName, &
         &            'ice_model_name', this%i_ice_model)

    ! Determine gas optics model(s) - firstly try the generic gas_model_name
    i_gas_model = -1
    call get_enum_code_CPU(gas_model_name, GasModelName, &
    ...
    ! ...then the band-specific values
    call get_enum_code_CPU(sw_gas_model_name, GasModelName, &
         &            'sw_gas_model_name', this%i_gas_model_sw)
    call get_enum_code_CPU(lw_gas_model_name, GasModelName, &
         &            'lw_gas_model_name', this%i_gas_model_lw)

    ! Determine solvers
    call get_enum_code_CPU(sw_solver_name, SolverName, &
         &            'sw_solver_name', this%i_solver_sw)
    call get_enum_code_CPU(lw_solver_name, SolverName, &
         &            'lw_solver_name', this%i_solver_lw)

    if (len_trim(sw_encroachment_name) > 1) then
      call get_enum_code_CPU(sw_encroachment_name, EncroachmentName, &
           &             'sw_encroachment_name', this%i_3d_sw_entrapment)
      write(nulout, '(a)') 'Warning: radiation namelist string "sw_encroachment_name" is deprecated: use "sw_entrapment_name"'
    else
      call get_enum_code_CPU(sw_entrapment_name, EntrapmentName, &
           &             'sw_entrapment_name', this%i_3d_sw_entrapment)
    end if

    ! Determine overlap scheme
    call get_enum_code_CPU(overlap_scheme_name, OverlapName, &
         &             'overlap_scheme_name', this%i_overlap_scheme)

    ! Determine cloud PDF shape
    call get_enum_code_CPU(cloud_pdf_shape_name, PdfShapeName, &
  ...
  subroutine print_config_CPU(this, iverbose)
    ...
    if (i_local_verbose >= 2) then
      ...
      write(nulout, '(a,a,a)') '  Data files expected in "', &
           &                   trim(this%directory_name), '"'
      call print_logical_CPU('  Clear-sky calculations are', 'do_clear', this%do_clear)
      call print_logical_CPU('  Saving intermediate radiative properties', &
           &   'do_save_radiative_properties', this%do_save_radiative_properties)
      call print_logical_CPU('  Saving spectral flux profiles', &
           &   'do_save_spectral_flux', this%do_save_spectral_flux)
      call print_enum_CPU('  Shortwave gas model is', GasModelName, 'i_gas_model_sw', &
           &          this%i_gas_model_sw)
      call print_enum_CPU('  Longwave gas model is', GasModelName, 'i_gas_model_lw', &
           &          this%i_gas_model_lw)
      call print_logical_CPU('  Aerosols are', 'use_aerosols', this%use_aerosols)
      if (this%use_aerosols) then
        call print_logical_CPU('  General aerosol optics', &
             &             'use_general_aerosol_optics', this%use_general_aerosol_optics)
      end if
      if (this%do_clouds) then
        write(nulout,'(a)') '  Clouds are ON'
      ...
      if (this%do_sw) then
        call print_logical_CPU('  Do cloud/aerosol/surface SW properties per g-point', &
             &  'do_cloud_aerosol_per_sw_g_point', this%do_cloud_aerosol_per_sw_g_point)
      end if
      if (this%do_lw) then
        call print_logical_CPU('  Do cloud/aerosol/surface LW properties per g-point', &
             &  'do_cloud_aerosol_per_lw_g_point', this%do_cloud_aerosol_per_lw_g_point)
      end if
      if (this%do_sw) then
        call print_logical_CPU('  Represent solar cycle in spectral irradiance', &
             &  'use_spectral_solar_cycle', this%use_spectral_solar_cycle)
        call print_logical_CPU('  Scale spectral solar irradiance', &
             &  'use_spectral_solar_scaling', this%use_spectral_solar_scaling)
      end if

      !---------------------------------------------------------------------
      write(nulout, '(a)') 'Surface and top-of-atmosphere settings:'
      call print_logical_CPU('  Saving top-of-atmosphere spectral fluxes', &
           &   'do_toa_spectral_flux', this%do_toa_spectral_flux)
      if (this%do_sw) then
        call print_logical_CPU('  Saving surface shortwave spectral fluxes', &
             &   'do_surface_sw_spectral_flux', this%do_surface_sw_spectral_flux)
        call print_logical_CPU('  Saving surface shortwave fluxes in abledo bands', &
             &   'do_canopy_fluxes_sw', this%do_canopy_fluxes_sw)
      end if
      if (this%do_lw) then
        call print_logical_CPU('  Saving surface longwave fluxes in emissivity bands', &
             &   'do_canopy_fluxes_lw', this%do_canopy_fluxes_lw)
        call print_logical_CPU('  Longwave derivative calculation is', &
             &   'do_lw_derivatives',this%do_lw_derivatives)
      end if
      if (this%do_sw) then
        call print_logical_CPU('  Nearest-neighbour spectral albedo mapping', &
             &   'do_nearest_spectral_sw_albedo', this%do_nearest_spectral_sw_albedo)
      end if
      if (this%do_lw) then
        call print_logical_CPU('  Nearest-neighbour spectral emissivity mapping', &
             &   'do_nearest_spectral_lw_emiss', this%do_nearest_spectral_lw_emiss)
      end if
      call print_logical_CPU('  Planck-weighted surface albedo/emiss mapping', &
           &   'do_weighted_surface_mapping', this%do_weighted_surface_mapping)

      !---------------------------------------------------------------------
      if (this%do_clouds) then
        write(nulout, '(a)') 'Cloud settings:'
        call print_real_CPU('  Cloud fraction threshold', &
             &   'cloud_fraction_threshold', this%cloud_fraction_threshold)
        call print_real_CPU('  Cloud mixing-ratio threshold', &
             &   'cloud_mixing_ratio_threshold', this%cloud_mixing_ratio_threshold)
        call print_logical_CPU('  General cloud optics', &
             &             'use_general_cloud_optics', this%use_general_cloud_optics)
        if (.not. this%use_general_cloud_optics) then
          call print_enum_CPU('  Liquid optics scheme is', LiquidModelName, &
               &          'i_liq_model',this%i_liq_model)
          call print_enum_CPU('  Ice optics scheme is', IceModelName, &
               &          'i_ice_model',this%i_ice_model)
          if (this%i_ice_model == IIceModelFu) then
            call print_logical_CPU('  Longwave ice optics bug in Fu scheme is', &
                 &   'do_fu_lw_ice_optics_bug',this%do_fu_lw_ice_optics_bug)
          end if
        end if
        call print_enum_CPU('  Cloud overlap scheme is', OverlapName, &
             &          'i_overlap_scheme',this%i_overlap_scheme)
        call print_logical_CPU('  Use "beta" overlap parameter is', &
             &   'use_beta_overlap', this%use_beta_overlap)
        call print_enum_CPU('  Cloud PDF shape is', PdfShapeName, &
             &          'i_cloud_pdf_shape',this%i_cloud_pdf_shape)
        call print_real_CPU('  Cloud inhom decorrelation scaling', &
             &   'cloud_inhom_decorr_scaling', this%cloud_inhom_decorr_scaling)
      end if
      ...
      write(nulout, '(a)') 'Solver settings:'
      if (this%do_sw) then
        call print_enum_CPU('  Shortwave solver is', SolverName, &
             &          'i_solver_sw', this%i_solver_sw)

        if (this%i_gas_model_sw == IGasModelMonochromatic) then
          call print_real_CPU('  Shortwave atmospheric optical depth', &
               &   'mono_sw_total_od', this%mono_sw_total_od)
          call print_real_CPU('  Shortwave particulate single-scattering albedo', &
               &   'mono_sw_single_scattering_albedo', &
               &   this%mono_sw_single_scattering_albedo)
          call print_real_CPU('  Shortwave particulate asymmetry factor', &
               &   'mono_sw_asymmetry_factor', &
               &   this%mono_sw_asymmetry_factor)
        end if
        call print_logical_CPU('  Shortwave delta scaling after merge with gases', &
             &   'do_sw_delta_scaling_with_gases', &
             &   this%do_sw_delta_scaling_with_gases)
      else
        call print_logical_CPU('  Shortwave calculations are','do_sw',this%do_sw)
      end if

      if (this%do_lw) then
        call print_enum_CPU('  Longwave solver is', SolverName, 'i_solver_lw', &
             &          this%i_solver_lw)

        if (this%i_gas_model_lw == IGasModelMonochromatic) then
          if (this%mono_lw_wavelength > 0.0_jprb) then
            call print_real_CPU('  Longwave effective wavelength (m)', &
                 &   'mono_lw_wavelength', this%mono_lw_wavelength)
          else
            write(nulout,'(a)') '  Longwave fluxes are broadband                              (mono_lw_wavelength<=0)'
          end if
          call print_real_CPU('  Longwave atmospheric optical depth', &
               &   'mono_lw_total_od', this%mono_lw_total_od)
          call print_real_CPU('  Longwave particulate single-scattering albedo', &
               &   'mono_lw_single_scattering_albedo', &
               &   this%mono_lw_single_scattering_albedo)
          call print_real_CPU('  Longwave particulate asymmetry factor', &
               &   'mono_lw_asymmetry_factor', &
               &   this%mono_lw_asymmetry_factor)
        end if
        call print_logical_CPU('  Longwave cloud scattering is', &
             &   'do_lw_cloud_scattering',this%do_lw_cloud_scattering)
        call print_logical_CPU('  Longwave aerosol scattering is', &
             &   'do_lw_aerosol_scattering',this%do_lw_aerosol_scattering)
      else
        call print_logical_CPU('  Longwave calculations are','do_lw', this%do_lw)
      end if

      if (this%i_solver_sw == ISolverSpartacus &
           &  .or. this%i_solver_lw == ISolverSpartacus) then
        write(nulout, '(a)') '  SPARTACUS options:'
        call print_integer_CPU('    Number of regions', 'n_regions', this%nregions)
        call print_real_CPU('    Max cloud optical depth per layer (LW)', &
             &   'max_cloud_od_lw', this%max_cloud_od_lw)
        call print_real_CPU('    Max cloud optical depth per layer (SW)', &
             &   'max_cloud_od_sw', this%max_cloud_od_sw)
        call print_enum_CPU('    Shortwave entrapment is', EntrapmentName, &
             &          'i_3d_sw_entrapment', this%i_3d_sw_entrapment)
        call print_logical_CPU('    Multilayer longwave horizontal transport is', &
             'do_3d_lw_multilayer_effects', this%do_3d_lw_multilayer_effects)
        call print_logical_CPU('    Use matrix exponential everywhere is', &
             &   'use_expm_everywhere', this%use_expm_everywhere)
        call print_logical_CPU('    3D effects are', 'do_3d_effects', &
             &             this%do_3d_effects)

        if (this%do_3d_effects) then
          call print_logical_CPU('    Longwave side emissivity parameterization is', &
               &  'do_lw_side_emissivity', this%do_lw_side_emissivity)
          call print_real_CPU('    Clear-to-thick edge fraction is', &
               &  'clear_to_thick_fraction', this%clear_to_thick_fraction)
          call print_real_CPU('    Overhead sun factor is', &
               &  'overhead_sun_factor', this%overhead_sun_factor)
          call print_real_CPU('    Max gas optical depth for 3D effects', &
               &   'max_gas_od_3d', this%max_gas_od_3d)
          call print_real_CPU('    Max 3D transfer rate', &
               &   'max_3d_transfer_rate', this%max_3d_transfer_rate)
          call print_real_CPU('    Min cloud effective size (m)', &
               &   'min_cloud_effective_size', this%min_cloud_effective_size)
          call print_real_CPU('    Overhang factor', &
               &   'overhang_factor', this%overhang_factor)
        end if
           ...
           &  .or. this%i_solver_sw == ISolverMcICAACC &
           &  .or. this%i_solver_lw == ISolverMcICAACC ) then
        call print_logical_CPU('  Use vectorizable McICA cloud generator', &
             &   'use_vectorizable_generator', this%use_vectorizable_generator)
      end if
  ...
  subroutine get_sw_weights_CPU(this, wavelength1, wavelength2, &
    ...
    call this%gas_optics_sw%spectral_def%calc_mapping_from_bands_CPU( &
         &  [wavelength1, wavelength2], [1, 2, 3], mapping, &
  ...
  subroutine define_sw_albedo_intervals_CPU(this, ninterval, wavelength_bound, &
    ...
    ! is called immediately and overwrites any existing mapping.
    if (this%is_consolidated) then
      call this%consolidate_sw_albedo_intervals_CPU
    end if
  ...
  subroutine define_lw_emiss_intervals_CPU(this, ninterval, wavelength_bound, &
    ...
    if (this%is_consolidated) then
      call this%consolidate_lw_emiss_intervals_CPU
    end if
  ...
  subroutine consolidate_sw_albedo_intervals_CPU(this)
    ...
    if (this%iverbosesetup >= 2) then
      write(nulout, '(a)') 'Surface shortwave albedo'
      if (.not. this%do_nearest_spectral_sw_albedo) then
        call this%gas_optics_sw%spectral_def%print_mapping_from_bands_CPU(this%sw_albedo_weights, &
  ...
  subroutine consolidate_lw_emiss_intervals_CPU(this)
    ...
    if (this%do_weighted_surface_mapping) then
      call this%gas_optics_lw%spectral_def%calc_mapping_from_bands_CPU( &
           &  this%lw_emiss_wavelength_bound(1:ninterval-1), this%i_lw_emiss_index(1:ninterval), &
    ...
    if (this%iverbosesetup >= 2) then
      write(nulout, '(a)') 'Surface longwave emissivity'
      if (.not. this%do_nearest_spectral_lw_emiss) then
        call this%gas_optics_lw%spectral_def%print_mapping_from_bands_CPU(this%lw_emiss_weights, &
