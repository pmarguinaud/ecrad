...
contains
  ...
  subroutine setup_gas_optics_ecckd_GPU(config, lacc)
    ...
    if (config%do_sw .and. config%i_gas_model_sw == IGasModelECCKD) then
      ...
      call config%gas_optics_sw%read_GPU(trim(config%gas_optics_sw_file_name), &
           &                         config%iverbosesetup, lacc=lacc)
      ...
      if (config%iverbosesetup >= 2) then
        call config%gas_optics_sw%print_GPU(lacc=lacc)
      end if

      ! Override solar spectral irradiance, if filename provided
      if (config%use_spectral_solar_cycle) then
        call config%gas_optics_sw%read_spectral_solar_cycle_GPU(config%ssi_file_name, &
             &           config%iverbosesetup, config%use_updated_solar_spectrum, lacc=lacc)
      end if
    ...
    if (config%do_lw .and. config%i_gas_model_lw == IGasModelECCKD) then
      ...
      call config%gas_optics_lw%read_GPU(trim(config%gas_optics_lw_file_name), &
           &                         config%iverbosesetup, lacc=lacc)
      ...
      if (config%iverbosesetup >= 2) then
        call config%gas_optics_lw%print_GPU(lacc=lacc)
      end if
  ...
  subroutine gas_optics_ecckd_GPU(ncol,nlev,istartcol,iendcol, &
    ...
    if (.not. is_volume_mixing_ratio) then
      call gas%get_scaling_GPU(IVolumeMixingRatio, concentration_scaling, lacc=lacc)
    else
    ...
    if (config%do_sw .and. config%i_gas_model_sw == IGasModelECCKD) then
      ...
      if (is_volume_mixing_ratio) then
        call config%gas_optics_sw%calc_optical_depth_GPU(ncol,nlev,istartcol,iendcol, &
             &  NMaxGases, thermodynamics%pressure_hl, &
             &  temperature_fl, gas%mixing_ratio, &
             &  od_sw, rayleigh_od_fl=ssa_sw, lacc=lacc)
      else
        call config%gas_optics_sw%calc_optical_depth_GPU(ncol,nlev,istartcol,iendcol, &
             &  NMaxGases, thermodynamics%pressure_hl, &
             &  temperature_fl, gas%mixing_ratio, &
             &  od_sw, rayleigh_od_fl=ssa_sw, concentration_scaling=concentration_scaling, lacc=lacc)
      end if
      ...
      if (present(incoming_sw)) then
        if (single_level%spectral_solar_cycle_multiplier == 0.0_jprb) then
          call config%gas_optics_sw%calc_incoming_sw_GPU(single_level%solar_irradiance, &
               &        incoming_sw, lacc=lacc)
        else
          call config%gas_optics_sw%calc_incoming_sw_GPU(single_level%solar_irradiance, &
               &        incoming_sw, single_level%spectral_solar_cycle_multiplier, lacc=lacc)
        end if
      end if
    ...
    if (config%do_lw .and. config%i_gas_model_lw == IGasModelECCKD) then
      ...
      if (is_volume_mixing_ratio) then
        call config%gas_optics_lw%calc_optical_depth_GPU(ncol,nlev,istartcol,iendcol, &
             &  NMaxGases, thermodynamics%pressure_hl, &
             &  temperature_fl, gas%mixing_ratio, &
             &  od_lw, lacc=lacc)
      else
        call config%gas_optics_lw%calc_optical_depth_GPU(ncol,nlev,istartcol,iendcol, &
             &  NMaxGases, thermodynamics%pressure_hl, &
             &  temperature_fl, gas%mixing_ratio, &
             &  od_lw, concentration_scaling=concentration_scaling, lacc=lacc)
      end if

      ! Calculate the Planck function for each g point
      do jcol = istartcol,iendcol
        call config%gas_optics_lw%calc_planck_function_GPU(nlev+1, &
             &  thermodynamics%temperature_hl(jcol,:), planck_hl(:,:,jcol), lacc=lacc)
      end do
      call config%gas_optics_lw%calc_planck_function_GPU(iendcol+1-istartcol, &
           &  single_level%skin_temperature(istartcol:iendcol), &
           &  lw_emission(:,:), lacc=lacc)
!NEC$ forced_collapse
  ...
  subroutine setup_gas_optics_ecckd_CPU(config)
    ...
    if (config%do_sw .and. config%i_gas_model_sw == IGasModelECCKD) then
      ...
      ! Read shortwave ecCKD gas optics NetCDF file
      call config%gas_optics_sw%read_CPU(trim(config%gas_optics_sw_file_name), &
      ...
      if (config%iverbosesetup >= 2) then
        call config%gas_optics_sw%print_CPU()
      end if

      ! Override solar spectral irradiance, if filename provided
      if (config%use_spectral_solar_cycle) then
        call config%gas_optics_sw%read_spectral_solar_cycle_CPU(config%ssi_file_name, &
    ...
    if (config%do_lw .and. config%i_gas_model_lw == IGasModelECCKD) then
      ...
      ! Read longwave ecCKD gas optics NetCDF file
      call config%gas_optics_lw%read_CPU(trim(config%gas_optics_lw_file_name), &
           &                         config%iverbosesetup)
      ...
      if (config%iverbosesetup >= 2) then
        call config%gas_optics_lw%print_CPU()
      end if
  ...
  subroutine set_gas_units_ecckd_CPU(gas, lacc)
    ...
    call gas%set_units_CPU(IVolumeMixingRatio, lacc=lacc)
  ...
  subroutine gas_optics_ecckd_CPU(ncol,nlev,istartcol,iendcol, &
    ...
    if (.not. is_volume_mixing_ratio) then
      call gas%get_scaling_CPU(IVolumeMixingRatio, concentration_scaling)
    else
      concentration_scaling = 1.0_jprb
    end if
    ...
    if (config%do_sw .and. config%i_gas_model_sw == IGasModelECCKD) then
      ...
      if (is_volume_mixing_ratio) then
        call config%gas_optics_sw%calc_optical_depth_CPU(ncol,nlev,istartcol,iendcol, &
             &  NMaxGases, thermodynamics%pressure_hl, &
             &  temperature_fl, gas%mixing_ratio, &
      ...
      if (present(incoming_sw)) then
        if (single_level%spectral_solar_cycle_multiplier == 0.0_jprb) then
          call config%gas_optics_sw%calc_incoming_sw_CPU(single_level%solar_irradiance, &
               &        incoming_sw)
        else
          call config%gas_optics_sw%calc_incoming_sw_CPU(single_level%solar_irradiance, &
    ...
    if (config%do_lw .and. config%i_gas_model_lw == IGasModelECCKD) then
      ...
      if (is_volume_mixing_ratio) then
        call config%gas_optics_lw%calc_optical_depth_CPU(ncol,nlev,istartcol,iendcol, &
             &  NMaxGases, thermodynamics%pressure_hl, &
             &  temperature_fl, gas%mixing_ratio, &
             &  od_lw)
      else
      ...
      ! Calculate the Planck function for each g point
      do jcol = istartcol,iendcol
        call config%gas_optics_lw%calc_planck_function_CPU(nlev+1, &
             &  thermodynamics%temperature_hl(jcol,:), planck_hl(:,:,jcol))
      end do
      call config%gas_optics_lw%calc_planck_function_CPU(iendcol+1-istartcol, &
           &  single_level%skin_temperature(istartcol:iendcol), &
           &  lw_emission(:,:))
!NEC$ forced_collapse
      lw_emission = lw_emission * (1.0_jprb - lw_albedo)
