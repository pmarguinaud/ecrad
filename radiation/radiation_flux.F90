...
integer                         :: jcol
  ...
  subroutine calc_surface_spectral_GPU(this, config, istartcol, iendcol, lacc)
    ...
    if (config%do_sw .and. config%do_surface_sw_spectral_flux) then
      ...
      if (use_indexed_sum_vec) then
        call indexed_sum_vec_GPU(this%sw_dn_direct_surf_g, &
             &               config%i_band_from_reordered_g_sw, &
             &               this%sw_dn_direct_surf_band, istartcol, iendcol, lacc=lacc)
        call indexed_sum_vec_GPU(this%sw_dn_diffuse_surf_g, &
             &               config%i_band_from_reordered_g_sw, &
             &               this%sw_dn_surf_band, istartcol, iendcol, lacc=lacc)
        do jcol = istartcol,iendcol
        ...
        !$ACC   VECTOR_LENGTH(32*((config%n_g_sw-1)/32+1)) ASYNC(1) IF(lacc)
        !$ACC LOOP GANG
        do jcol = istartcol,iendcol
          call indexed_sum_GPU(this%sw_dn_direct_surf_g(:,jcol), &
               &           config%i_band_from_reordered_g_sw, &
               &           this%sw_dn_direct_surf_band(:,jcol))
          call indexed_sum_GPU(this%sw_dn_diffuse_surf_g(:,jcol), &
               &           config%i_band_from_reordered_g_sw, &
               &           this%sw_dn_surf_band(:,jcol))
          this%sw_dn_surf_band(:,jcol) &
               &  = this%sw_dn_surf_band(:,jcol) &
      ...
      if (config%do_clear) then
        if (use_indexed_sum_vec) then
          call indexed_sum_vec_GPU(this%sw_dn_direct_surf_clear_g, &
               &               config%i_band_from_reordered_g_sw, &
               &               this%sw_dn_direct_surf_clear_band, istartcol, iendcol, lacc=lacc)
          call indexed_sum_vec_GPU(this%sw_dn_diffuse_surf_clear_g, &
               &               config%i_band_from_reordered_g_sw, &
               &               this%sw_dn_surf_clear_band, istartcol, iendcol, lacc=lacc)
          do jcol = istartcol,iendcol
          ...
          !$ACC LOOP GANG
          do jcol = istartcol,iendcol
            call indexed_sum_GPU(this%sw_dn_direct_surf_clear_g(:,jcol), &
                 &           config%i_band_from_reordered_g_sw, &
                 &           this%sw_dn_direct_surf_clear_band(:,jcol))
            call indexed_sum_GPU(this%sw_dn_diffuse_surf_clear_g(:,jcol), &
                 &           config%i_band_from_reordered_g_sw, &
                 &           this%sw_dn_surf_clear_band(:,jcol))
            this%sw_dn_surf_clear_band(:,jcol) &
                 &  = this%sw_dn_surf_clear_band(:,jcol) &
      ...
      if (config%use_canopy_full_spectrum_sw) then
        ...
        if (use_indexed_sum_vec) then
          call indexed_sum_vec_GPU(this%sw_dn_direct_surf_g, &
               ...
               &               this%sw_dn_direct_surf_canopy, istartcol, iendcol, lacc=lacc)
          call indexed_sum_vec_GPU(this%sw_dn_diffuse_surf_g, &
               &               config%i_albedo_from_band_sw(config%i_band_from_reordered_g_sw), &
               &               this%sw_dn_diffuse_surf_canopy, istartcol, iendcol, lacc=lacc)
        else
          !$ACC PARALLEL DEFAULT(PRESENT) NUM_GANGS(iendcol-istartcol+1) NUM_WORKERS(1) &
          !$ACC   VECTOR_LENGTH(32*(config%n_g_sw-1)/32+1) ASYNC(1) IF(lacc)
          !$ACC LOOP GANG
        ...
        !$ACC             config%sw_albedo_weights) IF(lacc)
        !$ACC LOOP GANG VECTOR COLLAPSE(2)
        do jcol = istartcol,iendcol
    ...
    if (config%do_lw .and. config%do_canopy_fluxes_lw) then
      if (config%use_canopy_full_spectrum_lw) then
        ...
        if (use_indexed_sum_vec) then
          call indexed_sum_vec_GPU(this%lw_dn_surf_g, &
               &               config%i_emiss_from_band_lw(config%i_band_from_reordered_g_lw), &
               &               this%lw_dn_surf_canopy, istartcol, iendcol, lacc=lacc)
        else
          !$ACC PARALLEL DEFAULT(PRESENT) &
          !$ACC    NUM_GANGS(iendcol-istartcol+1) NUM_WORKERS(1) &
          !$ACC    VECTOR_LENGTH(32*(config%n_g_lw-1)/32+1) ASYNC(1) IF(lacc)
                    !$ACC LOOP GANG
          do jcol = istartcol,iendcol
        ...
        !$ACC DATA CREATE(lw_dn_surf_band) ASYNC(1) IF(lacc)
        ...
        if (use_indexed_sum_vec) then
          call indexed_sum_vec_GPU(this%lw_dn_surf_g, &
               ...
               &               lw_dn_surf_band, istartcol, iendcol, lacc=lacc)
        else
          !$ACC PARALLEL DEFAULT(PRESENT) NUM_GANGS(iendcol-istartcol+1) NUM_WORKERS(1) &
          !$ACC   VECTOR_LENGTH(32*(config%n_g_lw-1)/32+1) ASYNC(1) IF(lacc)
          !$ACC LOOP GANG
          do jcol = istartcol,iendcol
            call indexed_sum_GPU(this%lw_dn_surf_g(:,jcol), &
                 &           config%i_band_from_reordered_g_lw, &
        ...
        !$ACC PARALLEL DEFAULT(PRESENT) &
        !$ACC   PRESENT(this%lw_dn_surf_canopy, config%lw_emiss_weights) ASYNC(1) IF(lacc)
        !$ACC LOOP GANG VECTOR COLLAPSE(2)
        do jcol = istartcol,iendcol
          do jalbedoband = 1,nalbedoband
  ...
  subroutine calc_toa_spectral_GPU(this, config, istartcol, iendcol, lacc)
    ...
    if (config%do_sw .and. config%do_toa_spectral_flux) then
      ...
      if (use_indexed_sum_vec) then
        call indexed_sum_vec_GPU(this%sw_dn_toa_g, &
             &               config%i_band_from_reordered_g_sw, &
             &               this%sw_dn_toa_band, istartcol, iendcol, lacc=lacc)
        call indexed_sum_vec_GPU(this%sw_up_toa_g, &
             &               config%i_band_from_reordered_g_sw, &
             &               this%sw_up_toa_band, istartcol, iendcol, lacc=lacc)
      else
        do jcol = istartcol,iendcol
          call indexed_sum_GPU(this%sw_dn_toa_g(:,jcol), &
               &           config%i_band_from_reordered_g_sw, &
               &           this%sw_dn_toa_band(:,jcol))
          call indexed_sum_GPU(this%sw_up_toa_g(:,jcol), &
               &           config%i_band_from_reordered_g_sw, &
      ...
      if (config%do_clear) then
        if (use_indexed_sum_vec) then
          call indexed_sum_vec_GPU(this%sw_up_toa_clear_g, &
               &               config%i_band_from_reordered_g_sw, &
               &               this%sw_up_toa_clear_band, istartcol, iendcol, lacc=lacc)
        else
          do jcol = istartcol,iendcol
            call indexed_sum_GPU(this%sw_up_toa_clear_g(:,jcol), &
                 &               config%i_band_from_reordered_g_sw, &
    ...
    if (config%do_lw .and. config%do_toa_spectral_flux) then
      ...
      if (use_indexed_sum_vec) then
        call indexed_sum_vec_GPU(this%lw_up_toa_g, &
             &               config%i_band_from_reordered_g_lw, &
             &               this%lw_up_toa_band, istartcol, iendcol, lacc=lacc)
      else
        do jcol = istartcol,iendcol
          call indexed_sum_GPU(this%lw_up_toa_g(:,jcol), &
               &           config%i_band_from_reordered_g_lw, &
      ...
      if (config%do_clear) then
        if (use_indexed_sum_vec) then
          call indexed_sum_vec_GPU(this%lw_up_toa_clear_g, &
               &               config%i_band_from_reordered_g_lw, &
               &               this%lw_up_toa_clear_band, istartcol, iendcol, lacc=lacc)
        else
          do jcol = istartcol,iendcol
            call indexed_sum_GPU(this%lw_up_toa_clear_g(:,jcol), &
                 &               config%i_band_from_reordered_g_lw, &
  ...
  subroutine create_device_GPU(this, lacc)
    ...
    !$ACC ENTER DATA CREATE(this%lw_up) IF((allocated(this%lw_up)) .AND. lacc) ASYNC(1)
    !$ACC ENTER DATA CREATE(this%lw_dn) IF((allocated(this%lw_dn)) .AND. lacc) ASYNC(1)
    !$ACC ENTER DATA CREATE(this%lw_up_clear) IF((allocated(this%lw_up_clear)) .AND. lacc) ASYNC(1)
    !$ACC ENTER DATA CREATE(this%lw_dn_clear) IF((allocated(this%lw_dn_clear)) .AND. lacc) ASYNC(1)
    !$ACC ENTER DATA CREATE(this%sw_up) IF((allocated(this%sw_up)) .AND. lacc) ASYNC(1)
    !$ACC ENTER DATA CREATE(this%sw_dn) IF((allocated(this%sw_dn)) .AND. lacc) ASYNC(1)
    !$ACC ENTER DATA CREATE(this%sw_up_clear) IF((allocated(this%sw_up_clear)) .AND. lacc) ASYNC(1)
    !$ACC ENTER DATA CREATE(this%sw_dn_clear) IF((allocated(this%sw_dn_clear)) .AND. lacc) ASYNC(1)
    !$ACC ENTER DATA CREATE(this%sw_dn_direct) IF((allocated(this%sw_dn_direct)) .AND. lacc) ASYNC(1)
    !$ACC ENTER DATA CREATE(this%sw_dn_direct_clear) IF((allocated(this%sw_dn_direct_clear)) .AND. lacc) ASYNC(1)
    !$ACC ENTER DATA CREATE(this%lw_up_band) IF((allocated(this%lw_up_band)) .AND. lacc) ASYNC(1)
    !$ACC ENTER DATA CREATE(this%lw_dn_band) IF((allocated(this%lw_dn_band)) .AND. lacc) ASYNC(1)
    !$ACC ENTER DATA CREATE(this%lw_up_clear_band) IF((allocated(this%lw_up_clear_band)) .AND. lacc) ASYNC(1)
    !$ACC ENTER DATA CREATE(this%lw_dn_clear_band) IF((allocated(this%lw_dn_clear_band)) .AND. lacc) ASYNC(1)
    !$ACC ENTER DATA CREATE(this%sw_up_band) IF((allocated(this%sw_up_band)) .AND. lacc) ASYNC(1)
    !$ACC ENTER DATA CREATE(this%sw_dn_band) IF((allocated(this%sw_dn_band)) .AND. lacc) ASYNC(1)
    !$ACC ENTER DATA CREATE(this%sw_up_clear_band) IF((allocated(this%sw_up_clear_band)) .AND. lacc) ASYNC(1)
    !$ACC ENTER DATA CREATE(this%sw_dn_clear_band) IF((allocated(this%sw_dn_clear_band)) .AND. lacc) ASYNC(1)
    !$ACC ENTER DATA CREATE(this%sw_dn_direct_band) IF((allocated(this%sw_dn_direct_band)) .AND. lacc) ASYNC(1)
    !$ACC ENTER DATA CREATE(this%sw_dn_direct_clear_band) IF((allocated(this%sw_dn_direct_clear_band)) .AND. lacc) ASYNC(1)
    !$ACC ENTER DATA CREATE(this%sw_dn_surf_band) IF((allocated(this%sw_dn_surf_band)) .AND. lacc) ASYNC(1)
    !$ACC ENTER DATA CREATE(this%sw_dn_direct_surf_band) IF((allocated(this%sw_dn_direct_surf_band)) .AND. lacc) ASYNC(1)
    !$ACC ENTER DATA CREATE(this%sw_dn_surf_clear_band) IF((allocated(this%sw_dn_surf_clear_band)) .AND. lacc) ASYNC(1)
    !$ACC ENTER DATA CREATE(this%sw_dn_direct_surf_clear_band) IF((allocated(this%sw_dn_direct_surf_clear_band)) .AND. lacc) ASYNC(1)
    !$ACC ENTER DATA CREATE(this%lw_dn_surf_canopy) IF((allocated(this%lw_dn_surf_canopy)) .AND. lacc) ASYNC(1)
    !$ACC ENTER DATA CREATE(this%sw_dn_diffuse_surf_canopy) IF((allocated(this%sw_dn_diffuse_surf_canopy)) .AND. lacc) ASYNC(1)
    !$ACC ENTER DATA CREATE(this%sw_dn_direct_surf_canopy) IF((allocated(this%sw_dn_direct_surf_canopy)) .AND. lacc) ASYNC(1)
    !$ACC ENTER DATA CREATE(this%cloud_cover_sw) IF((allocated(this%cloud_cover_sw)) .AND. lacc) ASYNC(1)
    !$ACC ENTER DATA CREATE(this%cloud_cover_lw) IF((allocated(this%cloud_cover_lw)) .AND. lacc) ASYNC(1)
    !$ACC ENTER DATA CREATE(this%lw_derivatives) IF((allocated(this%lw_derivatives)) .AND. lacc) ASYNC(1)
    !$ACC ENTER DATA CREATE(this%lw_dn_surf_g) IF((allocated(this%lw_dn_surf_g)) .AND. lacc) ASYNC(1)
    !$ACC ENTER DATA CREATE(this%lw_dn_surf_clear_g) IF((allocated(this%lw_dn_surf_clear_g)) .AND. lacc) ASYNC(1)
    !$ACC ENTER DATA CREATE(this%sw_dn_diffuse_surf_g) IF((allocated(this%sw_dn_diffuse_surf_g)) .AND. lacc) ASYNC(1)
    !$ACC ENTER DATA CREATE(this%sw_dn_direct_surf_g) IF((allocated(this%sw_dn_direct_surf_g)) .AND. lacc) ASYNC(1)
    !$ACC ENTER DATA CREATE(this%sw_dn_diffuse_surf_clear_g) IF((allocated(this%sw_dn_diffuse_surf_clear_g)) .AND. lacc) ASYNC(1)
    !$ACC ENTER DATA CREATE(this%sw_dn_direct_surf_clear_g) IF((allocated(this%sw_dn_direct_surf_clear_g)) .AND. lacc) ASYNC(1)
    !$ACC ENTER DATA CREATE(this%lw_up_toa_g) IF((allocated(this%lw_up_toa_g)) .AND. lacc) ASYNC(1)
    !$ACC ENTER DATA CREATE(this%lw_up_toa_clear_g) IF((allocated(this%lw_up_toa_clear_g)) .AND. lacc) ASYNC(1)
    !$ACC ENTER DATA CREATE(this%sw_dn_toa_g) IF((allocated(this%sw_dn_toa_g)) .AND. lacc) ASYNC(1)
    !$ACC ENTER DATA CREATE(this%sw_up_toa_g) IF((allocated(this%sw_up_toa_g)) .AND. lacc) ASYNC(1)
    !$ACC ENTER DATA CREATE(this%sw_up_toa_clear_g) IF((allocated(this%sw_up_toa_clear_g)) .AND. lacc) ASYNC(1)
    !$ACC ENTER DATA CREATE(this%lw_up_toa_band) IF((allocated(this%lw_up_toa_band)) .AND. lacc) ASYNC(1)
    !$ACC ENTER DATA CREATE(this%lw_up_toa_clear_band) IF((allocated(this%lw_up_toa_clear_band)) .AND. lacc) ASYNC(1)
    !$ACC ENTER DATA CREATE(this%sw_dn_toa_band) IF((allocated(this%sw_dn_toa_band)) .AND. lacc) ASYNC(1)
    !$ACC ENTER DATA CREATE(this%sw_up_toa_band) IF((allocated(this%sw_up_toa_band)) .AND. lacc) ASYNC(1)
    !$ACC ENTER DATA CREATE(this%sw_up_toa_clear_band) IF((allocated(this%sw_up_toa_clear_band)) .AND. lacc) ASYNC(1)
  ...
  subroutine update_host_GPU(this, lacc)
    ...
    !$ACC UPDATE HOST(this%lw_up) IF((allocated(this%lw_up)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE HOST(this%lw_dn) IF((allocated(this%lw_dn)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE HOST(this%lw_up_clear) IF((allocated(this%lw_up_clear)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE HOST(this%lw_dn_clear) IF((allocated(this%lw_dn_clear)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE HOST(this%sw_up) IF((allocated(this%sw_up)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE HOST(this%sw_dn) IF((allocated(this%sw_dn)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE HOST(this%sw_up_clear) IF((allocated(this%sw_up_clear)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE HOST(this%sw_dn_clear) IF((allocated(this%sw_dn_clear)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE HOST(this%sw_dn_direct) IF((allocated(this%sw_dn_direct)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE HOST(this%sw_dn_direct_clear) IF((allocated(this%sw_dn_direct_clear)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE HOST(this%lw_up_band) IF((allocated(this%lw_up_band)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE HOST(this%lw_dn_band) IF((allocated(this%lw_dn_band)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE HOST(this%lw_up_clear_band) IF((allocated(this%lw_up_clear_band)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE HOST(this%lw_dn_clear_band) IF((allocated(this%lw_dn_clear_band)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE HOST(this%sw_up_band) IF((allocated(this%sw_up_band)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE HOST(this%sw_dn_band) IF((allocated(this%sw_dn_band)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE HOST(this%sw_up_clear_band) IF((allocated(this%sw_up_clear_band)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE HOST(this%sw_dn_clear_band) IF((allocated(this%sw_dn_clear_band)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE HOST(this%sw_dn_direct_band) IF((allocated(this%sw_dn_direct_band)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE HOST(this%sw_dn_direct_clear_band) IF((allocated(this%sw_dn_direct_clear_band)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE HOST(this%sw_dn_surf_band) IF((allocated(this%sw_dn_surf_band)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE HOST(this%sw_dn_direct_surf_band) IF((allocated(this%sw_dn_direct_surf_band)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE HOST(this%sw_dn_surf_clear_band) IF((allocated(this%sw_dn_surf_clear_band)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE HOST(this%sw_dn_direct_surf_clear_band) IF((allocated(this%sw_dn_direct_surf_clear_band)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE HOST(this%lw_dn_surf_canopy) IF((allocated(this%lw_dn_surf_canopy)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE HOST(this%sw_dn_diffuse_surf_canopy) IF((allocated(this%sw_dn_diffuse_surf_canopy)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE HOST(this%sw_dn_direct_surf_canopy) IF((allocated(this%sw_dn_direct_surf_canopy)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE HOST(this%cloud_cover_sw) IF((allocated(this%cloud_cover_sw)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE HOST(this%cloud_cover_lw) IF((allocated(this%cloud_cover_lw)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE HOST(this%lw_derivatives) IF((allocated(this%lw_derivatives)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE HOST(this%lw_dn_surf_g) IF((allocated(this%lw_dn_surf_g)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE HOST(this%lw_dn_surf_clear_g) IF((allocated(this%lw_dn_surf_clear_g)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE HOST(this%sw_dn_diffuse_surf_g) IF((allocated(this%sw_dn_diffuse_surf_g)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE HOST(this%sw_dn_direct_surf_g) IF((allocated(this%sw_dn_direct_surf_g)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE HOST(this%sw_dn_diffuse_surf_clear_g) IF((allocated(this%sw_dn_diffuse_surf_clear_g)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE HOST(this%sw_dn_direct_surf_clear_g) IF((allocated(this%sw_dn_direct_surf_clear_g)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE HOST(this%lw_up_toa_g) IF((allocated(this%lw_up_toa_g)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE HOST(this%lw_up_toa_clear_g) IF((allocated(this%lw_up_toa_clear_g)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE HOST(this%sw_dn_toa_g) IF((allocated(this%sw_dn_toa_g)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE HOST(this%sw_up_toa_g) IF((allocated(this%sw_up_toa_g)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE HOST(this%sw_up_toa_clear_g) IF((allocated(this%sw_up_toa_clear_g)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE HOST(this%lw_up_toa_band) IF((allocated(this%lw_up_toa_band)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE HOST(this%lw_up_toa_clear_band) IF((allocated(this%lw_up_toa_clear_band)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE HOST(this%sw_dn_toa_band) IF((allocated(this%sw_dn_toa_band)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE HOST(this%sw_up_toa_band) IF((allocated(this%sw_up_toa_band)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE HOST(this%sw_up_toa_clear_band) IF((allocated(this%sw_up_toa_clear_band)) .AND. lacc) ASYNC(1)
  ...
  subroutine update_device_GPU(this, lacc)
    ...
    !$ACC UPDATE DEVICE(this%lw_up) IF((allocated(this%lw_up)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE DEVICE(this%lw_dn) IF((allocated(this%lw_dn)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE DEVICE(this%lw_up_clear) IF((allocated(this%lw_up_clear)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE DEVICE(this%lw_dn_clear) IF((allocated(this%lw_dn_clear)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE DEVICE(this%sw_up) IF((allocated(this%sw_up)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE DEVICE(this%sw_dn) IF((allocated(this%sw_dn)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE DEVICE(this%sw_up_clear) IF((allocated(this%sw_up_clear)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE DEVICE(this%sw_dn_clear) IF((allocated(this%sw_dn_clear)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE DEVICE(this%sw_dn_direct) IF((allocated(this%sw_dn_direct)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE DEVICE(this%sw_dn_direct_clear) IF((allocated(this%sw_dn_direct_clear)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE DEVICE(this%lw_up_band) IF((allocated(this%lw_up_band)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE DEVICE(this%lw_dn_band) IF((allocated(this%lw_dn_band)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE DEVICE(this%lw_up_clear_band) IF((allocated(this%lw_up_clear_band)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE DEVICE(this%lw_dn_clear_band) IF((allocated(this%lw_dn_clear_band)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE DEVICE(this%sw_up_band) IF((allocated(this%sw_up_band)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE DEVICE(this%sw_dn_band) IF((allocated(this%sw_dn_band)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE DEVICE(this%sw_up_clear_band) IF((allocated(this%sw_up_clear_band)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE DEVICE(this%sw_dn_clear_band) IF((allocated(this%sw_dn_clear_band)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE DEVICE(this%sw_dn_direct_band) IF((allocated(this%sw_dn_direct_band)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE DEVICE(this%sw_dn_direct_clear_band) IF((allocated(this%sw_dn_direct_clear_band)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE DEVICE(this%sw_dn_surf_band) IF((allocated(this%sw_dn_surf_band)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE DEVICE(this%sw_dn_direct_surf_band) IF((allocated(this%sw_dn_direct_surf_band)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE DEVICE(this%sw_dn_surf_clear_band) IF((allocated(this%sw_dn_surf_clear_band)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE DEVICE(this%sw_dn_direct_surf_clear_band) IF((allocated(this%sw_dn_direct_surf_clear_band)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE DEVICE(this%lw_dn_surf_canopy) IF((allocated(this%lw_dn_surf_canopy)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE DEVICE(this%sw_dn_diffuse_surf_canopy) IF((allocated(this%sw_dn_diffuse_surf_canopy)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE DEVICE(this%sw_dn_direct_surf_canopy) IF((allocated(this%sw_dn_direct_surf_canopy)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE DEVICE(this%cloud_cover_sw) IF((allocated(this%cloud_cover_sw)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE DEVICE(this%cloud_cover_lw) IF((allocated(this%cloud_cover_lw)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE DEVICE(this%lw_derivatives) IF((allocated(this%lw_derivatives)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE DEVICE(this%lw_dn_surf_g) IF((allocated(this%lw_dn_surf_g)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE DEVICE(this%lw_dn_surf_clear_g) IF((allocated(this%lw_dn_surf_clear_g)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE DEVICE(this%sw_dn_diffuse_surf_g) IF((allocated(this%sw_dn_diffuse_surf_g)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE DEVICE(this%sw_dn_direct_surf_g) IF((allocated(this%sw_dn_direct_surf_g)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE DEVICE(this%sw_dn_diffuse_surf_clear_g) IF((allocated(this%sw_dn_diffuse_surf_clear_g)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE DEVICE(this%sw_dn_direct_surf_clear_g) IF((allocated(this%sw_dn_direct_surf_clear_g)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE DEVICE(this%lw_up_toa_g) IF((allocated(this%lw_up_toa_g)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE DEVICE(this%lw_up_toa_clear_g) IF((allocated(this%lw_up_toa_clear_g)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE DEVICE(this%sw_dn_toa_g) IF((allocated(this%sw_dn_toa_g)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE DEVICE(this%sw_up_toa_g) IF((allocated(this%sw_up_toa_g)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE DEVICE(this%sw_up_toa_clear_g) IF((allocated(this%sw_up_toa_clear_g)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE DEVICE(this%lw_up_toa_band) IF((allocated(this%lw_up_toa_band)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE DEVICE(this%lw_up_toa_clear_band) IF((allocated(this%lw_up_toa_clear_band)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE DEVICE(this%sw_dn_toa_band) IF((allocated(this%sw_dn_toa_band)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE DEVICE(this%sw_up_toa_band) IF((allocated(this%sw_up_toa_band)) .AND. lacc) ASYNC(1)
    !$ACC UPDATE DEVICE(this%sw_up_toa_clear_band) IF((allocated(this%sw_up_toa_clear_band)) .AND. lacc) ASYNC(1)
  ...
  subroutine delete_device_GPU(this, lacc)
    ...
    !$ACC EXIT DATA DELETE(this%lw_up) IF((allocated(this%lw_up)) .AND. lacc) ASYNC(1)
    !$ACC EXIT DATA DELETE(this%lw_dn) IF((allocated(this%lw_dn)) .AND. lacc) ASYNC(1)
    !$ACC EXIT DATA DELETE(this%lw_up_clear) IF((allocated(this%lw_up_clear)) .AND. lacc) ASYNC(1)
    !$ACC EXIT DATA DELETE(this%lw_dn_clear) IF((allocated(this%lw_dn_clear)) .AND. lacc) ASYNC(1)
    !$ACC EXIT DATA DELETE(this%sw_up) IF((allocated(this%sw_up)) .AND. lacc) ASYNC(1)
    !$ACC EXIT DATA DELETE(this%sw_dn) IF((allocated(this%sw_dn)) .AND. lacc) ASYNC(1)
    !$ACC EXIT DATA DELETE(this%sw_up_clear) IF((allocated(this%sw_up_clear)) .AND. lacc) ASYNC(1)
    !$ACC EXIT DATA DELETE(this%sw_dn_clear) IF((allocated(this%sw_dn_clear)) .AND. lacc) ASYNC(1)
    !$ACC EXIT DATA DELETE(this%sw_dn_direct) IF((allocated(this%sw_dn_direct)) .AND. lacc) ASYNC(1)
    !$ACC EXIT DATA DELETE(this%sw_dn_direct_clear) IF((allocated(this%sw_dn_direct_clear)) .AND. lacc) ASYNC(1)
    !$ACC EXIT DATA DELETE(this%lw_up_band) IF((allocated(this%lw_up_band)) .AND. lacc) ASYNC(1)
    !$ACC EXIT DATA DELETE(this%lw_dn_band) IF((allocated(this%lw_dn_band)) .AND. lacc) ASYNC(1)
    !$ACC EXIT DATA DELETE(this%lw_up_clear_band) IF((allocated(this%lw_up_clear_band)) .AND. lacc) ASYNC(1)
    !$ACC EXIT DATA DELETE(this%lw_dn_clear_band) IF((allocated(this%lw_dn_clear_band)) .AND. lacc) ASYNC(1)
    !$ACC EXIT DATA DELETE(this%sw_up_band) IF((allocated(this%sw_up_band)) .AND. lacc) ASYNC(1)
    !$ACC EXIT DATA DELETE(this%sw_dn_band) IF((allocated(this%sw_dn_band)) .AND. lacc) ASYNC(1)
    !$ACC EXIT DATA DELETE(this%sw_up_clear_band) IF((allocated(this%sw_up_clear_band)) .AND. lacc) ASYNC(1)
    !$ACC EXIT DATA DELETE(this%sw_dn_clear_band) IF((allocated(this%sw_dn_clear_band)) .AND. lacc) ASYNC(1)
    !$ACC EXIT DATA DELETE(this%sw_dn_direct_band) IF((allocated(this%sw_dn_direct_band)) .AND. lacc) ASYNC(1)
    !$ACC EXIT DATA DELETE(this%sw_dn_direct_clear_band) IF((allocated(this%sw_dn_direct_clear_band)) .AND. lacc) ASYNC(1)
    !$ACC EXIT DATA DELETE(this%sw_dn_surf_band) IF((allocated(this%sw_dn_surf_band)) .AND. lacc) ASYNC(1)
    !$ACC EXIT DATA DELETE(this%sw_dn_direct_surf_band) IF((allocated(this%sw_dn_direct_surf_band)) .AND. lacc) ASYNC(1)
    !$ACC EXIT DATA DELETE(this%sw_dn_surf_clear_band) IF((allocated(this%sw_dn_surf_clear_band)) .AND. lacc) ASYNC(1)
    !$ACC EXIT DATA DELETE(this%sw_dn_direct_surf_clear_band) IF((allocated(this%sw_dn_direct_surf_clear_band)) .AND. lacc) ASYNC(1)
    !$ACC EXIT DATA DELETE(this%lw_dn_surf_canopy) IF((allocated(this%lw_dn_surf_canopy)) .AND. lacc) ASYNC(1)
    !$ACC EXIT DATA DELETE(this%sw_dn_diffuse_surf_canopy) IF((allocated(this%sw_dn_diffuse_surf_canopy)) .AND. lacc) ASYNC(1)
    !$ACC EXIT DATA DELETE(this%sw_dn_direct_surf_canopy) IF((allocated(this%sw_dn_direct_surf_canopy)) .AND. lacc) ASYNC(1)
    !$ACC EXIT DATA DELETE(this%cloud_cover_sw) IF((allocated(this%cloud_cover_sw)) .AND. lacc) ASYNC(1)
    !$ACC EXIT DATA DELETE(this%cloud_cover_lw) IF((allocated(this%cloud_cover_lw)) .AND. lacc) ASYNC(1)
    !$ACC EXIT DATA DELETE(this%lw_derivatives) IF((allocated(this%lw_derivatives)) .AND. lacc) ASYNC(1)
    !$ACC EXIT DATA DELETE(this%lw_dn_surf_g) IF((allocated(this%lw_dn_surf_g)) .AND. lacc) ASYNC(1)
    !$ACC EXIT DATA DELETE(this%lw_dn_surf_clear_g) IF((allocated(this%lw_dn_surf_clear_g)) .AND. lacc) ASYNC(1)
    !$ACC EXIT DATA DELETE(this%sw_dn_diffuse_surf_g) IF((allocated(this%sw_dn_diffuse_surf_g)) .AND. lacc) ASYNC(1)
    !$ACC EXIT DATA DELETE(this%sw_dn_direct_surf_g) IF((allocated(this%sw_dn_direct_surf_g)) .AND. lacc) ASYNC(1)
    !$ACC EXIT DATA DELETE(this%sw_dn_diffuse_surf_clear_g) IF((allocated(this%sw_dn_diffuse_surf_clear_g)) .AND. lacc) ASYNC(1)
    !$ACC EXIT DATA DELETE(this%sw_dn_direct_surf_clear_g) IF((allocated(this%sw_dn_direct_surf_clear_g)) .AND. lacc) ASYNC(1)
    !$ACC EXIT DATA DELETE(this%lw_up_toa_g) IF((allocated(this%lw_up_toa_g)) .AND. lacc) ASYNC(1)
    !$ACC EXIT DATA DELETE(this%lw_up_toa_clear_g) IF((allocated(this%lw_up_toa_clear_g)) .AND. lacc) ASYNC(1)
    !$ACC EXIT DATA DELETE(this%sw_dn_toa_g) IF((allocated(this%sw_dn_toa_g)) .AND. lacc) ASYNC(1)
    !$ACC EXIT DATA DELETE(this%sw_up_toa_g) IF((allocated(this%sw_up_toa_g)) .AND. lacc) ASYNC(1)
    !$ACC EXIT DATA DELETE(this%sw_up_toa_clear_g) IF((allocated(this%sw_up_toa_clear_g)) .AND. lacc) ASYNC(1)
    !$ACC EXIT DATA DELETE(this%lw_up_toa_band) IF((allocated(this%lw_up_toa_band)) .AND. lacc) ASYNC(1)
    !$ACC EXIT DATA DELETE(this%lw_up_toa_clear_band) IF((allocated(this%lw_up_toa_clear_band)) .AND. lacc) ASYNC(1)
    !$ACC EXIT DATA DELETE(this%sw_dn_toa_band) IF((allocated(this%sw_dn_toa_band)) .AND. lacc) ASYNC(1)
    !$ACC EXIT DATA DELETE(this%sw_up_toa_band) IF((allocated(this%sw_up_toa_band)) .AND. lacc) ASYNC(1)
    !$ACC EXIT DATA DELETE(this%sw_up_toa_clear_band) IF((allocated(this%sw_up_toa_clear_band)) .AND. lacc) ASYNC(1)
...
integer                         :: jcol
  ...
  subroutine calc_surface_spectral_CPU(this, config, istartcol, iendcol)
    ...
    if (config%do_sw .and. config%do_surface_sw_spectral_flux) then
      ...
      if (use_indexed_sum_vec) then
        call indexed_sum_vec_CPU(this%sw_dn_direct_surf_g, &
             &               config%i_band_from_reordered_g_sw, &
             &               this%sw_dn_direct_surf_band, istartcol, iendcol)
        call indexed_sum_vec_CPU(this%sw_dn_diffuse_surf_g, &
             &               config%i_band_from_reordered_g_sw, &
        ...
        do jcol = istartcol,iendcol
          call indexed_sum_CPU(this%sw_dn_direct_surf_g(:,jcol), &
               &           config%i_band_from_reordered_g_sw, &
               &           this%sw_dn_direct_surf_band(:,jcol))
          call indexed_sum_CPU(this%sw_dn_diffuse_surf_g(:,jcol), &
               &           config%i_band_from_reordered_g_sw, &
               &           this%sw_dn_surf_band(:,jcol))
          this%sw_dn_surf_band(:,jcol) &
               &  = this%sw_dn_surf_band(:,jcol) &
      ...
      if (config%do_clear) then
        if (use_indexed_sum_vec) then
          call indexed_sum_vec_CPU(this%sw_dn_direct_surf_clear_g, &
               &               config%i_band_from_reordered_g_sw, &
               &               this%sw_dn_direct_surf_clear_band, istartcol, iendcol)
          call indexed_sum_vec_CPU(this%sw_dn_diffuse_surf_clear_g, &
               &               config%i_band_from_reordered_g_sw, &
          ...
          do jcol = istartcol,iendcol
            call indexed_sum_CPU(this%sw_dn_direct_surf_clear_g(:,jcol), &
                 &           config%i_band_from_reordered_g_sw, &
                 &           this%sw_dn_direct_surf_clear_band(:,jcol))
            call indexed_sum_CPU(this%sw_dn_diffuse_surf_clear_g(:,jcol), &
                 &           config%i_band_from_reordered_g_sw, &
                 &           this%sw_dn_surf_clear_band(:,jcol))
            this%sw_dn_surf_clear_band(:,jcol) &
                 &  = this%sw_dn_surf_clear_band(:,jcol) &
    ...
    if (config%do_lw .and. config%do_canopy_fluxes_lw) then
      if (config%use_canopy_full_spectrum_lw) then
        ...
        if (use_indexed_sum_vec) then
          call indexed_sum_vec_CPU(this%lw_dn_surf_g, &
               &               config%i_emiss_from_band_lw(config%i_band_from_reordered_g_lw), &
               &               this%lw_dn_surf_canopy, istartcol, iendcol)
        else
          do jcol = istartcol,iendcol
            call indexed_sum_CPU(this%lw_dn_surf_g(:,jcol), &
                 &           config%i_emiss_from_band_lw(config%i_band_from_reordered_g_lw), &
  ...
  subroutine calc_toa_spectral_CPU(this, config, istartcol, iendcol)
    ...
    if (config%do_sw .and. config%do_toa_spectral_flux) then
      ...
      if (use_indexed_sum_vec) then
        call indexed_sum_vec_CPU(this%sw_dn_toa_g, &
             &               config%i_band_from_reordered_g_sw, &
             &               this%sw_dn_toa_band, istartcol, iendcol)
        call indexed_sum_vec_CPU(this%sw_up_toa_g, &
             &               config%i_band_from_reordered_g_sw, &
             &               this%sw_up_toa_band, istartcol, iendcol)
      else
        do jcol = istartcol,iendcol
          call indexed_sum_CPU(this%sw_dn_toa_g(:,jcol), &
               &           config%i_band_from_reordered_g_sw, &
               &           this%sw_dn_toa_band(:,jcol))
          call indexed_sum_CPU(this%sw_up_toa_g(:,jcol), &
               &           config%i_band_from_reordered_g_sw, &
               &           this%sw_up_toa_band(:,jcol))
        end do
      end if
      ...
      if (config%do_clear) then
        if (use_indexed_sum_vec) then
          call indexed_sum_vec_CPU(this%sw_up_toa_clear_g, &
               &               config%i_band_from_reordered_g_sw, &
               &               this%sw_up_toa_clear_band, istartcol, iendcol)
        else
          do jcol = istartcol,iendcol
            call indexed_sum_CPU(this%sw_up_toa_clear_g(:,jcol), &
                 &               config%i_band_from_reordered_g_sw, &
                 &               this%sw_up_toa_clear_band(:,jcol))
          end do
        end if
    ...
    if (config%do_lw .and. config%do_toa_spectral_flux) then
      ...
      if (use_indexed_sum_vec) then
        call indexed_sum_vec_CPU(this%lw_up_toa_g, &
             &               config%i_band_from_reordered_g_lw, &
             &               this%lw_up_toa_band, istartcol, iendcol)
      else
        do jcol = istartcol,iendcol
          call indexed_sum_CPU(this%lw_up_toa_g(:,jcol), &
               &           config%i_band_from_reordered_g_lw, &
               &           this%lw_up_toa_band(:,jcol))
        end do
      end if
      ...
      if (config%do_clear) then
        if (use_indexed_sum_vec) then
          call indexed_sum_vec_CPU(this%lw_up_toa_clear_g, &
               &               config%i_band_from_reordered_g_lw, &
               &               this%lw_up_toa_clear_band, istartcol, iendcol)
        else
          do jcol = istartcol,iendcol
            call indexed_sum_CPU(this%lw_up_toa_clear_g(:,jcol), &
                 &               config%i_band_from_reordered_g_lw, &
                 &               this%lw_up_toa_clear_band(:,jcol))
          end do
        end if
