...
#include "radiation_delta_eddington.h"
  ...
  subroutine setup_general_cloud_optics_data_GPU(this, file_name, specdef, &
    ...
    call specdef%calc_mapping_GPU(wavenumber, mapping, &
         weighting_temperature=weighting_temperature, use_bands=use_bands, lacc=lacc)
    ...
    if (iverb >= 2) then
      ...
      write(nulout,'(a,i0,a,i0,a)') '  Wavenumber range: ', int(specdef%min_wavenumber_GPU(lacc)), '-', &
           &  int(specdef%max_wavenumber_GPU(lacc)), ' cm-1'
    end if
  ...
  subroutine setup_general_cloud_optics_data_CPU(this, file_name, specdef, &
    ...
    ! Define the mapping matrix
    call specdef%calc_mapping_CPU(wavenumber, mapping, &
    ...
    if (iverb >= 2) then
      ...
      write(nulout,'(a,i0,a,f6.1,a,f6.1,a)') '  Effective radius look-up: ', nre, ' points in range ', &
           &  effective_radius(1)*1.0e6_jprb, '-', effective_radius(nre)*1.0e6_jprb, ' um'
      write(nulout,'(a,i0,a,i0,a)') '  Wavenumber range: ', int(specdef%min_wavenumber_CPU()), '-', &
           &  int(specdef%max_wavenumber_CPU()), ' cm-1'
    end if
