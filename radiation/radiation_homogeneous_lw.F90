    ...
    do jcol = istartcol,iendcol
      ...
      do jlev = 1,nlev
        if (config%do_clear .or. cloud%fraction(jcol,jlev) &
          ...
          if (config%do_lw_aerosol_scattering) then
            ...
            g_total   = g(:,jlev,jcol)
            call calc_two_stream_gammas_lw_GPU(ng, ssa_total, g_total, &
                 &  gamma1, gamma2)
            call calc_reflectance_transmittance_lw_GPU(ng, &
            ...
            call calc_no_scattering_transmittance_lw_GPU(ng, od(:,jlev,jcol), &
                 &  planck_hl(:,jlev,jcol), planck_hl(:,jlev+1, jcol), &
      ...
      if (config%do_clear) then
        ...
        if (config%do_save_spectral_flux) then
          call indexed_sum_profile_GPU(flux_up, config%i_spec_from_reordered_g_lw, &
               &                   flux%lw_up_clear_band(:,jcol,:), lacc=lacc)
          call indexed_sum_profile_GPU(flux_dn, config%i_spec_from_reordered_g_lw, &
               &                   flux%lw_dn_clear_band(:,jcol,:), lacc=lacc)
        end if
      ...
      if (is_cloudy_profile .or. .not. config%do_clear) then
        ...
        if (config%do_save_spectral_flux) then
          call indexed_sum_profile_GPU(flux_up, config%i_spec_from_reordered_g_lw, &
               &                   flux%lw_up_band(:,jcol,:), lacc=lacc)
          call indexed_sum_profile_GPU(flux_dn, config%i_spec_from_reordered_g_lw, &
               &                   flux%lw_dn_band(:,jcol,:), lacc=lacc)
        end if
  ...
  subroutine solver_homogeneous_lw_CPU(nlev,istartcol,iendcol, &
    ...
    do jcol = istartcol,iendcol
      ...
      do jlev = 1,nlev
        if (config%do_clear .or. cloud%fraction(jcol,jlev) &
          ...
          if (config%do_lw_aerosol_scattering) then
            ...
            ssa_total = ssa(:,jlev,jcol)
            g_total   = g(:,jlev,jcol)
            call calc_two_stream_gammas_lw_CPU(ng, ssa_total, g_total, &
                 &  gamma1, gamma2)
            call calc_reflectance_transmittance_lw_CPU(ng, &
                 &  od(:,jlev,jcol), gamma1, gamma2, &
                 &  planck_hl(:,jlev,jcol), planck_hl(:,jlev+1,jcol), &
                 &  reflectance(:,jlev), transmittance(:,jlev), &
                 &  source_up(:,jlev), source_dn(:,jlev))
            ...
            ! Non-scattering case: use simpler functions for
            ! transmission and emission
            call calc_no_scattering_transmittance_lw_CPU(ng, od(:,jlev,jcol), &
                 &  planck_hl(:,jlev,jcol), planck_hl(:,jlev+1, jcol), &
                 &  transmittance(:,jlev), source_up(:,jlev), source_dn(:,jlev))          
            ! Ensure that clear-sky reflectance is zero since it may be
            ! used in cloudy-sky case
      ...
      if (config%do_clear) then
        ...
        ! Save the spectral fluxes if required
        if (config%do_save_spectral_flux) then
          call indexed_sum_profile_CPU(flux_up, config%i_spec_from_reordered_g_lw, &
               &                   flux%lw_up_clear_band(:,jcol,:))
          call indexed_sum_profile_CPU(flux_dn, config%i_spec_from_reordered_g_lw, &
