    ...
    do jcol = istartcol,iendcol
      ...
      if (single_level%cos_sza(jcol) > 0.0_jprb) then
        ...
        if (.not. config%do_sw_delta_scaling_with_gases) then
          ...
          do jlev = 1,nlev
            if (config%do_clear .or. cloud%fraction(jcol,jlev) &
              ...
              call calc_two_stream_gammas_sw_GPU(ng, cos_sza, &
                   &  ssa(:,jlev,jcol), g(:,jlev,jcol), &
                   &  gamma1, gamma2, gamma3)
              call calc_reflectance_transmittance_sw_GPU(ng, &
                   &  cos_sza, &
          ...
          do jlev = 1,nlev
            if (config%do_clear .or. cloud%fraction(jcol,jlev) &
              ...
              call calc_two_stream_gammas_sw_GPU(ng, &
        ...
        if (config%do_clear) then
          ...
          call adding_ica_sw_GPU(ng, nlev, incoming_sw(:,jcol), &
          ...
          if (config%do_save_spectral_flux) then
            call indexed_sum_profile_GPU(flux_up, config%i_spec_from_reordered_g_sw, &
                 &                   flux%sw_up_clear_band(:,jcol,:), lacc=lacc)
            call indexed_sum_profile_GPU(flux_dn_direct, config%i_spec_from_reordered_g_sw, &
                 &                   flux%sw_dn_clear_band(:,jcol,:), lacc=lacc)
            if (allocated(flux%sw_dn_direct_clear_band)) then
              flux%sw_dn_direct_clear_band(:,jcol,:) &
                   &  = flux%sw_dn_clear_band(:,jcol,:)
            end if
            call add_indexed_sum_profile_GPU(flux_dn_diffuse, &
        ...
        if (is_cloudy_profile .or. .not. config%do_clear) then
          ...
          if (config%do_save_spectral_flux) then
            call indexed_sum_profile_GPU(flux_up, config%i_spec_from_reordered_g_sw, &
                 &                   flux%sw_up_band(:,jcol,:), lacc=lacc)
            call indexed_sum_profile_GPU(flux_dn_direct, config%i_spec_from_reordered_g_sw, &
                 &                   flux%sw_dn_band(:,jcol,:), lacc=lacc)
      ...
      if (single_level%cos_sza(jcol) > 0.0_jprb) then
        ...
        if (.not. config%do_sw_delta_scaling_with_gases) then
          ...
          do jlev = 1,nlev
            if (config%do_clear .or. cloud%fraction(jcol,jlev) &
                 &                 < config%cloud_fraction_threshold) then
              call calc_two_stream_gammas_sw_CPU(ng, cos_sza, &
                   &  ssa(:,jlev,jcol), g(:,jlev,jcol), &
                   &  gamma1, gamma2, gamma3)
              call calc_reflectance_transmittance_sw_CPU(ng, &
                   &  cos_sza, &
                   &  od(:,jlev,jcol), ssa(:,jlev,jcol), &
                   &  gamma1, gamma2, gamma3, &
                   &  reflectance(:,jlev), transmittance(:,jlev), &
          ...
          do jlev = 1,nlev
            if (config%do_clear .or. cloud%fraction(jcol,jlev) &
              ...
              g_total   =   g(:,jlev,jcol)
              call delta_eddington(od_total, ssa_total, g_total)
              call calc_two_stream_gammas_sw_CPU(ng, &
                   &  cos_sza, ssa_total, g_total, &
                   &  gamma1, gamma2, gamma3)
              call calc_reflectance_transmittance_sw_CPU(ng, &
                   &  cos_sza, od_total, ssa_total, &
        ...
        if (config%do_clear) then
          ! Use adding method to compute fluxes
          call adding_ica_sw_CPU(ng, nlev, incoming_sw(:,jcol), &
               &  albedo_diffuse(:,jcol), albedo_direct(:,jcol), &
               &  cos_sza, reflectance, transmittance, ref_dir, trans_dir_diff, &
               &  trans_dir_dir, flux_up, flux_dn_diffuse, flux_dn_direct, &
               &  albedo=tmp_work_albedo, &
          ...
          ! Save the spectral fluxes if required
          if (config%do_save_spectral_flux) then
            call indexed_sum_profile_CPU(flux_up, config%i_spec_from_reordered_g_sw, &
                 &                   flux%sw_up_clear_band(:,jcol,:))
            call indexed_sum_profile_CPU(flux_dn_direct, config%i_spec_from_reordered_g_sw, &
                 &                   flux%sw_dn_clear_band(:,jcol,:))
            if (allocated(flux%sw_dn_direct_clear_band)) then
              flux%sw_dn_direct_clear_band(:,jcol,:) &
                   &  = flux%sw_dn_clear_band(:,jcol,:)
            end if
            call add_indexed_sum_profile_CPU(flux_dn_diffuse, &
                 &                       config%i_spec_from_reordered_g_sw, &
                 &                       flux%sw_dn_clear_band(:,jcol,:))
          end if
