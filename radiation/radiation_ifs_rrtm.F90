...
#include "srtm_init.intfb.h"
    ...
    if (config%do_setup_ifsrrtm) then
      !call SURDI
      call SURRTAB
      call SURRTPK_GPU(lacc=lacc)
      call SURRTRF_GPU(lacc=lacc)
      if (do_lw) then
        call RRTM_INIT_140GP_GPU(directory, lacc=lacc)
      end if
      if (do_sw) then
        call SRTM_INIT_GPU(directory, lacc=lacc)
      end if
    end if
    ...
    if (do_sw) then
      ...
      call config%gas_optics_sw%spectral_def%allocate_bands_only_GPU(SolarReferenceTemperature, &
           ...
           &   12850.0_jprb, 16000.0_jprb, 22650.0_jprb, 29000.0_jprb, 38000.0_jprb, 50000.0_jprb, 2600.0_jprb], lacc=lacc)
    ...
    if (do_lw) then
      ...
      call config%gas_optics_lw%spectral_def%allocate_bands_only_GPU(TerrestrialReferenceTemperature, &
           ...
           &   1390.0_jprb, 1480.0_jprb, 1800.0_jprb, 2080.0_jprb, 2250.0_jprb, 2380.0_jprb, 2600.0_jprb, 3250.0_jprb], lacc=lacc)
  ...
  subroutine set_gas_units_rrtm_GPU(gas, lacc)
    ...
    call gas%set_units_GPU(IMassMixingRatio, lacc=lacc)
...
#include "srtm_gas_optical_depth.intfb.h"
    ...
    call gas%assert_units_GPU(IMassMixingRatio, lacc=lacc)
    ...
    CALL RRTM_PREPARE_GASES_GPU &
         ...
         &  ZPAVEL , ZTAVEL , ZPZ , ZTZ, IREFLECT, lacc=lacc)

    if (do_lw) then

      CALL RRTM_SETCOEF_140GP_GPU &
           ...
           &  ZRAT_N2OCO2, ZRAT_N2OCO2_1, ZRAT_O3CO2, ZRAT_O3CO2_1, lacc=lacc)
      ...
      CALL RRTM_GAS_OPTICAL_DEPTH_GPU &
      ...
      if (present(lw_albedo)) then
        ...
        call planck_function_atmos_GPU(nlev, istartcol, iendcol, config, &
             &                     thermodynamics, ZPFRAC, planck_hl, lacc=lacc)

        if (single_level%is_simple_surface) then
          call planck_function_surf_GPU(istartcol, iendcol, config, &
    ...
    if (do_sw) then
      ...
      CALL SRTM_SETCOEF_GPU &
           ...
           & JP      , JT       , JT1     , single_level%cos_sza(istartcol:iendcol), lacc=lacc  &
           & )
      ...
      CALL SRTM_GAS_OPTICAL_DEPTH_GPU &
           ...
           & ZFAC00  , ZFAC01   , ZFAC10 , ZFAC11  ,&
           & JP      , JT       , JT1    ,&
           & ZOD_SW  , ZSSA_SW  , ZINCSOL, lacc=lacc )
...
!      if (.true.) then
        ...
        !$ACC   VECTOR_LENGTH(32) ASYNC(1) IF(lacc)
        !$ACC LOOP GANG
        do jcol = istartcol,iendcol
          !$ACC LOOP SEQ
          do jlev = 1,nlev
  ...
  subroutine planck_function_GPU(config, temperature, planck_surf, lacc)
    ...
    call planck_function_surf_GPU(1, 1, config, spread(temperature,1,1), &
         &                    frac, planck_surf, lacc=lacc)
...
#include "srtm_init.intfb.h"
    ...
    if (config%do_setup_ifsrrtm) then
      ...
      call SURRTRF_CPU
      if (do_lw) then
        call RRTM_INIT_140GP_CPU(directory)
      end if
      if (do_sw) then
        call SRTM_INIT_CPU(directory)
      end if
    end if
  ...
  subroutine set_gas_units_rrtm_CPU(gas, lacc)
    ...
    call gas%set_units_CPU(IMassMixingRatio, lacc=lacc)
...
#include "srtm_gas_optical_depth.intfb.h"
    ...
    ! Check we have gas mixing ratios in the right units
    call gas%assert_units_CPU(IMassMixingRatio)

    ! Warning: O2 is hard-coded within the following function so the
    ! user-provided concentrations of this gas are ignored for both
    ! the longwave and shortwave
    CALL RRTM_PREPARE_GASES_CPU &
         & ( istartcol, iendcol, ncol, nlev, &
         &   thermodynamics%pressure_hl(:,istartlev:iendlev+1), &
         &   pressure_fl, &
         &   thermodynamics%temperature_hl(:,istartlev:iendlev+1), &
    ...
    if (do_lw) then
      ...
      CALL RRTM_GAS_OPTICAL_DEPTH_CPU &
           &( istartcol, iendcol, nlev, ZOD_LW, ZPAVEL, ZCOLDRY, ZCOLBRD, ZWX ,&
           &  ZTAUAERL, ZFAC00 , ZFAC01, ZFAC10 , ZFAC11 , ZFORFAC,ZFORFRAC,INDFOR, &
           &  JP, JT, JT1, ZONEMINUS ,&
           &  ZCOLH2O , ZCOLCO2, ZCOLO3, ZCOLN2O, ZCOLCH4, ZCOLO2,ZCO2MULT ,&
      ...
      if (present(lw_albedo)) then
        ...
        call planck_function_atmos_CPU(nlev, istartcol, iendcol, config, &
             &                     thermodynamics, ZPFRAC, planck_hl)

        if (single_level%is_simple_surface) then
          call planck_function_surf_CPU(istartcol, iendcol, config, &
               &                    single_level%skin_temperature, ZPFRAC(:,:,1), &
               &                    lw_emission)
    ...
    if (do_sw) then
      ...
      CALL SRTM_GAS_OPTICAL_DEPTH_CPU &
           &( istartcol, iendcol , nlev  , ZONEMINUS_ARRAY,&
           & single_level%cos_sza(istartcol:iendcol), ILAYTROP,&
           & ZCOLCH4 , ZCOLCO2  , ZCOLH2O, ZCOLMOL , ZCOLO2   , ZCOLO3,&
           & ZFORFAC , ZFORFRAC , INDFOR , ZSELFFAC, ZSELFFRAC, INDSELF,&
...
!      if (.true.) then
  ...
  subroutine planck_function_CPU(config, temperature, planck_surf)
    ...
    call planck_function_surf_CPU(1, 1, config, spread(temperature,1,1), &
         &                    frac, planck_surf)
