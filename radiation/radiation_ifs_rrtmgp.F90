...
contains
  ...
  ! and incoming shortwave radiation at top-of-atmosphere
    ...
    idx_hcfc22 = string_loc_in_array_GPU('hcfc22',config%rrtmgp_gas_names, lacc)
    ...
    if (config%i_gas_model_sw == IGasModelRRTMGP_NN .or. config%i_gas_model_sw == IGasModelRRTMGP) then
      ...
      call config%gas_optics_sw%spectral_def%allocate_bands_only_GPU( SolarReferenceTemperature, &
          & config%k_dist_sw%band_lims_wvn(1,:), config%k_dist_sw%band_lims_wvn(2,:), lacc=lacc)
    ...
    if (config%i_gas_model_lw == IGasModelRRTMGP_NN .or. config%i_gas_model_lw == IGasModelRRTMGP) then
      ...
      call config%gas_optics_lw%spectral_def%allocate_bands_only_GPU( TerrestrialReferenceTemperature, &
      & config%k_dist_lw%band_lims_wvn(1,:), config%k_dist_lw%band_lims_wvn(2,:), lacc=lacc)
  ...
  call gas%set_units_GPU(IVolumeMixingRatio, lacc=lacc)
  ...
  subroutine gas_optics_ifs_rrtmgp_GPU(ncol,nlev,istartcol,iendcol, &
    ...
    call gas%assert_units_GPU(IVolumeMixingRatio, scale_factor=1.0_jprb, &
         &                istatus=is_volume_mixing_ratio, lacc=lacc)
    ...
    call stop_on_err_GPU(gas_rrtmgp%init(config%rrtmgp_gas_names), lacc=lacc)
    ...
    call stop_on_err_GPU(gas_rrtmgp%set_vmr("n2", 0.781000018_jprb), lacc=lacc)
    ...
    do jgas = 1,NMaxGases
      if (gas%is_present(jgas)) then
        ...
        if (GasLowerCaseName(jgas)=="hcfc22") then
          ...
          call stop_on_err_GPU(gas_rrtmgp%set_vmr('cfc22',transpose(gas%mixing_ratio(istartcol:iendcol,:,jgas))), lacc=lacc)
        else
          call stop_on_err_GPU(gas_rrtmgp%set_vmr(GasLowerCaseName(jgas), &
              & transpose(gas%mixing_ratio(istartcol:iendcol,:,jgas))), lacc=lacc)
        end if
      end if
    end do
    ...
    if (config%do_sw .and. (config%i_gas_model_sw == IGasModelRRTMGP .or. &
      ...
      if (use_neural_nets) then
        call stop_on_err_GPU(config%k_dist_sw%gas_optics_ext_ecrad( &
            ...
            &   neural_nets = config%rrtmgp_neural_nets(1:2)), lacc=lacc)
      else
        call stop_on_err_GPU(config%k_dist_sw%gas_optics_ext_ecrad( &
            &   ncol_loc, nlev, config%n_g_sw, &
            &   pressure_fl, &
            ...
            &   temperature_fl, &
            &   gas_rrtmgp, &
            &   od_sw(:,:,istartcol:iendcol), ssa_sw(:,:,istartcol:iendcol)), lacc=lacc)!, incoming_sw))
      end if
    ...
    if (config%do_lw .and. (config%i_gas_model_lw == IGasModelRRTMGP .or. &
        ...
        if (use_neural_nets) then
          call stop_on_err_GPU(config%k_dist_lw%gas_optics_int_ecrad( &
              ...
              &   neural_nets = config%rrtmgp_neural_nets(3:)), lacc=lacc)
        else
          call stop_on_err_GPU(config%k_dist_lw%gas_optics_int_ecrad( &
              &   ncol_loc, nlev, config%n_g_lw, &
              &   pressure_fl, &
              ...
              &   single_level%skin_temperature(istartcol:iendcol), &
              &   gas_rrtmgp, &
              &   od_lw(:,:,istartcol:iendcol), lw_emission(:,istartcol:iendcol), planck_hl(:,:,istartcol:iendcol)), lacc=lacc)
        end if
  ...
  pure function string_loc_in_array_GPU(string, array, lacc)
    ...
    string_loc_in_array_GPU = -1
    lc_string = lower_case_GPU(trim(string), lacc)
    do i = 1, size(array)
      if(lc_string == lower_case_GPU(trim(array(i)), lacc)) then
        string_loc_in_array_GPU = i
        exit
      end if
    end do
  ...
  subroutine setup_gas_optics_ifs_rrtmgp_CPU(config)
    ...
    ! HCFC22 is called CFC22 in RRTMGP look up tables; we can change the name here
    idx_hcfc22 = string_loc_in_array_CPU('hcfc22',config%rrtmgp_gas_names)
    if (idx_hcfc22 /= -1) config%rrtmgp_gas_names(idx_hcfc22) = 'cfc22'
    ...
    if (config%i_gas_model_sw == IGasModelRRTMGP_NN .or. config%i_gas_model_sw == IGasModelRRTMGP) then
      ...
      ! Store band positions if using generalized cloud or aerosol
      ! Read from the RRTMGP k-distribution derived type
      call config%gas_optics_sw%spectral_def%allocate_bands_only_CPU( SolarReferenceTemperature, &
    ...
    if (config%i_gas_model_lw == IGasModelRRTMGP_NN .or. config%i_gas_model_lw == IGasModelRRTMGP) then
      ...
      call config%gas_optics_lw%spectral_def%allocate_bands_only_CPU( TerrestrialReferenceTemperature, &
  ...
  call gas%set_units_CPU(IVolumeMixingRatio)
  ...
  subroutine gas_optics_ifs_rrtmgp_CPU(ncol,nlev,istartcol,iendcol, &
    ...
    ! Check that the gas concentrations are stored in volume mixing
    ! ratio with no scaling; if not, return a vector of scalings
    call gas%assert_units_CPU(IVolumeMixingRatio, scale_factor=1.0_jprb, &
    ...
    ! Initialize RRTMGP gas derived type
    call stop_on_err_CPU(gas_rrtmgp%init(config%rrtmgp_gas_names))

    ! Write nitrogen with a constant value - here we assume it's missing from ecRAD gas type
    ! (but that 'N2' is already in config%rrtmgp_gas_names)
    ! Bit iffy to hardcode like this, but avoids polluting gas%mixing_ratio with more gases that are not used
    call stop_on_err_CPU(gas_rrtmgp%set_vmr("n2", 0.781000018_jprb))
    ...
    do jgas = 1,NMaxGases
      if (gas%is_present(jgas)) then
        ...
        if (GasLowerCaseName(jgas)=="hcfc22") then
          ! HCFC22 has different name in RRTMGP
          call stop_on_err_CPU(gas_rrtmgp%set_vmr('cfc22',transpose(gas%mixing_ratio(istartcol:iendcol,:,jgas))))
        else
          call stop_on_err_CPU(gas_rrtmgp%set_vmr(GasLowerCaseName(jgas), &
    ...
    if (config%do_sw .and. (config%i_gas_model_sw == IGasModelRRTMGP .or. &
      ...
      if (use_neural_nets) then
        call stop_on_err_CPU(config%k_dist_sw%gas_optics_ext_ecrad( &
            &   ncol_loc, nlev, config%n_g_sw, &
            &   pressure_fl, &
            &   pressure_hl, &
            &   temperature_fl, &
    ...
    if (config%do_lw .and. (config%i_gas_model_lw == IGasModelRRTMGP .or. &
        &   config%i_gas_model_lw == IGasModelRRTMGP_NN)) then
        if (use_neural_nets) then
          call stop_on_err_CPU(config%k_dist_lw%gas_optics_int_ecrad( &
              &   ncol_loc, nlev, config%n_g_lw, &
              &   pressure_fl, &
              &   pressure_hl, &
              &   temperature_fl, &
  ...
  pure function string_loc_in_array_CPU(string, array)
    ...
    string_loc_in_array_CPU = -1
    lc_string = lower_case_CPU(trim(string))
    do i = 1, size(array)
      if(lc_string == lower_case_CPU(trim(array(i)))) then
        string_loc_in_array_CPU = i
        exit
      end if
    end do
