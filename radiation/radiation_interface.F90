...
contains
  ...
  subroutine setup_radiation_GPU(config, lacc)
    ...
    if (config%i_gas_model_sw == IGasModelMonochromatic) then
      ...
      if (config%i_gas_model_sw == IGasModelIFSRRTMG .or. config%i_gas_model_lw == IGasModelIFSRRTMG) then
        call setup_gas_optics_rrtm_GPU(config, trim(config%directory_name), lacc=lacc)
      end if
      if (config%i_gas_model_sw == IGasModelECCKD .or. config%i_gas_model_lw == IGasModelECCKD) then
        call setup_gas_optics_ecckd_GPU(config, lacc=lacc)
      end if
      if (config%i_gas_model_sw == IGasModelRRTMGP .or. config%i_gas_model_lw == IGasModelRRTMGP .or. &
        ...
        call setup_gas_optics_ifs_rrtmgp_GPU(config, lacc=lacc)
      end if
    end if
    ...
    if (config%do_sw) then
      call config%consolidate_sw_albedo_intervals_GPU(lacc=lacc)
    end if
    if (config%do_lw) then
      call config%consolidate_lw_emiss_intervals_GPU(lacc=lacc)
    end if
    ...
    if (config%do_clouds) then
      ...
      elseif (config%use_general_cloud_optics) then
        call setup_general_cloud_optics_GPU(config, lacc=lacc)
      else
        call setup_cloud_optics_GPU(config, lacc=lacc)
      end if
    end if
...
!        call setup_aerosol_optics_mono(config)
        ...
        call setup_aerosol_optics_GPU(config, lacc=lacc)
      end if
    end if
    ...
    if (         config%i_solver_sw == ISolverMcICA &
      ...
      call config%pdf_sampler%setup_GPU(config%cloud_pdf_file_name, &
           &                        iverbose=config%iverbosesetup, lacc=lacc)
    end if
  ...
  subroutine set_gas_units_GPU(config, gas, lacc)
    ...
    if (config%i_gas_model_sw == IGasModelMonochromatic) then
      call set_gas_units_mono_GPU(gas, lacc=lacc)
    elseif (config%i_gas_model_sw == IGasModelIFSRRTMG &
      ...
      call set_gas_units_rrtm_GPU(gas, lacc=lacc)
    elseif (config%i_gas_model_sw == IGasModelRRTMGP &
      ...
      call set_gas_units_rrtmgp_GPU(gas, lacc=lacc)
    else
      ! Use volume mixing ratio preferred by ecCKD
      call set_gas_units_ecckd_GPU(gas, lacc=lacc)
    end if
  ...
  subroutine radiation_GPU(ncol, nlev, istartcol, iendcol, config, &
    ...
    !$ACC             sw_albedo_diffuse, incoming_sw) IF(lacc)
    ...
    if (thermodynamics%pressure_hl(istartcol,2) &
      ...
      ! fluxes
      call radiation_reverse_GPU(ncol, nlev, istartcol, iendcol, config, &
           &  single_level, thermodynamics, gas, cloud, aerosol, flux, lacc=lacc)
    else
      ...
      call single_level%get_albedos_GPU(istartcol, iendcol, config, &
      ...
      if (config%i_gas_model_sw == IGasModelMonochromatic) then
        call gas_optics_mono_GPU(ncol,nlev,istartcol,iendcol, config, &
             &  single_level, thermodynamics, gas, lw_albedo, &
             &  od_lw, od_sw, ssa_sw, &
             &  planck_hl, lw_emission, incoming_sw, lacc=lacc)
      else
        if (config%i_gas_model_sw == IGasModelIFSRRTMG &
             &   .or. config%i_gas_model_lw == IGasModelIFSRRTMG) then
          call gas_optics_rrtm_GPU(ncol,nlev,istartcol,iendcol, config, &
               &  single_level, thermodynamics, gas, &
               &  od_lw, od_sw, ssa_sw, lw_albedo=lw_albedo, &
               &  planck_hl=planck_hl, lw_emission=lw_emission, &
               &  incoming_sw=incoming_sw, lacc=lacc)
        end if
        if (config%i_gas_model_sw == IGasModelRRTMGP &
             &   .or. config%i_gas_model_lw == IGasModelRRTMGP &
             &   .or. config%i_gas_model_sw == IGasModelRRTMGP_NN &
             &   .or. config%i_gas_model_lw == IGasModelRRTMGP_NN) then
          call gas_optics_ifs_rrtmgp_GPU(ncol,nlev,istartcol,iendcol,config, &
               &  single_level, thermodynamics, gas, &
               &  od_lw, od_sw, ssa_sw, lw_albedo=lw_albedo, &
               &  planck_hl=planck_hl, lw_emission=lw_emission, &
               &  incoming_sw=incoming_sw, lacc=lacc)
        ...
        if (config%i_gas_model_sw == IGasModelECCKD &
          ...
          call gas_optics_ecckd_GPU(ncol,nlev,istartcol,iendcol, config, &
               &  single_level, thermodynamics, gas, &
               &  od_lw, od_sw, ssa_sw, lw_albedo=lw_albedo, &
               &  planck_hl=planck_hl, lw_emission=lw_emission, &
               &  incoming_sw=incoming_sw, lacc=lacc)
      ...
      if (config%do_clouds) then
        ...
        call cloud%crop_cloud_fraction_GPU(istartcol, iendcol, &
             &            config%cloud_fraction_threshold, &
             &            config%cloud_mixing_ratio_threshold, lacc=lacc)

        ! Compute hydrometeor absorption/scattering properties in each
        ! shortwave and longwave band
        if (config%i_gas_model_sw == IGasModelMonochromatic) then
          call cloud_optics_mono_GPU(nlev, istartcol, iendcol, &
               &  config, thermodynamics, cloud, &
               &  od_lw_cloud, ssa_lw_cloud, g_lw_cloud, &
               &  od_sw_cloud, ssa_sw_cloud, g_sw_cloud, lacc=lacc)
        elseif (config%use_general_cloud_optics) then
          call general_cloud_optics_GPU(nlev, istartcol, iendcol, &
               &  config, thermodynamics, cloud, &
               &  od_lw_cloud, ssa_lw_cloud, g_lw_cloud, &
               &  od_sw_cloud, ssa_sw_cloud, g_sw_cloud, lacc=lacc)
        else
          call cloud_optics_GPU(nlev, istartcol, iendcol, &
               &  config, thermodynamics, cloud, &
               &  od_lw_cloud, ssa_lw_cloud, g_lw_cloud, &
               &  od_sw_cloud, ssa_sw_cloud, g_sw_cloud, lacc=lacc)
        end if
...
!               &  od_lw, ssa_lw, g_lw, od_sw, ssa_sw, g_sw)
          ...
          call add_aerosol_optics_GPU(config%n_g_sw, config%n_g_lw, &
               &  config%n_bands_sw, config%n_bands_lw,  &
               &  nlev,istartcol,iendcol, &
               &  config, thermodynamics, gas, aerosol, &
               &  od_lw, ssa_lw, g_lw, od_sw, ssa_sw, g_sw, lacc=lacc)
        end if
      else
      ...
      if (config%do_save_radiative_properties) then
        if (istartcol == 1 .and. iendcol == ncol) then
          ...
          !$ACC             incoming_sw, od_sw, ssa_sw, g_sw, od_sw_cloud, ssa_sw_cloud, g_sw_cloud) IF(lacc)
          call cloud%update_host_GPU(lacc=lacc)
          call flux%update_host_GPU(lacc=lacc)
        call save_radiative_properties_GPU(trim(rad_prop_file_name), &
             &  nlev, istartcol, iendcol, &
      ...
      if (config%do_lw) then
        ...
        if (config%i_solver_lw == ISolverMcICA) then
          !$ACC UPDATE HOST(od_lw, ssa_lw, g_lw, od_lw_cloud, ssa_lw_cloud, g_lw_cloud, planck_hl, lw_emission, lw_albedo) IF(lacc)
          !$ACC WAIT(1) IF(lacc)
          call cloud%update_host_GPU(lacc=lacc)
          call flux%update_host_GPU(lacc=lacc)
          !$ACC WAIT(1) IF(lacc)
          ! Compute fluxes using the McICA longwave solver
          call solver_mcica_lw_GPU(nlev,istartcol,iendcol, &
               &  config, single_level, cloud, &
               &  od_lw, ssa_lw, g_lw, od_lw_cloud, ssa_lw_cloud, &
               &  g_lw_cloud, planck_hl, lw_emission, lw_albedo, flux, lacc=lacc)
          !$ACC WAIT(1) IF(lacc)
          call flux%update_device_GPU(lacc=lacc)
          !$ACC WAIT(1) IF(lacc)
        else if (config%i_solver_lw == ISolverMcICAACC) then
          ! Compute fluxes using the McICA ACC longwave solver
          call solver_mcica_acc_lw_GPU(nlev,istartcol,iendcol, &
               &  config, single_level, cloud, &
               &  od_lw, ssa_lw, g_lw, od_lw_cloud, ssa_lw_cloud, &
               &  g_lw_cloud, planck_hl, lw_emission, lw_albedo, flux, lacc=lacc)
        else if (config%i_solver_lw == ISolverSPARTACUS) then
          ! Compute fluxes using the SPARTACUS longwave solver
          call solver_spartacus_lw_GPU(config%n_g_lw, nlev,istartcol,iendcol, &
               &  config, thermodynamics, cloud, &
               &  od_lw, ssa_lw, g_lw, od_lw_cloud, ssa_lw_cloud, g_lw_cloud, &
               &  planck_hl, lw_emission, lw_albedo, flux, lacc=lacc)
        else if (config%i_solver_lw == ISolverTripleclouds) then
          ! Compute fluxes using the Tripleclouds longwave solver
          call solver_tripleclouds_lw_GPU(config%n_g_lw,nlev,istartcol,iendcol, &
               &  config, cloud, &
               &  od_lw, ssa_lw, g_lw, od_lw_cloud, ssa_lw_cloud, g_lw_cloud, &
               &  planck_hl, lw_emission, lw_albedo, flux, lacc=lacc)
        elseif (config%i_solver_lw == ISolverHomogeneous) then
          ! Compute fluxes using the homogeneous solver
          call solver_homogeneous_lw_GPU(nlev,istartcol,iendcol, &
               &  config, cloud, &
               &  od_lw, ssa_lw, g_lw, od_lw_cloud, ssa_lw_cloud, &
               &  g_lw_cloud, planck_hl, lw_emission, lw_albedo, flux, lacc=lacc)
        else
          ! Compute fluxes using the cloudless solver
          call solver_cloudless_lw_GPU(nlev,istartcol,iendcol, &
      ...
      if (config%do_sw) then
        ...
        if (config%i_solver_sw == ISolverMcICA) then
          !$ACC UPDATE HOST(od_sw, ssa_sw, g_sw, od_sw_cloud, ssa_sw_cloud, g_sw_cloud, sw_albedo_direct, sw_albedo_diffuse, incoming_sw) IF(lacc)
          !$ACC WAIT(1) IF(lacc)
          call cloud%update_host_GPU(lacc=lacc)
          call flux%update_host_GPU(lacc=lacc)
          !$ACC WAIT(1) IF(lacc)
          ! Compute fluxes using the McICA shortwave solver
          call solver_mcica_sw_GPU(nlev,istartcol,iendcol, &
               &  config, single_level, cloud, &
               &  od_sw, ssa_sw, g_sw, od_sw_cloud, ssa_sw_cloud, &
               &  g_sw_cloud, sw_albedo_direct, sw_albedo_diffuse, &
               &  incoming_sw, flux, lacc=lacc)
          !$ACC WAIT(1) IF(lacc)
          call flux%update_device_GPU(lacc=lacc)
          !$ACC WAIT(1) IF(lacc)
        else if (config%i_solver_sw == ISolverMcICAACC) then
          ! Compute fluxes using the McICA ACC shortwave solver
          call solver_mcica_acc_sw_GPU(nlev,istartcol,iendcol, &
               &  config, single_level, cloud, &
               &  od_sw, ssa_sw, g_sw, od_sw_cloud, ssa_sw_cloud, &
               &  g_sw_cloud, sw_albedo_direct, sw_albedo_diffuse, &
               &  incoming_sw, flux, lacc=lacc)
          ...
          call solver_spartacus_sw_GPU(config%n_g_sw,nlev,istartcol,iendcol, &
               &  config, single_level, thermodynamics, cloud, &
               &  od_sw, ssa_sw, g_sw, od_sw_cloud, ssa_sw_cloud, &
               &  g_sw_cloud, sw_albedo_direct, sw_albedo_diffuse, &
               &  incoming_sw, flux, lacc=lacc)
          ...
          call solver_tripleclouds_sw_GPU(config%n_g_sw,nlev,istartcol,iendcol, &
               &  config, single_level, cloud, &
               &  od_sw, ssa_sw, g_sw, od_sw_cloud, ssa_sw_cloud, &
               &  g_sw_cloud, sw_albedo_direct, sw_albedo_diffuse, &
               &  incoming_sw, flux, lacc=lacc)
        elseif (config%i_solver_sw == ISolverHomogeneous) then
          ! Compute fluxes using the homogeneous solver
          call solver_homogeneous_sw_GPU(nlev,istartcol,iendcol, &
               &  config, single_level, cloud, &
               &  od_sw, ssa_sw, g_sw, od_sw_cloud, ssa_sw_cloud, &
               &  g_sw_cloud, sw_albedo_direct, sw_albedo_diffuse, &
               &  incoming_sw, flux, lacc=lacc)
          ...
          ! Compute fluxes using the cloudless solver
          call solver_cloudless_sw_GPU(nlev,istartcol,iendcol, &
      ...
      ! fluxes in g points
      call flux%calc_surface_spectral_GPU(config, istartcol, iendcol, lacc=lacc)
      call flux%calc_toa_spectral_GPU    (config, istartcol, iendcol, lacc=lacc)
  ...
  subroutine radiation_reverse_GPU(ncol, nlev, istartcol, iendcol, config, &
    ...
    call thermodynamics_rev%allocate_GPU(ncol, nlev, lacc=lacc)
    call cloud_rev%allocate_GPU(ncol, nlev, lacc=lacc)
    call flux_rev%allocate_GPU(config, istartcol, iendcol, nlev, lacc=lacc)
    if (allocated(aerosol%mixing_ratio)) then
      istartlev = nlev + 1 - aerosol%iendlev
      iendlev   = nlev + 1 - aerosol%istartlev
      call aerosol_rev%allocate_GPU(ncol, istartlev, iendlev, &
           &                    config%n_aerosol_types, lacc=lacc)
    end if
    ...
    call gas%reverse_GPU(istartcol, iendcol, gas_rev, lacc=lacc)
    ...
    call radiation_GPU(ncol, nlev,istartcol,iendcol, &
         &  config, single_level, thermodynamics_rev, gas_rev, &
         &  cloud_rev, aerosol_rev, flux_rev, lacc=lacc)
    ...
    call thermodynamics_rev%deallocate_GPU(lacc=lacc)
    call gas_rev%deallocate_GPU(lacc=lacc)
    call cloud_rev%deallocate_GPU(lacc=lacc)
    call flux_rev%deallocate_GPU(lacc=lacc)
    if (allocated(aerosol%mixing_ratio)) then
      call aerosol_rev%deallocate_GPU(lacc=lacc)
    end if

  end subroutine radiation_reverse_GPU

  subroutine setup_radiation_CPU(config)
    ...
    ! Consolidate configuration data, including setting data file
    ! names
    call config%consolidate_CPU()

    ! Load the look-up tables from files in the specified directory
    if (config%i_gas_model_sw == IGasModelMonochromatic) then
      call setup_gas_optics_mono_CPU(config, trim(config%directory_name))
    else
      ! Note that we can run RRTMG and ECCKD for different parts of
      ! the spectrum: the setup routines only configure the relevant
      ! part.
      if (config%i_gas_model_sw == IGasModelIFSRRTMG .or. config%i_gas_model_lw == IGasModelIFSRRTMG) then
        call setup_gas_optics_rrtm_CPU(config, trim(config%directory_name))
      end if
      if (config%i_gas_model_sw == IGasModelECCKD .or. config%i_gas_model_lw == IGasModelECCKD) then
        call setup_gas_optics_ecckd_CPU(config)
      end if
      if (config%i_gas_model_sw == IGasModelRRTMGP .or. config%i_gas_model_lw == IGasModelRRTMGP .or. &
        & config%i_gas_model_sw == IGasModelRRTMGP_NN .or. config%i_gas_model_lw == IGasModelRRTMGP_NN) then
        call setup_gas_optics_ifs_rrtmgp_CPU(config)
      end if
    end if
    ...
    ! and longwave spectral bands
    if (config%do_sw) then
      call config%consolidate_sw_albedo_intervals_CPU
    end if
    if (config%do_lw) then
      call config%consolidate_lw_emiss_intervals_CPU
    end if
    ...
    if (config%do_clouds) then
      if (config%i_gas_model_sw == IGasModelMonochromatic) then
        !      call setup_cloud_optics_mono(config)
      elseif (config%use_general_cloud_optics) then
        call setup_general_cloud_optics_CPU(config)
      else
        call setup_cloud_optics_CPU(config)
      end if
    end if
...
!        call setup_aerosol_optics_mono(config)
        ...
        call setup_aerosol_optics_CPU(config)
      end if
    end if
    ...
    if (         config%i_solver_sw == ISolverMcICA &
         ...
         &  .or. config%i_solver_sw == ISolverMcICAACC &
         &  .or. config%i_solver_lw == ISolverMcICAACC ) then
      call config%pdf_sampler%setup_CPU(config%cloud_pdf_file_name, &
  ...
  subroutine set_gas_units_CPU(config, gas, lacc)
    ...
    if (config%i_gas_model_sw == IGasModelMonochromatic) then
      call set_gas_units_mono_CPU(gas)
    elseif (config%i_gas_model_sw == IGasModelIFSRRTMG &
         &  .or. config%i_gas_model_lw == IGasModelIFSRRTMG) then
      ! Convert to mass-mixing ratio for RRTMG; note that ecCKD can
      ! work with this but performs an internal scaling
      call set_gas_units_rrtm_CPU(gas, lacc=lacc)
    elseif (config%i_gas_model_sw == IGasModelRRTMGP &
         &  .or. config%i_gas_model_lw == IGasModelRRTMGP &
         &  .or. config%i_gas_model_sw == IGasModelRRTMGP_NN &
         &  .or. config%i_gas_model_lw == IGasModelRRTMGP_NN ) then
      call set_gas_units_rrtmgp_CPU(gas)
    else
      ! Use volume mixing ratio preferred by ecCKD
      call set_gas_units_ecckd_CPU(gas)
    end if
  ...
  subroutine radiation_CPU(ncol, nlev, istartcol, iendcol, config, &
    ...
    if (thermodynamics%pressure_hl(istartcol,2) &
      ...
      ! call the radiation scheme and then reverses the returned
      ! fluxes
      call radiation_reverse_CPU(ncol, nlev, istartcol, iendcol, config, &
           &  single_level, thermodynamics, gas, cloud, aerosol, flux)
    else
      ...
      ! Extract surface albedos at each gridpoint
      call single_level%get_albedos_CPU(istartcol, iendcol, config, &
           &                        sw_albedo_direct, sw_albedo_diffuse, &
           &                        lw_albedo)
      ...
      ! range of atmospheric columns
      if (config%i_gas_model_sw == IGasModelMonochromatic) then
        call gas_optics_mono_CPU(ncol,nlev,istartcol,iendcol, config, &
             &  single_level, thermodynamics, gas, lw_albedo, &
             &  od_lw, od_sw, ssa_sw, &
        ...
        if (config%i_gas_model_sw == IGasModelIFSRRTMG &
             &   .or. config%i_gas_model_lw == IGasModelIFSRRTMG) then
          call gas_optics_rrtm_CPU(ncol,nlev,istartcol,iendcol, config, &
               &  single_level, thermodynamics, gas, &
               &  od_lw, od_sw, ssa_sw, lw_albedo=lw_albedo, &
               &  planck_hl=planck_hl, lw_emission=lw_emission, &
        ...
        if (config%i_gas_model_sw == IGasModelRRTMGP &
             ...
             &   .or. config%i_gas_model_sw == IGasModelRRTMGP_NN &
             &   .or. config%i_gas_model_lw == IGasModelRRTMGP_NN) then
          call gas_optics_ifs_rrtmgp_CPU(ncol,nlev,istartcol,iendcol,config, &
               &  single_level, thermodynamics, gas, &
               &  od_lw, od_sw, ssa_sw, lw_albedo=lw_albedo, &
               &  planck_hl=planck_hl, lw_emission=lw_emission, &
               &  incoming_sw=incoming_sw)
        ...
        if (config%i_gas_model_sw == IGasModelECCKD &
             &   .or. config%i_gas_model_lw == IGasModelECCKD) then
          call gas_optics_ecckd_CPU(ncol,nlev,istartcol,iendcol, config, &
               &  single_level, thermodynamics, gas, &
               &  od_lw, od_sw, ssa_sw, lw_albedo=lw_albedo, &
               &  planck_hl=planck_hl, lw_emission=lw_emission, &
               &  incoming_sw=incoming_sw)
      ...
      if (config%do_clouds) then
        ...
        ! assume that a cloud is present if cloud%fraction > 0.0.
        ! WARNING: not 100% tested on GPU as it has no effect on result
        call cloud%crop_cloud_fraction_CPU(istartcol, iendcol, &
             &            config%cloud_fraction_threshold, &
        ...
        ! shortwave and longwave band
        if (config%i_gas_model_sw == IGasModelMonochromatic) then
          call cloud_optics_mono_CPU(nlev, istartcol, iendcol, &
               &  config, thermodynamics, cloud, &
               &  od_lw_cloud, ssa_lw_cloud, g_lw_cloud, &
               &  od_sw_cloud, ssa_sw_cloud, g_sw_cloud)
        elseif (config%use_general_cloud_optics) then
          call general_cloud_optics_CPU(nlev, istartcol, iendcol, &
               &  config, thermodynamics, cloud, &
               &  od_lw_cloud, ssa_lw_cloud, g_lw_cloud, &
               &  od_sw_cloud, ssa_sw_cloud, g_sw_cloud)
        else
          call cloud_optics_CPU(nlev, istartcol, iendcol, &
               &  config, thermodynamics, cloud, &
               &  od_lw_cloud, ssa_lw_cloud, g_lw_cloud, &
               &  od_sw_cloud, ssa_sw_cloud, g_sw_cloud)
        end if
...
!               &  od_lw, ssa_lw, g_lw, od_sw, ssa_sw, g_sw)
          ...
          call add_aerosol_optics_CPU(config%n_g_sw, config%n_g_lw, &
               &  config%n_bands_sw, config%n_bands_lw,  &
               &  nlev,istartcol,iendcol, &
               &  config, thermodynamics, gas, aerosol, &
               &  od_lw, ssa_lw, g_lw, od_sw, ssa_sw, g_sw)
      ...
      if (config%do_save_radiative_properties) then
        ...
        call save_radiative_properties_CPU(trim(rad_prop_file_name), &
             &  nlev, istartcol, iendcol, &
             &  config, single_level, thermodynamics, cloud, &
             &  planck_hl, lw_emission, lw_albedo, &
             &  sw_albedo_direct, sw_albedo_diffuse, incoming_sw, &
      ...
      if (config%do_lw) then
        ...
        if (config%i_solver_lw == ISolverMcICA) then
          ! Compute fluxes using the McICA longwave solver
          call solver_mcica_lw_CPU(nlev,istartcol,iendcol, &
               &  config, single_level, cloud, &
               &  od_lw, ssa_lw, g_lw, od_lw_cloud, ssa_lw_cloud, &
               &  g_lw_cloud, planck_hl, lw_emission, lw_albedo, flux)
        else if (config%i_solver_lw == ISolverMcICAACC) then
          ! Compute fluxes using the McICA ACC longwave solver
          call solver_mcica_acc_lw_CPU(nlev,istartcol,iendcol, &
               &  config, single_level, cloud, &
               &  od_lw, ssa_lw, g_lw, od_lw_cloud, ssa_lw_cloud, &
               &  g_lw_cloud, planck_hl, lw_emission, lw_albedo, flux)
        else if (config%i_solver_lw == ISolverSPARTACUS) then
          ! Compute fluxes using the SPARTACUS longwave solver
          call solver_spartacus_lw_CPU(config%n_g_lw, nlev,istartcol,iendcol, &
               &  config, thermodynamics, cloud, &
               &  od_lw, ssa_lw, g_lw, od_lw_cloud, ssa_lw_cloud, g_lw_cloud, &
               &  planck_hl, lw_emission, lw_albedo, flux)
        else if (config%i_solver_lw == ISolverTripleclouds) then
          ! Compute fluxes using the Tripleclouds longwave solver
          call solver_tripleclouds_lw_CPU(config%n_g_lw,nlev,istartcol,iendcol, &
               &  config, cloud, &
               &  od_lw, ssa_lw, g_lw, od_lw_cloud, ssa_lw_cloud, g_lw_cloud, &
               &  planck_hl, lw_emission, lw_albedo, flux)
        elseif (config%i_solver_lw == ISolverHomogeneous) then
          ! Compute fluxes using the homogeneous solver
          call solver_homogeneous_lw_CPU(nlev,istartcol,iendcol, &
               &  config, cloud, &
               &  od_lw, ssa_lw, g_lw, od_lw_cloud, ssa_lw_cloud, &
               &  g_lw_cloud, planck_hl, lw_emission, lw_albedo, flux)
        else
          ! Compute fluxes using the cloudless solver
          call solver_cloudless_lw_CPU(nlev,istartcol,iendcol, &
               &  config, od_lw, ssa_lw, g_lw, &
               &  planck_hl, lw_emission, lw_albedo, flux)
        end if
      end if
      ...
      if (config%do_sw) then
        ...
        if (config%i_solver_sw == ISolverMcICA) then
          ! Compute fluxes using the McICA shortwave solver
          call solver_mcica_sw_CPU(nlev,istartcol,iendcol, &
               &  config, single_level, cloud, &
               &  od_sw, ssa_sw, g_sw, od_sw_cloud, ssa_sw_cloud, &
               &  g_sw_cloud, sw_albedo_direct, sw_albedo_diffuse, &
               &  incoming_sw, flux)
          ...
          ! Compute fluxes using the McICA ACC shortwave solver
          call solver_mcica_acc_sw_CPU(nlev,istartcol,iendcol, &
               &  config, single_level, cloud, &
               &  od_sw, ssa_sw, g_sw, od_sw_cloud, ssa_sw_cloud, &
               &  g_sw_cloud, sw_albedo_direct, sw_albedo_diffuse, &
               &  incoming_sw, flux)
          ...
          ! Compute fluxes using the SPARTACUS shortwave solver
          call solver_spartacus_sw_CPU(config%n_g_sw,nlev,istartcol,iendcol, &
               &  config, single_level, thermodynamics, cloud, &
               &  od_sw, ssa_sw, g_sw, od_sw_cloud, ssa_sw_cloud, &
               &  g_sw_cloud, sw_albedo_direct, sw_albedo_diffuse, &
               &  incoming_sw, flux)
          ...
          ! Compute fluxes using the Tripleclouds shortwave solver
          call solver_tripleclouds_sw_CPU(config%n_g_sw,nlev,istartcol,iendcol, &
               &  config, single_level, cloud, &
               &  od_sw, ssa_sw, g_sw, od_sw_cloud, ssa_sw_cloud, &
               &  g_sw_cloud, sw_albedo_direct, sw_albedo_diffuse, &
               &  incoming_sw, flux)
        elseif (config%i_solver_sw == ISolverHomogeneous) then
          ! Compute fluxes using the homogeneous solver
          call solver_homogeneous_sw_CPU(nlev,istartcol,iendcol, &
               &  config, single_level, cloud, &
               &  od_sw, ssa_sw, g_sw, od_sw_cloud, ssa_sw_cloud, &
               &  g_sw_cloud, sw_albedo_direct, sw_albedo_diffuse, &
               &  incoming_sw, flux)
          ...
          ! Compute fluxes using the cloudless solver
          call solver_cloudless_sw_CPU(nlev,istartcol,iendcol, &
               &  config, single_level, od_sw, ssa_sw, g_sw, &
               &  sw_albedo_direct, sw_albedo_diffuse, &
               &  incoming_sw, flux)
        end if
      ...
      ! Store surface downwelling, and TOA, fluxes in bands from
      ! fluxes in g points
      call flux%calc_surface_spectral_CPU(config, istartcol, iendcol)
      call flux%calc_toa_spectral_CPU    (config, istartcol, iendcol)
  ...
  subroutine radiation_reverse_CPU(ncol, nlev, istartcol, iendcol, config, &
    ...
    ! Allocate reversed arrays
    call thermodynamics_rev%allocate_CPU(ncol, nlev)
    call cloud_rev%allocate_CPU(ncol, nlev)
    call flux_rev%allocate_CPU(config, istartcol, iendcol, nlev)
    if (allocated(aerosol%mixing_ratio)) then
      istartlev = nlev + 1 - aerosol%iendlev
      iendlev   = nlev + 1 - aerosol%istartlev
      call aerosol_rev%allocate_CPU(ncol, istartlev, iendlev, &
    ...
    ! Fill reversed gas arrays
    call gas%reverse_CPU(istartcol, iendcol, gas_rev)
    ...
    ! Run radiation scheme on reversed profiles
    call radiation_CPU(ncol, nlev,istartcol,iendcol, &
         &  config, single_level, thermodynamics_rev, gas_rev, &
    ...
    ! Deallocate reversed arrays
    call thermodynamics_rev%deallocate_CPU
    call gas_rev%deallocate_CPU
    call cloud_rev%deallocate_CPU
    call flux_rev%deallocate_CPU
    if (allocated(aerosol%mixing_ratio)) then
      call aerosol_rev%deallocate_CPU
    end if
