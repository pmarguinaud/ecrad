...
contains
  ...
  subroutine solver_mcica_acc_lw_GPU(nlev,istartcol,iendcol, &
    ...
    !$ACC             g_cloud, planck_hl, emission, albedo, flux) ASYNC(1) IF(lacc)
    ...
    do jcol = istartcol,iendcol
      ...
      do jlev = 1,nlev-1
        ...
        if (config%use_beta_overlap) then
          overlap_alpha = beta2alpha_GPU(overlap_param(jlev,jcol), &
    ...
    !$ACC   NUM_GANGS(iendcol-istartcol+1) NUM_WORKERS((config%n_g_lw-1)/32+1) VECTOR_LENGTH(32) IF(lacc)
    ...
    !$ACC   trans_clear, transmittance)
      ...
      ! Clear-sky calculation
        ...
        call calc_no_scattering_transmittance_lw_GPU(ng*nlev, od(:,:,jcol), &
               &  planck_hl(:,1:nlev,jcol), planck_hl(:,2:nlev+1, jcol), &
               &  trans_clear, source_up_clear, &
               &  source_dn_clear)
        ! Simpler down-then-up method to compute fluxes
        call calc_fluxes_no_scattering_lw_GPU(ng, nlev, &
      ...
      ! the longwave
      call cloud_generator_acc_GPU(ng, nlev, &
        ...
        do jlev = 1,nlev
          ...
          if (frac(jlev,jcol) >= config%cloud_fraction_threshold) then
            ...
            if (config%do_lw_cloud_scattering) then
              ...
              ! transmission and emission
              call calc_no_scattering_transmittance_lw_GPU(ng, od_total, &
                   &  planck_hl(:,jlev,jcol), planck_hl(:,jlev+1, jcol), &
...
!               &  flux_up(:,:,jcol), flux_dn(:,:,jcol))
          call fast_adding_ica_lw_GPU(ng, nlev, reflectance, transmittance, source_up, source_dn, &
               &  emission(:,jcol), albedo(:,jcol), &
        ...
        if (config%do_lw_derivatives) then
          call calc_lw_derivatives_ica_GPU(ng, nlev, jcol, transmittance, flux_up(:,nlev+1,jcol), &
               &                       flux%lw_derivatives)
          if (total_cloud_cover < 1.0_jprb - config%cloud_fraction_threshold) then
            ...
            call modify_lw_derivatives_ica_GPU(ng, nlev, jcol, trans_clear, flux_up_clear(:,nlev+1,jcol), &
        ...
        if (config%do_lw_derivatives) then
          call calc_lw_derivatives_ica_GPU(ng, nlev, jcol, trans_clear, flux_up_clear(:,nlev+1,jcol), &
               &                       flux%lw_derivatives)
    ...
    ! Loop through columns
    !$ACC PARALLEL DEFAULT(NONE) ASYNC(1) &
    !$ACC   NUM_GANGS((iendcol-istartcol+1)*(nlev+1)) NUM_WORKERS(1) VECTOR_LENGTH(32*((config%n_g_lw-1)/32+1)) IF(lacc)
  ...
  subroutine solver_mcica_acc_lw_CPU(nlev,istartcol,iendcol, &
    ...
    do jcol = istartcol,iendcol
      ...
      do jlev = 1,nlev-1
        ! Convert to "alpha" overlap parameter if necessary
        if (config%use_beta_overlap) then
          overlap_alpha = beta2alpha_CPU(overlap_param(jlev,jcol), &
                &                     frac(jlev,jcol), frac(jlev+1,jcol))
        else
          overlap_alpha = overlap_param(jlev,jcol)
        end if
    ...
    ! Loop through columns
      ...
      if (config%do_lw_aerosol_scattering) then
        ...
        call calc_ref_trans_lw_CPU(ng*nlev, &
        ...
        call adding_ica_lw_CPU(ng, nlev, &
        ...
        ! Non-scattering case: use simpler functions for
        ! transmission and emission
        call calc_no_scattering_transmittance_lw_CPU(ng*nlev, od(:,:,jcol), &
               &  planck_hl(:,1:nlev,jcol), planck_hl(:,2:nlev+1, jcol), &
               &  trans_clear, source_up_clear, &
               &  source_dn_clear)
        ! Simpler down-then-up method to compute fluxes
        call calc_fluxes_no_scattering_lw_CPU(ng, nlev, &
             &  trans_clear, source_up_clear, source_dn_clear, &
             &  emission(:,jcol), albedo(:,jcol), &
             &  flux_up_clear(:,:,jcol), flux_dn_clear(:,:,jcol))
      ...
      ! Do cloudy-sky calculation; add a prime number to the seed in
      ! the longwave
      call cloud_generator_acc_CPU(ng, nlev, &
           &  single_level%iseed(jcol) + 997, &
           &  config%cloud_fraction_threshold, &
           &  frac(:,jcol), overlap_param(:,jcol), &
           &  config%cloud_inhom_decorr_scaling, frac_std(:,jcol), &
      ...
      if (total_cloud_cover >= config%cloud_fraction_threshold) then
        ...
        do jlev = 1,nlev
          ...
          if (frac(jlev,jcol) >= config%cloud_fraction_threshold) then
            ...
            if (config%do_lw_cloud_scattering) then
              ...
              ! No-scattering case: use simpler functions for
              ! transmission and emission
              call calc_no_scattering_transmittance_lw_CPU(ng, od_total, &
                   &  planck_hl(:,jlev,jcol), planck_hl(:,jlev+1, jcol), &
                   &  transmittance(:,jlev), source_up(:,jlev), &
                   &  source_dn(:,jlev))
            end if
        ...
        if (config%do_lw_aerosol_scattering) then
          ...
          call adding_ica_lw_CPU(ng, nlev, reflectance, transmittance, source_up, source_dn, &
               &  emission(:,jcol), albedo(:,jcol), &
               &  flux_up(:,:,jcol), flux_dn(:,:,jcol))
...
!               &  emission(:,jcol), albedo(:,jcol), &
!               &  flux_up(:,:,jcol), flux_dn(:,:,jcol))
          call fast_adding_ica_lw_CPU(ng, nlev, reflectance, transmittance, source_up, source_dn, &
               &  emission(:,jcol), albedo(:,jcol), &
               &  is_clear_sky_layer(:,jcol), i_cloud_top, flux_dn_clear(:,:,jcol), &
               &  flux_up(:,:,jcol), flux_dn(:,:,jcol), &
               &  albedo=tmp_work_albedo, &
        ...
        ! (2015) approximate radiation update scheme
        if (config%do_lw_derivatives) then
          call calc_lw_derivatives_ica_CPU(ng, nlev, jcol, transmittance, flux_up(:,nlev+1,jcol), &
               &                       flux%lw_derivatives)
          if (total_cloud_cover < 1.0_jprb - config%cloud_fraction_threshold) then
            ! Modify the existing derivative with the contribution from the clear sky
            call modify_lw_derivatives_ica_CPU(ng, nlev, jcol, trans_clear, flux_up_clear(:,nlev+1,jcol), &
                 &                         1.0_jprb-total_cloud_cover, flux%lw_derivatives)
          end if
        end if
        ...
        if (config%do_lw_derivatives) then
          call calc_lw_derivatives_ica_CPU(ng, nlev, jcol, trans_clear, flux_up_clear(:,nlev+1,jcol), &
               &                       flux%lw_derivatives)
