...
#include "radiation_delta_eddington.h"
  ...
  subroutine solver_mcica_acc_sw_GPU(nlev,istartcol,iendcol, &
    ...
    !$ACC             incoming_sw, flux) ASYNC(1) IF(lacc)
    ...
    !$ACC   NUM_GANGS(iendcol-istartcol+1) NUM_WORKERS((config%n_g_sw-1)/32+1) VECTOR_LENGTH(32) IF(lacc)
    ...
    !$ACC   trans_dir_dir, trans_dir_dir_clear, transmittance, total_cloud_cover)
      ...
      if (single_level%cos_sza(jcol) > 0.0_jprb) then
        ...
        if (.not. config%do_sw_delta_scaling_with_gases) then
          ...
          call calc_ref_trans_sw_GPU(ng*nlev, &
    ...
    !$ACC   NUM_GANGS((iendcol-istartcol+1)*(nlev+1)) NUM_WORKERS(1) VECTOR_LENGTH(32*((config%n_g_sw-1)/32+1)) IF(lacc)
    ...
    ! Loop through columns
      ...
      if (single_level%cos_sza(jcol) > 0.0_jprb) then
        ...
        if (.not. config%do_sw_delta_scaling_with_gases) then
          ! Delta-Eddington scaling has already been performed to the
          ! aerosol part of od, ssa and g
          call calc_ref_trans_sw_CPU(ng*nlev, &
            &  cos_sza, od(:,:,jcol), ssa(:,:,jcol), g(:,:,jcol), &
            &  ref_clear, trans_clear, &
            &  ref_dir_clear, trans_dir_diff_clear, &
            &  trans_dir_dir_clear)
