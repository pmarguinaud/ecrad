...
contains
  ...
  subroutine solver_mcica_lw_GPU(nlev,istartcol,iendcol, &
    ...
    do jcol = istartcol,iendcol
      ...
      if (config%do_lw_aerosol_scattering) then
        ...
        call calc_ref_trans_lw_GPU(ng*nlev, &
        ...
        call adding_ica_lw_GPU(ng, nlev, &
      ...
      ! the longwave
      call cloud_generator_GPU(ng, nlev, config%i_overlap_scheme, &
           ...
           &  use_vectorizable_generator=config%use_vectorizable_generator, lacc=lacc)
      ...
      if (total_cloud_cover >= config%cloud_fraction_threshold) then
        ...
        if (config%do_lw_aerosol_scattering) then
          ...
          call adding_ica_lw_GPU(ng, nlev, reflectance, transmittance, source_up, source_dn, &
               &  emission(:,jcol), albedo(:,jcol), &
               &  flux_up, flux_dn, lacc=lacc)
        ...
        if (config%do_lw_derivatives) then
          call calc_lw_derivatives_ica_GPU(ng, nlev, jcol, transmittance, flux_up(:,nlev+1), &
               &                       flux%lw_derivatives)
          if (total_cloud_cover < 1.0_jprb - config%cloud_fraction_threshold) then
            ...
            call modify_lw_derivatives_ica_GPU(ng, nlev, jcol, trans_clear, flux_up_clear(:,nlev+1), &
        ...
        if (config%do_lw_derivatives) then
          call calc_lw_derivatives_ica_GPU(ng, nlev, jcol, trans_clear, flux_up_clear(:,nlev+1), &
               &                       flux%lw_derivatives)
  ...
  subroutine solver_mcica_lw_CPU(nlev,istartcol,iendcol, &
    ...
    do jcol = istartcol,iendcol
      ...
      ! Do cloudy-sky calculation; add a prime number to the seed in
      ! the longwave
      call cloud_generator_CPU(ng, nlev, config%i_overlap_scheme, &
           &  single_level%iseed(jcol) + 997, &
           &  config%cloud_fraction_threshold, &
           &  cloud%fraction(jcol,:), cloud%overlap_param(jcol,:), &
           &  config%cloud_inhom_decorr_scaling, cloud%fractional_std(jcol,:), &
      ...
      if (total_cloud_cover >= config%cloud_fraction_threshold) then
        ...
        ! (2015) approximate radiation update scheme
        if (config%do_lw_derivatives) then
          call calc_lw_derivatives_ica_CPU(ng, nlev, jcol, transmittance, flux_up(:,nlev+1), &
               &                       flux%lw_derivatives)
          if (total_cloud_cover < 1.0_jprb - config%cloud_fraction_threshold) then
            ! Modify the existing derivative with the contribution from the clear sky
            call modify_lw_derivatives_ica_CPU(ng, nlev, jcol, trans_clear, flux_up_clear(:,nlev+1), &
                 &                         1.0_jprb-total_cloud_cover, flux%lw_derivatives)
          end if
        end if
        ...
        flux%lw_dn_surf_g(:,jcol) = flux%lw_dn_surf_clear_g(:,jcol)
        if (config%do_lw_derivatives) then
          call calc_lw_derivatives_ica_CPU(ng, nlev, jcol, trans_clear, flux_up_clear(:,nlev+1), &
               &                       flux%lw_derivatives)
