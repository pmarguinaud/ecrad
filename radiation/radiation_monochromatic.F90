...
#include "radiation_delta_eddington.h"
  ...
  subroutine gas_optics_mono_GPU(ncol,nlev,istartcol,iendcol, &
    ...
    if (single_level%is_simple_surface) then
      if (config%mono_lw_wavelength <= 0.0_jprb) then
        ...
        lw_emission(1,istartcol:iendcol) = Pi*planck_function_GPU(config%mono_lw_wavelength, &
             &             single_level%skin_temperature(istartcol:iendcol), lacc) &
             &  * single_level%lw_emissivity(istartcol:iendcol,1)
        do jlev = 1,nlev+1
          planck_hl(1,jlev,istartcol:iendcol) = Pi*planck_function_GPU(config%mono_lw_wavelength, &
               &             thermodynamics%temperature_hl(istartcol:iendcol,jlev), lacc)
        end do
      end if
    else
  ...
  elemental function planck_function_GPU(wavelength, temperature, lacc)
    ...
    if (temperature > 0.0_jprb) then
      planck_function_GPU = 2.0_jprb * PlanckConstant * SpeedOfLight**2 &
           &   / (wavelength**5 &
           &   * (exp(PlanckConstant*SpeedOfLight &
           &         /(wavelength*BoltzmannConstant*temperature)) - 1.0_jprb))
    else
      planck_function_GPU = 0.0_jprb
    end if
  ...
  subroutine gas_optics_mono_CPU(ncol,nlev,istartcol,iendcol, &
    ...
    if (single_level%is_simple_surface) then
      if (config%mono_lw_wavelength <= 0.0_jprb) then
        ...
        ! Single wavelength: multiply by pi to convert W sr-1 m-3 to W m-3
        lw_emission(1,istartcol:iendcol) = Pi*planck_function_CPU(config%mono_lw_wavelength, &
             &             single_level%skin_temperature(istartcol:iendcol)) &
             &  * single_level%lw_emissivity(istartcol:iendcol,1)
        do jlev = 1,nlev+1
          planck_hl(1,jlev,istartcol:iendcol) = Pi*planck_function_CPU(config%mono_lw_wavelength, &
  ...
  elemental function planck_function_CPU(wavelength, temperature)
    ...
    if (temperature > 0.0_jprb) then
      planck_function_CPU = 2.0_jprb * PlanckConstant * SpeedOfLight**2 &
           &   / (wavelength**5 &
           &   * (exp(PlanckConstant*SpeedOfLight &
           &         /(wavelength*BoltzmannConstant*temperature)) - 1.0_jprb))
    else
      planck_function_CPU = 0.0_jprb
    end if
