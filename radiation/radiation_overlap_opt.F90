...
!   2022-09-08 P. Ukkonen  Faster overlap computations by removing column dimension
!                          to reduce memory use, making nreg a constant, etc.
module radiation_overlap_opt

  implicit none
  private
  public :: calc_overlap_matrices_nocol, calc_overlap_matrices_nocol_dp
...
contains
  ...
  pure function calc_beta_overlap_matrix_dp(nreg, op, frac_upper, frac_lower, &
       &  frac_threshold) result(overlap_matrix)
  ...
  pure function calc_alpha_overlap_matrix_dp(nreg, op, op_inhom, &
       &  frac_upper, frac_lower) result(overlap_matrix)
  ...
  subroutine calc_overlap_matrices_nocol(nlev,nreg_in, &
    ...
    do jlev = 1,nlev+1
      ...
      if (jlev == 1 .or. jlev > nlev) then
        ...
        if (op(1) >= 0.0_jprb) then
          if (used_decorrelation_scaling==0.5_jprb) then 
            op(2:nreg) = op(1)*op(1)
          else
  ...
  ! Double precision version for SPARTACUS-SW
  subroutine calc_overlap_matrices_nocol_dp(nlev,nreg, &
    &     region_fracs, overlap_param, v_matrix, u_matrix, decorrelation_scaling, &
    &     cloud_fraction_threshold, cloud_cover, use_beta_overlap)
  ...
  end subroutine calc_overlap_matrices_nocol_dp
  
end module radiation_overlap_opt
