...
contains
  ...
  subroutine save_inputs_GPU(file_name, config, single_level, thermodynamics, &
    ...
    call gas%get_GPU(IH2O, IMassMixingRatio, mixing_ratio, lacc=lacc)
    call out_file%put("q", mixing_ratio)
    call gas%get_GPU(IO3, IMassMixingRatio, mixing_ratio, lacc=lacc)
    call out_file%put("o3_mmr", mixing_ratio)
    do jgas = 1,NMaxGases
      if (gas%is_present(jgas) .and. jgas /= IH2O .and. jgas /= IO3) then
        write(var_name,'(a,a)') trim(GasLowerCaseName(jgas)), '_vmr'
        call gas%get_GPU(jgas, IVolumeMixingRatio, mixing_ratio, lacc=lacc)
  ...
  subroutine save_inputs_CPU(file_name, config, single_level, thermodynamics, &
    ...
    allocate(mixing_ratio(ncol,nlev))
    call gas%get_CPU(IH2O, IMassMixingRatio, mixing_ratio)
    call out_file%put("q", mixing_ratio)
    call gas%get_CPU(IO3, IMassMixingRatio, mixing_ratio)
    call out_file%put("o3_mmr", mixing_ratio)
    do jgas = 1,NMaxGases
      if (gas%is_present(jgas) .and. jgas /= IH2O .and. jgas /= IO3) then
        write(var_name,'(a,a)') trim(GasLowerCaseName(jgas)), '_vmr'
        call gas%get_CPU(jgas, IVolumeMixingRatio, mixing_ratio)
        call out_file%put(trim(var_name), mixing_ratio)
      end if
    end do
    deallocate(mixing_ratio)
