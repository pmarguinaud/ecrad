...
#include "radiation_optical_depth_scaling.h"
  ...
  subroutine solver_spartacus_lw_GPU(ng_lw_in, nlev,istartcol,iendcol, &
    ...
    ! real(jprb), dimension(ng_lw_in, 2:nreg,nlev) &
    !      &  :: od_region_cld, ssa_region_cld, g_region_cld
    real(jprb), dimension(ng_lw_in,nlev) :: od_region_clear

    real(jprb), dimension(:,:), contiguous, pointer   :: ssa_clear, g_clear
    real(jprb), dimension(ng_lw_in,nlev), target :: zero_array

    ! Scattering optical depths of gases and clouds
    real(jprb) :: scat_od(ng_lw_in), scat_od_cloud
    ...
    ! real(jprb), dimension(ng_lw_in, nreg-1, nlev) :: gamma1_cld, gamma2_cld
    real(jprb), dimension(ng_lw_in, nlev) :: gamma1_clear, gamma2_clear
    ...
    real(jprb), dimension(ng_lw_in, nreg, nreg, nlev) :: reflectance, transmittance

    ! Clear-sky diffuse reflection and transmission matrices of each
    ! layer
    real(jprb), dimension(ng_lw_in, nlev) :: ref_clear, trans_clear
    ...
    ! emission
    real(jprb), dimension(ng_lw_in, nreg, nlev) :: source_up, source_dn
    ! ...clear-sky equivalents
    real(jprb), dimension(ng_lw_in, nlev)       :: source_up_clear, source_dn_clear
    ...
    ! top-of-atmosphere
    real(jprb), dimension(ng_lw_in, nreg, nlev+1) :: total_source
    ! ...clear-sky equivalent
    real(jprb), dimension(ng_lw_in, nlev+1) :: total_source_clear

    ! As total_source, but just below a layer interface
    real(jprb), dimension(ng_lw_in, nreg) :: total_source_below
    ...
    ! top-of-atmosphere
    real(jprb), dimension(ng_lw_in, nreg, nreg, nlev+1) :: total_albedo
    ! ...clear-sky equivalent
    real(jprb), dimension(ng_lw_in, nlev+1) :: total_albedo_clear

    ! As total_albedo, but just below a layer interface
    real(jprb), dimension(ng_lw_in, nreg, nreg) :: total_albedo_below

    ! Temporary variables for albedo computations
    real(jprb), dimension(ng_lw_in, nreg, nreg) :: albedo_mat
    real(jprb), dimension(ng_lw_in, nreg)       :: albedo_part

    ! The following is used to store matrices of the form I-A*B that
    ! are used on the denominator of some expressions
    real(jprb), dimension(ng_lw_in, nreg, nreg) :: denominator
    ! Clear-sky equivalent, but actually its reciprocal to replace
    ! some divisions by multiplications
    real(jprb), dimension(ng_lw_in) :: inv_denom_scalar

    ! Layer depth (m)
    real(jprb) :: dz

    ! Upwelling and downwelling fluxes above/below layer interfaces
    real(jprb), dimension(ng_lw_in, nreg) &
    ...
    ! interface)
    real(jprb), dimension(ng_lw_in) :: flux_up_clear, flux_dn_clear
    ...
    real(jprb), dimension(ng_lw_in) :: side_emiss
    ...
    do jcol = istartcol, iendcol
      ...
      call calc_overlap_matrices_GPU(nlev, nreg, is_clear_sky_layer, &
        ...
        &  cloud_cov=flux%cloud_cover_lw(jcol), lacc=lacc)
      ...
      if (config%do_lw_aerosol_scattering) then
        call calc_ref_trans_lw_GPU(ng_lw_in*nlev, &
        ...
        call calc_no_scattering_transmittance_lw_GPU(ng_lw_in*nlev, &
            &  od(:,:, jcol), &
            &  planck_hl(:,1:nlev,jcol), planck_hl(:,2:nlev+1,jcol), &
            &  trans_clear, &
            &  source_up_clear, source_dn_clear)
      ...
      do while (are_clouds_below)
        ...
        allocate(od_region_cld(ng_lw_in,2:nreg,jtop:jbot), ssa_region_cld(ng_lw_in,2:nreg,jtop:jbot))
        allocate(g_region_cld (ng_lw_in,2:nreg,jtop:jbot))
        allocate(gamma1_cld(ng_lw_in,2:nreg, jtop:jbot), gamma2_cld(ng_lw_in,2:nreg, jtop:jbot))
        ...
        do jlev = jtop, jbot
          ...
          do jreg = 2,nreg
            ! Loop over g-points
            do jg = 1,ng_lw_in
              if (ng_lw_in == config%n_bands_lw) then
        ...
        call calc_two_stream_gammas_lw_GPU(ng_lw_in*(nreg-1)*nlev_cld, &
        ...
        do jlev = jtop, jbot
          ...
          if (ng3D < ng_lw_in) then
            ...
            do jreg = 2, nreg
              call calc_ref_trans_lw_GPU(ng_lw_in-ng3D, &
                  &  od_region_cld(ng3D+1:ng_lw_in,jreg,jlev), &
                  &  ssa_region_cld(ng3D+1:ng_lw_in,jreg,jlev), g_region_cld(ng3D+1:ng_lw_in,jreg,jlev), &
                  ! array temps are created for the next two arguments
                  &  region_fracs(jreg,jlev,jcol)*planck_hl(ng3D+1:ng_lw_in,jlev,jcol), &
                  &  region_fracs(jreg,jlev,jcol)*planck_hl(ng3D+1:ng_lw_in,jlev+1,jcol), &
                  &  reflectance(ng3D+1:ng_lw_in,jreg,jreg,jlev), &
                  &  transmittance(ng3D+1:ng_lw_in,jreg,jreg,jlev), &
                  &  source_up(ng3D+1:ng_lw_in,jreg,jlev), &
                  &  source_dn(ng3D+1:ng_lw_in,jreg,jlev))
            end do
            n_calls_meador_weaver &
                &  = n_calls_meador_weaver + (ng_lw_in-ng3D)*(nreg-1)
          end if
      ...
      if (config%do_clear) then
        ...
        total_albedo_clear(1:ng_lw_in,nlev+1) = total_albedo(1:ng_lw_in,1,1,nlev+1)
      end if
      ...
      do jlev = nlev,1,-1
        ...
        if (is_clear_sky_layer(jlev)) then
          ...
          call identity_minus_mat_x_mat_3_lw(ng_lw_in, total_albedo(:,:,:,jlev+1), &
          ...
          associate(B1=>total_albedo_below)
            ...
            call mat_x_mat_3_lw(ng_lw_in, &                             	! 1. B = C*D
                  &  total_albedo(:,:,:,jlev+1), transmittance(:,:,:,jlev), B1)
            call solve_mat_3_lw(ng_lw_in, denominator, B1, albedo_mat)            	! 2. solve AX=B
            call mat_x_mat_3_lw(ng_lw_in, transmittance(:,:,:,jlev), albedo_mat, & ! 3. Y = Z*X ...
                  & total_albedo_below)
            total_albedo_below = total_albedo_below + reflectance(:,:,:,jlev) ! + R
          end associate
          ! Source
          albedo_part = mat_x_vec_3_lw(ng_lw_in,total_albedo(:,:,:,jlev+1),source_dn(:,:,jlev))
          albedo_part = albedo_part + total_source(:,:,jlev+1)
          call solve_vec_3_lw_inplace(ng_lw_in, denominator, albedo_part) ! Solve Ax=b, overwriting b with x
          total_source_below = source_up(:,:,jlev)  &
              & + mat_x_vec_3_lw(ng_lw_in,transmittance(:,:,:,jlev), albedo_part)
        ...
        if (is_clear_sky_layer(jlev) .and. is_clear_sky_layer(jlev-1)) then
          ...
          total_source(:,:,jlev) = singlemat_x_vec_lw(ng_lw_in,&
...
!          if (config%do_3d_effects .or. config%do_3d_lw_multilayer_effects) then
          if (config%do_3d_lw_multilayer_effects) then
            ...
            total_albedo(:,:,:,jlev) = singlemat_x_mat(ng_lw_in,ng_lw_in,nreg,&
                 &  u_matrix(:,:,jlev), &
                 &  mat_x_singlemat(ng_lw_in,ng_lw_in,nreg,total_albedo_below,&
                 &  v_matrix(:,:,jlev)))
          else
            total_albedo(:,:,:,jlev) = 0.0_jprb
      ...
      do jlev = 1,nlev
        ...
        if (is_clear_sky_layer(jlev)) then
          ...
          call identity_minus_mat_x_mat_3_lw(ng_lw_in,reflectance(:,:,:,jlev), &
                &  total_albedo(:,:,:,jlev+1), denominator)
          flux_dn_above = solve_vec_3_lw(ng_lw_in,denominator, &
               &  mat_x_vec_3_lw(ng_lw_in,transmittance(:,:,:,jlev),flux_dn_below) &
               &  + mat_x_vec_3_lw(ng_lw_in,reflectance(:,:,:,jlev), &
               &  total_source(:,:,jlev+1)) &
               &  + source_dn(:,:,jlev))
          flux_up_above = mat_x_vec_3_lw(ng_lw_in,total_albedo(:,:,:,jlev+1), &
        ...
        if (is_clear_sky_layer(jlev) .and. is_clear_sky_layer(jlev+1)) then
          ...
          flux_dn_below = singlemat_x_vec_lw(ng_lw_in,v_matrix(:,:,jlev+1), &
               &    flux_dn_above)
        end if
        ...
        if (config%do_save_spectral_flux) then
          ...
          call indexed_sum_GPU(sum(flux_dn_above,2), &
               &           config%i_spec_from_reordered_g_lw, &
          ...
          if (config%do_clear) then
            ...
            call indexed_sum_GPU(flux_dn_clear, &
                 &           config%i_spec_from_reordered_g_lw, &
      ...
      if (config%do_lw_derivatives) then
        ...
        ! from the surface
        call calc_lw_derivatives_matrix_GPU(ng_lw_in, nlev, nreg, jcol, transmittance, &
             &  u_matrix(:,:,:), sum(flux_up_above,2), flux%lw_derivatives, lacc=lacc)
      end if
  ...
  subroutine solver_spartacus_lw_CPU(ng_lw_in, nlev,istartcol,iendcol, &
    ...
    do jcol = istartcol, iendcol
      ...
      ! Compute wavelength-independent overlap matrices u_matrix and v_matrix
      call calc_overlap_matrices_CPU(nlev, nreg, is_clear_sky_layer, &
        &  region_fracs(:,:,jcol), cloud%overlap_param(jcol,:), &
        &  v_matrix, u_matrix=u_matrix, decorrelation_scaling=config%cloud_inhom_decorr_scaling, &
        &  cloud_fraction_threshold=config%cloud_fraction_threshold, &
        &  use_beta_overlap=config%use_beta_overlap, &
      ...
      if (config%do_lw_aerosol_scattering) then
        call calc_ref_trans_lw_CPU(ng_lw_in*nlev, &
            &  od_region_clear, ssa_clear, g_clear, &
            &  planck_hl(:,1:nlev,jcol), planck_hl(:,2:nlev+1,jcol), &
            &  ref_clear, trans_clear, &
            &  source_up_clear, source_dn_clear, &
        ...
        call calc_no_scattering_transmittance_lw_CPU(ng_lw_in*nlev, &
            &  od(:,:, jcol), &
            &  planck_hl(:,1:nlev,jcol), planck_hl(:,2:nlev+1,jcol), &
            &  trans_clear, &
            &  source_up_clear, source_dn_clear)
      ...
      do while (are_clouds_below)
        ...
        ! Calculate two-stream variables gamma1 and gamma2 of
        ! all g-points and cloudy regions at once
        call calc_two_stream_gammas_lw_CPU(ng_lw_in*(nreg-1)*nlev_cld, &
          &  ssa_region_cld, g_region_cld, &
          &  gamma1_cld(:,:,jtop:jbot), gamma2_cld(:,:,jtop:jbot))
        ...
        do jlev = jtop, jbot
          ...
          if (ng3D < ng_lw_in) then
            ...
            ! solutions to the coupled ODEs for the sources.
            do jreg = 2, nreg
              call calc_ref_trans_lw_CPU(ng_lw_in-ng3D, &
                  &  od_region_cld(ng3D+1:ng_lw_in,jreg,jlev), &
                  &  ssa_region_cld(ng3D+1:ng_lw_in,jreg,jlev), g_region_cld(ng3D+1:ng_lw_in,jreg,jlev), &
                  ! array temps are created for the next two arguments
                  &  region_fracs(jreg,jlev,jcol)*planck_hl(ng3D+1:ng_lw_in,jlev,jcol), &
...
!          if (config%do_3d_effects .or. config%do_3d_lw_multilayer_effects) then
      ...
      if (config%do_save_spectral_flux) then
        call indexed_sum_CPU(sum(total_source(:,:,1),2), &
             &           config%i_spec_from_reordered_g_lw, &
             &           flux%lw_up_band(:,jcol,1))
        flux%lw_dn_band(:,jcol,1) = 0.0_jprb
        if (config%do_clear) then
          call indexed_sum_CPU(total_source_clear(:,1), &
               &           config%i_spec_from_reordered_g_lw, &
               &           flux%lw_up_clear_band(:,jcol,1))
          flux%lw_dn_clear_band(:,jcol,1) = 0.0_jprb
        end if
      ...
      do jlev = 1,nlev
        ...
        if (config%do_save_spectral_flux) then
          call indexed_sum_CPU(sum(flux_up_above,2), &
               &           config%i_spec_from_reordered_g_lw, &
               &           flux%lw_up_band(:,jcol,jlev+1))
          call indexed_sum_CPU(sum(flux_dn_above,2), &
               &           config%i_spec_from_reordered_g_lw, &
               &           flux%lw_dn_band(:,jcol,jlev+1))
          if (config%do_clear) then
            call indexed_sum_CPU(flux_up_clear, &
                 &           config%i_spec_from_reordered_g_lw, &
                 &           flux%lw_up_clear_band(:,jcol,jlev+1))
            call indexed_sum_CPU(flux_dn_clear, &
                 &           config%i_spec_from_reordered_g_lw, &
                 &           flux%lw_dn_clear_band(:,jcol,jlev+1))
          end if
        end if
      ...
      if (config%do_lw_derivatives) then
        ...
        ! regions first to provide a simple spectral flux upwelling
        ! from the surface
        call calc_lw_derivatives_matrix_CPU(ng_lw_in, nlev, nreg, jcol, transmittance, &
