...
#include "radiation_optical_depth_scaling.h"
  ...
  subroutine solver_spartacus_sw_GPU(ng_sw_in, nlev,istartcol,iendcol, &
    ...
    !     real(jprb), dimension(ng_sw_in, nreg) &
    !          &  :: od_region, ssa_region
    real(jprb), dimension(ng_sw_in, nlev) :: od_region_clear
    ...
    real(jprb), dimension(ng_sw_in, nlev) &
    ...
    ! layer
    real(jprb), dimension(ng_sw_in, nlev) :: ref_clear, trans_clear
    ...
    ! (trans_dir_dir).
    real(jprb), dimension(ng_sw_in, nreg, nreg, nlev) &
    ...
    real(jprb), dimension(ng_sw_in, nreg) :: source_dn
    ! ...clear-sky equivalent
    real(jprb), dimension(ng_sw_in) :: source_dn_clear
    ...
    real(jprb), dimension(ng_sw_in, nreg) :: total_source
    ...
    real(jprb), dimension(ng_sw_in) :: direct_dn_clear
    ...
    ! above occurs
    real(jprb), dimension(ng_sw_in) :: fractal_fac_dir
    real(jprb), dimension(ng_sw_in) :: fractal_fac_dif
    ...
    ! interface)
    real(jprb), dimension(ng_sw_in) :: flux_up_clear, flux_dn_clear
...
!     ng_sw_in   = config%n_g_sw
    ...
    ! Since the size of Gamma is ng_sw_in*nlevs*9*9, and ng_sw_in changes with the gas optics model (32 for ECCKD),
    ! some expression which depends on ng_sw_in and working precision (and ideally cache size) should be used
    ! nlev_cld_limit = 1536 / (ng_sw_in* sizeof(1.0_jprb))  != 12 if ng_sw_in=32 and wp=sp (sizeof=4)
    nlev_cld_limit = max(nint(800.0_jprb / (ng_sw_in* sizeof(1.0_jprb))),1)  != 2 for RRTMG, 6 for ECCKD
    ...
    call calc_region_properties_GPU(nlev,nreg,istartcol,iendcol, &
         &  config%i_cloud_pdf_shape == IPdfShapeGamma, &
         &  cloud%fraction, cloud%fractional_std, region_fracs, &
         &  od_scaling, config%cloud_fraction_threshold, lacc=lacc)
    ...
    do jcol = istartcol, iendcol
      ...
      call calc_overlap_matrices_dp_GPU(nlev, nreg, is_clear_sky_layer, &
          ...
          &  cloud_cov=flux%cloud_cover_sw(jcol), lacc=lacc)
      ...
      ! which in the shortwave does not waste many computations because ng3D ~ ng_sw_in
      ...
      call calc_ref_trans_sw_GPU(ng_sw_in*nlev, &
      ...
      do while (are_clouds_below)
        ...
        ! So instead of Gamma_z1(ng_sw_in,jtop:jbot,        9,9),
        !               Gamma_z1(ng_sw_in,jtop:jbot_block,  9,9)
        ...
        allocate(od_region_cld(ng_sw_in,jtop:jbot,2:nreg), ssa_region_cld(ng_sw_in,jtop:jbot,2:nreg))
        allocate(g_region_cld (ng_sw_in,jtop:jbot,2:nreg))
        allocate(gamma1_cld(ng_sw_in,jtop:jbot,2:nreg), gamma2_cld(ng_sw_in,jtop:jbot,2:nreg), &
            & gamma3_cld(ng_sw_in,jtop:jbot,2:nreg))
        allocate(Gamma_z1(ng_sw_in, jtop:jbot,3*nreg,3*nreg))
        ...
        do jreg = 2,nreg
          do jlev = jtop, jbot
            ...
            ! Mapping from g-point to band
              if (ng_sw_in == config%n_bands_sw) then ! help vectorization for ECCKD
                iband = jg
              else
          ...
          call calc_two_stream_gammas_sw_GPU(ng_sw_in*nlev_cld, &
        ...
        ! 3D effects need to be represented in (ng_sw_in*nlevs) square matrices each of dimension 3*nreg by 3*nreg,
        ...
        ! Clear-sky region
        jreg = 1
        call write_gamma_diag_GPU(ng_sw_in*nlev_cld, nreg, jreg, od_region_clear(:,jtop:jbot), &
        &   gamma1_clear(:,jtop:jbot), gamma2_clear(:,jtop:jbot), gamma3_clear(:,jtop:jbot), &
        &   ssa(:,jtop:jbot,jcol), one_over_mu0, Gamma_z1, lacc=lacc)
        ! Cloudy regions
        do jreg = 2, nreg
          call write_gamma_diag_GPU(ng_sw_in*nlev_cld, nreg, jreg, od_region_cld(:,:,jreg), &
          &   gamma1_cld(:,:,jreg), gamma2_cld(:,:,jreg), gamma3_cld(:,:,jreg), &
          &   ssa_region_cld(:,:,jreg), one_over_mu0, Gamma_z1, lacc=lacc)
        end do
        ...
        call expm_sw(ng3D, ng_sw_in, nlev_cld, Gamma_z1)

        ! Update count of expm calls
        n_calls_expm = n_calls_expm + ng3D

        ! Following computations are not able to be batched by collapsing ng_sw_in and nlev because of
        ! different dimension order - trans_dir_dir(ng_sw_in,nreg,nreg,nlev) and Gamma(ng_sw_in,nlev,nreg,nreg)
        associate(gamma_z11=>albedo_part, gamma_z21 => total_albedo_below, &
          ...
          do jlev = jtop, jbot
            ...
            call solve_mat_3_sw(ng_sw_in, gamma_z11, &
      ...
      do jlev = nlev,1,-1
        ...
        if (is_clear_sky_layer(jlev)) then
          ...
          call identity_minus_mat_x_mat_3_sw(ng_sw_in, &
          ...
          associate(B1=>total_albedo_below, B2=>total_albedo_below_direct, X=>albedo_part)
            ...
            call mat_x_mat_3_sw(ng_sw_in,&                                ! 1. B = C*D
                & total_albedo(:,:,:,jlev+1), transmittance(:,:,:,jlev), B1)
            call solve_mat_3_sw(ng_sw_in,denominator, B1, X)              ! 2. solve AX=B
            call mat_x_mat_3_sw(ng_sw_in, transmittance(:,:,:,jlev), X, & ! 3. Y = Z*X ...
                & total_albedo_below)
            total_albedo_below = total_albedo_below + reflectance(:,:,:,jlev) ! + R

            ! Direct
            call mat_x_mat_3_sw(ng_sw_in, &                               ! 1. B = C1*D1 ...
                &  total_albedo_direct(:,:,:,jlev+1), trans_dir_dir(:,:,:,jlev), X)
            call mat_x_mat_3_sw(ng_sw_in, &                               ! + C2+D3
                & total_albedo(:,:,:,jlev+1), trans_dir_diff(:,:,:,jlev), B2)
            B2 = X + B2
            call solve_mat_3_sw(ng_sw_in,denominator,B2, X)               ! 2. solve AX=B
            call mat_x_mat_3_sw(ng_sw_in,transmittance(:,:,:,jlev),X,B2)  ! Z*X
        ...
        if ((config%i_3d_sw_entrapment == IEntrapmentExplicitNonFractal &
          ...
          call step_migrations_GPU(ng_sw_in, nreg, cloud%fraction(jcol,jlev), &
        ...
        if (is_clear_sky_layer(jlev) .and. is_clear_sky_layer(jlev-1)) then
          ...
          albedo_part = mat_x_singlemat_3_sw(ng_sw_in,&
               & total_albedo_below,v_matrix(:,:,jlev))
          total_albedo(:,:,:,jlev) = singlemat_x_mat_3_sw(ng_sw_in,&
               &  u_matrix(:,:,jlev), albedo_part)
          albedo_part = mat_x_singlemat_3_sw(ng_sw_in,&
               & total_albedo_below_direct,v_matrix(:,:,jlev))
          total_albedo_direct(:,:,:,jlev) = singlemat_x_mat_3_sw(ng_sw_in,&
               &  u_matrix(:,:,jlev), albedo_part)
          ...
          albedo_part2 = mat_x_singlemat_3_sw(ng_sw_in,albedo_part,&
               & v_matrix(:,:,jlev))
          total_albedo(:,:,:,jlev) = singlemat_x_mat_3_sw(ng_sw_in,&
...
!             ! only need e21, e12, e32, e23 for fastexpm and e11, e22 for the minvals,
            ...
            call fast_expm_exchange_3(2*nreg*ng_sw_in, entrapment)
      ...
      flux_up_above = mat_x_vec_3_sw(ng_sw_in,total_albedo_direct(:,:,:,1),direct_dn_below)
      ...
      if (config%do_save_spectral_flux) then
        call indexed_sum_GPU(sum(flux_up_above(:,:),2), &
             &           config%i_spec_from_reordered_g_sw, &
             &           flux%sw_up_band(:,jcol,1))
        call indexed_sum_GPU(sum(direct_dn_below(:,:),2), &
             &           config%i_spec_from_reordered_g_sw, &
        ...
        if (config%do_clear) then
          ...
          call indexed_sum_GPU(flux_up_clear, &
               &           config%i_spec_from_reordered_g_sw, &
      ...
      do jlev = 1,nlev
        ...
        ! source_dn(:,:) = mat_x_vec_3_sw(ng_sw_in,trans_dir_diff(:,:,:,jlev),direct_dn_below, &
        !     &  is_clear_sky_layer(jlev))
        ! direct_dn_above = mat_x_vec_3_sw(ng_sw_in,trans_dir_dir(:,:,:,jlev),direct_dn_below, &
        !     &  is_clear_sky_layer(jlev))
        ...
        if (is_clear_sky_layer(jlev)) then
          ...
          call identity_minus_mat_x_mat_3_sw(ng_sw_in,reflectance(:,:,:,jlev), &
              &  total_albedo(:,:,:,jlev+1), denominator)
          total_source = mat_x_vec_3_sw(ng_sw_in,total_albedo_direct(:,:,:,jlev+1),direct_dn_above)

          flux_dn_above = solve_vec_3_sw(ng_sw_in,denominator, &
               &  mat_x_vec_3_sw(ng_sw_in,transmittance(:,:,:,jlev),flux_dn_below) &
               &  + mat_x_vec_3_sw(ng_sw_in,reflectance(:,:,:,jlev), total_source(:,:)) &
               &  + source_dn(:,:))
          flux_up_above = mat_x_vec_3_sw(ng_sw_in,total_albedo(:,:,:,jlev+1), &
        ...
        if (is_clear_sky_layer(jlev) .and. is_clear_sky_layer(jlev+1)) then
          ...
          ! layer below
          flux_dn_below = singlemat_x_vec_sw(ng_sw_in,v_matrix(:,:,jlev+1), &
               &  flux_dn_above)
          direct_dn_below = singlemat_x_vec_sw(ng_sw_in,v_matrix(:,:,jlev+1), &
               &  direct_dn_above)
        end if
        ...
        if (config%do_save_spectral_flux) then
          ! Down
          call indexed_sum_GPU(sum(direct_dn_above,2), &
               &           config%i_spec_from_reordered_g_sw, &
          ...
          if (config%do_clear) then
            call indexed_sum_GPU(direct_dn_clear, &
                 &           config%i_spec_from_reordered_g_sw, &
          ...
          call add_indexed_sum_GPU(sum(flux_dn_above,2), &
               &           config%i_spec_from_reordered_g_sw, &
               &           flux%sw_dn_band(:,jcol,jlev+1), lacc=lacc)
          if (config%do_clear) then
            call indexed_sum_GPU(flux_up_clear, &
                 &           config%i_spec_from_reordered_g_sw, &
                 &           flux%sw_up_clear_band(:,jcol,jlev+1))
            call add_indexed_sum_GPU(flux_dn_clear, &
                 &           config%i_spec_from_reordered_g_sw, &
                 &           flux%sw_dn_clear_band(:,jcol,jlev+1), lacc=lacc)
          end if
        end if
...
!     ng_sw_in   = config%n_g_sw
    ...
    ! Compute the wavelength-independent region fractions and
    ! optical-depth scalings
    call calc_region_properties_CPU(nlev,nreg,istartcol,iendcol, &
         &  config%i_cloud_pdf_shape == IPdfShapeGamma, &
         &  cloud%fraction, cloud%fractional_std, region_fracs, &
    ...
    do jcol = istartcol, iendcol
      ...
      ! Compute wavelength-independent overlap matrices u_matrix and v_matrix
      call calc_overlap_matrices_dp_CPU(nlev, nreg, is_clear_sky_layer, &
          &  region_fracs(:,:,jcol), cloud%overlap_param(jcol,:), &
          &  u_matrix=u_matrix, v_matrix=v_matrix, &
          &  decorrelation_scaling=config%cloud_inhom_decorr_scaling, &
          &  cloud_fraction_threshold=config%cloud_fraction_threshold, &
      ...
      ! Compute reflectance, transmittance and associated terms for
      ! clear skies, using the Meador-Weaver formulas
      call calc_ref_trans_sw_CPU(ng_sw_in*nlev, &
          &  mu0, od_region_clear, ssa(:,:,jcol), g(:,:,jcol), &
          &  ref_clear, trans_clear, ref_dir_clear, trans_dir_diff_clear, trans_dir_dir_clear, &
          &  gamma1_clear, gamma2_clear, gamma3_clear)
      ...
      do while (are_clouds_below)
        ...
        do jreg = 2,nreg
          ...
          ! Calculate two-stream variables gamma1-gamma3 for cloudy regions
          call calc_two_stream_gammas_sw_CPU(ng_sw_in*nlev_cld, &
            &  mu0, ssa_region_cld(:,:,jreg), g_region_cld(:,:,jreg), &
            &  gamma1_cld(:,:,jreg), gamma2_cld(:,:,jreg), gamma3_cld(:,:,jreg))
        ...
        ! Clear-sky region
        jreg = 1
        call write_gamma_diag_CPU(ng_sw_in*nlev_cld, nreg, jreg, od_region_clear(:,jtop:jbot), &
        &   gamma1_clear(:,jtop:jbot), gamma2_clear(:,jtop:jbot), gamma3_clear(:,jtop:jbot), &
        &   ssa(:,jtop:jbot,jcol), one_over_mu0, Gamma_z1)
        ! Cloudy regions
        do jreg = 2, nreg
          call write_gamma_diag_CPU(ng_sw_in*nlev_cld, nreg, jreg, od_region_cld(:,:,jreg), &
          &   gamma1_cld(:,:,jreg), gamma2_cld(:,:,jreg), gamma3_cld(:,:,jreg), &
      ...
      do jlev = nlev,1,-1
        ...
        if ((config%i_3d_sw_entrapment == IEntrapmentExplicitNonFractal &
          ...
          !  distances just above the base of the layer, and need to
          !  step them to just below the top of the same layer
          call step_migrations_CPU(ng_sw_in, nreg, cloud%fraction(jcol,jlev), &
               & layer_depth(jlev), tan_diffuse_angle_3d, tan_sza, &
               &  reflectance(:,:,:,jlev), transmittance(:,:,:,jlev), &
               &  ref_dir(:,:,:,jlev), trans_dir_dir(:,:,:,jlev), &
               &  trans_dir_diff(:,:,:,jlev), total_albedo(:,:,:,jlev+1), &
...
!             ! only need e21, e12, e32, e23 for fastexpm and e11, e22 for the minvals,
      ...
      ! Save the spectral fluxes if required
      if (config%do_save_spectral_flux) then
        call indexed_sum_CPU(sum(flux_up_above(:,:),2), &
             &           config%i_spec_from_reordered_g_sw, &
             &           flux%sw_up_band(:,jcol,1))
        call indexed_sum_CPU(sum(direct_dn_below(:,:),2), &
             &           config%i_spec_from_reordered_g_sw, &
             &           flux%sw_dn_band(:,jcol,1))
        flux%sw_dn_band(:,jcol,1) = mu0 * flux%sw_dn_band(:,jcol,1)
        if (allocated(flux%sw_dn_direct_band)) then
        ...
        if (config%do_clear) then
          flux%sw_dn_clear_band(:,jcol,1) = flux%sw_dn_band(:,jcol,1)
          call indexed_sum_CPU(flux_up_clear, &
               &           config%i_spec_from_reordered_g_sw, &
               &           flux%sw_up_clear_band(:,jcol,1))
          if (allocated(flux%sw_dn_direct_clear_band)) then
            flux%sw_dn_direct_clear_band(:,jcol,1) &
      ...
      do jlev = 1,nlev
        ...
        if (config%do_save_spectral_flux) then
          ! Down
          call indexed_sum_CPU(sum(direct_dn_above,2), &
               &           config%i_spec_from_reordered_g_sw, &
               &           flux%sw_dn_band(:,jcol,jlev+1))
          flux%sw_dn_band(:,jcol,jlev+1) = mu0 * flux%sw_dn_band(:,jcol,jlev+1)
          ...
          if (config%do_clear) then
            call indexed_sum_CPU(direct_dn_clear, &
                 &           config%i_spec_from_reordered_g_sw, &
                 &           flux%sw_dn_clear_band(:,jcol,jlev+1))
            flux%sw_dn_clear_band(:,jcol,jlev+1) = mu0 &
                 &   * flux%sw_dn_clear_band(:,jcol,jlev+1)
          ...
          call indexed_sum_CPU(sum(flux_up_above,2), &
               &           config%i_spec_from_reordered_g_sw, &
               &           flux%sw_up_band(:,jcol,jlev+1))
          call add_indexed_sum_CPU(sum(flux_dn_above,2), &
               &           config%i_spec_from_reordered_g_sw, &
               &           flux%sw_dn_band(:,jcol,jlev+1))
          if (config%do_clear) then
            call indexed_sum_CPU(flux_up_clear, &
                 &           config%i_spec_from_reordered_g_sw, &
                 &           flux%sw_up_clear_band(:,jcol,jlev+1))
            call add_indexed_sum_CPU(flux_dn_clear, &
                 &           config%i_spec_from_reordered_g_sw, &
                 &           flux%sw_dn_clear_band(:,jcol,jlev+1))
          end if
        end if
