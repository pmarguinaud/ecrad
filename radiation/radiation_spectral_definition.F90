...
contains
  ...
  subroutine allocate_bands_only_GPU(this, reference_temperature, wavenumber1, wavenumber2, lacc)
    ...
    call this%deallocate_GPU(lacc=lacc)
  ...
  pure function find_wavenumber_GPU(this, wavenumber, lacc)
    ...
    if (wavenumber < this%wavenumber1(1) .or. wavenumber > this%wavenumber2(this%nwav)) then
      ...
      find_wavenumber_GPU = 0
    else
      find_wavenumber_GPU = 1
      do while (wavenumber > this%wavenumber2(find_wavenumber_GPU) &
           &    .and. find_wavenumber_GPU < this%nwav)
        find_wavenumber_GPU = find_wavenumber_GPU + 1
      end do
    end if
    ...
    if (use_bands_local) then
      ...
      if (present(weighting_temperature)) then
        if (weighting_temperature > 0.0_jprb) then
          planck_weight = calc_planck_function_wavenumber_GPU(wavenumber, &
               &                          weighting_temperature, lacc)
        else
        ...
        planck_weight = calc_planck_function_wavenumber_GPU(wavenumber, &
             &                          this%reference_temperature, lacc)
      end if
      ...
      if (allocated(this%solar_spectral_irradiance)) then
        ...
        planck_weight = calc_planck_function_wavenumber_GPU(0.5_jprb &
      ...
      do jwav = 1,nwav
        ...
        isd1 = this%find_GPU(wavenum1, lacc)
        if (isd1 < 1) then
          cycle
        end if
        if (jwav > 1) then
          ...
          isd0 = this%find_GPU(wavenum0, lacc)
          if (isd0 == isd1) then
...
!                 &       / (wavenum1-wavenum0)
        ...
        if (jwav < nwav) then
          ...
          ! wavenumbers in this%gpoint_fraction
          isd2 = this%find_GPU(wavenum2, lacc)
...
!                 &       / (wavenum2-wavenum1)
  ...
  subroutine calc_mapping_from_bands_GPU(this, &
    ...
    if (use_bands_local) then
      ...
      do jband = 1,this%nband
        do jint = 1,ninterval
          ...
          if (wavenumber2_bound > wavenumber1_bound) then
            ...
            planck_sample = calc_planck_function_wavenumber_GPU(wavenumber_sample, &
                 &                                 this%reference_temperature, lacc)
      ...
      if (allocated(this%solar_spectral_irradiance)) then
        ...
        planck = calc_planck_function_wavenumber_GPU(wavenumber_mid, &
             &                       this%reference_temperature, lacc)
      end if
  ...
  subroutine calc_mapping_from_wavenumber_bands_GPU(this, &
    ...
    call calc_mapping_from_bands_GPU(this, wavelength_bound, i_intervals, mapping, use_bands, use_fluxes, lacc=lacc)
  ...
  pure function min_wavenumber_GPU(this, lacc)
    ...
    if (this%nwav > 0) then
      min_wavenumber_GPU = this%wavenumber1(1)
    else
      min_wavenumber_GPU = minval(this%wavenumber1_band)
    end if
  ...
  pure function max_wavenumber_GPU(this, lacc)
    ...
    if (this%nwav > 0) then
      max_wavenumber_GPU = this%wavenumber1(this%nwav)
    else
      max_wavenumber_GPU = maxval(this%wavenumber2_band)
    end if
  ...
  elemental function calc_planck_function_wavenumber_GPU(wavenumber, temperature, lacc)
    ...
    real(jprd) :: freq ! Hz
    real(jprd) :: planck_fn_freq
    logical, intent (in) :: lacc ! W m-2 Hz-1
    ...
    if (temperature > 0.0_jprd) then
      ...
      calc_planck_function_wavenumber_GPU = real(planck_fn_freq * 100.0_jprd * real(SpeedOfLight,jprd), jprb)
    else
      calc_planck_function_wavenumber_GPU = 1.0_jprb
    end if
  ...
  pure function find_wavenumber_CPU(this, wavenumber)
    ...
    if (wavenumber < this%wavenumber1(1) .or. wavenumber > this%wavenumber2(this%nwav)) then
      ! Wavenumber not present
      find_wavenumber_CPU = 0
    else
      find_wavenumber_CPU = 1
      do while (wavenumber > this%wavenumber2(find_wavenumber_CPU) &
           &    .and. find_wavenumber_CPU < this%nwav)
        find_wavenumber_CPU = find_wavenumber_CPU + 1
      end do
    end if
  ...
  subroutine calc_mapping_CPU(this, wavenumber, mapping, weighting_temperature, use_bands)
    ...
    if (use_bands_local) then
      ...
      if (present(weighting_temperature)) then
        if (weighting_temperature > 0.0_jprb) then
          planck_weight = calc_planck_function_wavenumber_CPU(wavenumber, &
      ...
      if (allocated(this%solar_spectral_irradiance)) then
        ...
        planck_weight = calc_planck_function_wavenumber_CPU(0.5_jprb &
             &  * (this%wavenumber1 + this%wavenumber2), &
             &  this%reference_temperature)
      end if
...
!                 &       / (wavenum2-wavenum1)
  ...
  subroutine calc_mapping_from_bands_CPU(this, &
    ...
    if (use_bands_local) then
      ...
      do jband = 1,this%nband
        do jint = 1,ninterval
          ...
          if (wavenumber2_bound > wavenumber1_bound) then
            ...
            wavenumber_sample = wavenumber1_bound + [(isamp,isamp=0,nsample-1)] &
                 &  * (wavenumber2_bound-wavenumber1_bound) / real(nsample-1,jprb)
            planck_sample = calc_planck_function_wavenumber_CPU(wavenumber_sample, &
                 &                                 this%reference_temperature)
            mapping(i_intervals(jint),jband) = mapping(i_intervals(jint),jband) &
                 &  + sum(planck_sample*weight_sample) * (wavenumber2_bound-wavenumber1_bound)
            if (use_fluxes_local) then
      ...
      if (allocated(this%solar_spectral_irradiance)) then
        ...
        planck = calc_planck_function_wavenumber_CPU(wavenumber_mid, &
             &                       this%reference_temperature)
      end if
  ...
  subroutine calc_mapping_from_wavenumber_bands_CPU(this, &
    ...
    call calc_mapping_from_bands_CPU(this, wavelength_bound, i_intervals, mapping, use_bands, use_fluxes)
  ...
  pure function min_wavenumber_CPU(this)
    ...
    if (this%nwav > 0) then
      min_wavenumber_CPU = this%wavenumber1(1)
    else
      min_wavenumber_CPU = minval(this%wavenumber1_band)
    end if
  ...
  pure function max_wavenumber_CPU(this)
    ...
    if (this%nwav > 0) then
      max_wavenumber_CPU = this%wavenumber1(this%nwav)
    else
      max_wavenumber_CPU = maxval(this%wavenumber2_band)
    end if
  ...
  elemental function calc_planck_function_wavenumber_CPU(wavenumber, temperature)
    ...
    if (temperature > 0.0_jprd) then
      ...
      planck_fn_freq = 2.0_jprd * real(PlanckConstant,jprd) * freq**3 &
           &  / (real(SpeedOfLight,jprd)**2 * (exp(real(PlanckConstant,jprd)*freq &
           &     /(real(BoltzmannConstant,jprd)*real(temperature,jprd))) - 1.0_jprd))
      calc_planck_function_wavenumber_CPU = real(planck_fn_freq * 100.0_jprd * real(SpeedOfLight,jprd), jprb)
    else
      calc_planck_function_wavenumber_CPU = 1.0_jprb
    end if
