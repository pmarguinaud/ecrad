...
! Allow size of inner dimension (number of g-points) to be known at compile time if NG_LW is defined
    ...
    ! real(jprb), dimension(ng_lw_in) :: od_total, ssa_total, g_total
    real(jprb), dimension(ng_lw_in, 2:nregions) :: od_total, ssa_total, g_total, &
          &                                  planck_hl_bot, planck_hl_top

    ! Modified optical depth after Tripleclouds scaling to represent
    ! cloud inhomogeneity
    real(jprb), dimension(ng_lw_in) :: od_cloud_new
    ...
    ! real(jprb), dimension(ng_lw_in, nregions, nlev) :: reflectance, transmittance
    ...
    real(jprb), dimension(ng_lw_in, 2:nregions, nlev) :: reflectance
    real(jprb), dimension(ng_lw_in, nregions, nlev) ::  transmittance

    ! Emission by a layer into the upwelling or downwelling diffuse
    ! streams, cloudy regions
    real(jprb), dimension(ng_lw_in, 2:nregions, nlev) &
         &  :: source_up, source_dn

    ! Clear-sky reflectance and transmittance
    real(jprb), dimension(ng_lw_in, nlev) &
    ...
    real(jprb), dimension(ng_lw_in, nregions) &
    ...
    call calc_region_properties_GPU(nlev,nregions,istartcol,iendcol, &
    ...
    do jcol = istartcol, iendcol
      ...
      call calc_overlap_matrices_GPU(nlev, nregions, is_clear_sky_layer, &
      ...
      if (.not. config%do_lw_aerosol_scattering) then
        ...
        call calc_no_scattering_transmittance_lw_GPU(ng_lw_in*nlev, od(:,:,jcol), &
        ...
        call calc_fluxes_no_scattering_lw_GPU(ng_lw_in, nlev, &
             &  trans_clear, source_up_clear, source_dn_clear, &
        ...
        call adding_ica_lw_GPU(ng_lw_in, nlev, &
             &  ref_clear, trans_clear, source_up_clear, source_dn_clear, &
             &  emission(:,jcol), albedo(:,jcol), &
             &  flux_up_clear, flux_dn_clear, lacc=lacc)
      end if
      ...
      if (config%do_clear) then
        ...
        if (config%do_save_spectral_flux) then
          do jlev = 1,nlev+1
            call indexed_sum_GPU(flux_up_clear(:,jlev), &
                 &           config%i_spec_from_reordered_g_lw, &
                 &           flux%lw_up_clear_band(:,jcol,jlev))
            call indexed_sum_GPU(flux_dn_clear(:,jlev), &
                 &           config%i_spec_from_reordered_g_lw, &
      ...
      ! G-points are batched (collapsed) with cloudy regions (ng_lw_in*2) for bigger vector length (helps ECCKD)
      ...
      do jlev = i_cloud_top,nlev ! Start at cloud top and work down
        if (is_clear_sky_layer(jlev)) then
          ...
          if (config%do_lw_cloud_scattering) then
            call calc_ref_trans_lw_GPU(ng_lw_in*2, &
            ...
            call calc_no_scattering_transmittance_lw_GPU(ng_lw_in*2, &
                &  od_total(:,2:nregions), &
                &  planck_hl_top(:,2:nregions), planck_hl_bot(:,2:nregions), &
      ...
      allocate(total_albedo(ng_lw_in,nregions,i_cloud_top:nlev+1), total_source(ng_lw_in,nregions,i_cloud_top:nlev+1))
      ...
      if (config%do_save_spectral_flux) then
        call indexed_sum_GPU(flux_up(:,1), &
             &           config%i_spec_from_reordered_g_lw, &
    ...
    ! --------------------------------------------------------
      ...
      do jlev = i_cloud_top,nlev
        ...
        if (.not. (is_clear_sky_layer(jlev) &
          ...
          flux_dn = singlemat_x_vec_lw(ng_lw_in, v_matrix(:,:,jlev+1), flux_dn)
        ...
        ! Save the spectral fluxes if required
        if (config%do_save_spectral_flux) then
          call indexed_sum_GPU(sum(flux_up,2), &
               &           config%i_spec_from_reordered_g_lw, &
               &           flux%lw_up_band(:,jcol,jlev+1))
          call indexed_sum_GPU(sum(flux_dn,2), &
               &           config%i_spec_from_reordered_g_lw, &
      ...
      if (config%do_lw_derivatives) then
        ...
        ! from the surface
        call calc_lw_derivatives_region_GPU(ng_lw_in, nlev, nregions, jcol, transmittance, &
             &  u_matrix(:,:,:), sum(flux_up,2), flux%lw_derivatives, lacc=lacc)
      end if
...
! Allow size of inner dimension (number of g-points) to be known at compile time if NG_LW is defined
    ...
    ! Compute the wavelength-independent region fractions and
    ! optical-depth scalings
    call calc_region_properties_CPU(nlev,nregions,istartcol,iendcol, &
         &  config%i_cloud_pdf_shape == IPdfShapeGamma, &
         &  cloud%fraction, cloud%fractional_std, region_fracs, &
    ...
    do jcol = istartcol, iendcol
      ...
      ! Compute wavelength-independent overlap matrices u_matrix and v_matrix
      call calc_overlap_matrices_CPU(nlev, nregions, is_clear_sky_layer, &
        &  region_fracs(:,:,jcol), cloud%overlap_param(jcol,:), &
        &  v_matrix, u_matrix=u_matrix, decorrelation_scaling=config%cloud_inhom_decorr_scaling, &
        &  cloud_fraction_threshold=config%cloud_fraction_threshold, &
        &  use_beta_overlap=config%use_beta_overlap, &
      ...
      if (.not. config%do_lw_aerosol_scattering) then
        ...
        ! the first two dimensions of the input arrays are unpacked
        ! into vectors inside the routine
        call calc_no_scattering_transmittance_lw_CPU(ng_lw_in*nlev, od(:,:,jcol), &
             &  planck_hl(:,1:nlev,jcol), planck_hl(:,2:nlev+1, jcol), &
             &  trans_clear, source_up_clear, source_dn_clear)
        ! Ensure that clear-sky reflectance is zero since it may be
        ! used in cloudy-sky case
        ref_clear = 0.0_jprb
        ! Simple down-then-up method to compute fluxes
        call calc_fluxes_no_scattering_lw_CPU(ng_lw_in, nlev, &
             &  trans_clear, source_up_clear, source_dn_clear, &
             &  emission(:,jcol), albedo(:,jcol), &
             &  flux_up_clear, flux_dn_clear)
      else
        ...
        call calc_ref_trans_lw_CPU(ng_lw_in*nlev, &
             ...
             &  source_up_clear, source_dn_clear)
        ! Use adding method to compute fluxes
        call adding_ica_lw_CPU(ng_lw_in, nlev, &
             &  ref_clear, trans_clear, source_up_clear, source_dn_clear, &
             &  emission(:,jcol), albedo(:,jcol), &
             &  flux_up_clear, flux_dn_clear)
      end if
      ...
      if (config%do_clear) then
        ...
        if (config%do_save_spectral_flux) then
          do jlev = 1,nlev+1
            call indexed_sum_CPU(flux_up_clear(:,jlev), &
                 &           config%i_spec_from_reordered_g_lw, &
                 &           flux%lw_up_clear_band(:,jcol,jlev))
            call indexed_sum_CPU(flux_dn_clear(:,jlev), &
                 &           config%i_spec_from_reordered_g_lw, &
                 &           flux%lw_dn_clear_band(:,jcol,jlev))
          end do
        end if
      ...
      do jlev = i_cloud_top,nlev ! Start at cloud top and work down
        if (is_clear_sky_layer(jlev)) then
          ...
          if (config%do_lw_cloud_scattering) then
            call calc_ref_trans_lw_CPU(ng_lw_in*2, &
              &  od_total(:,2:nregions), ssa_total(:,2:nregions), g_total(:,2:nregions), &
              &  planck_hl_top(:,2:nregions), planck_hl_bot(:,2:nregions), &
              &  reflectance(:,2:nregions,jlev), transmittance(:,2:nregions,jlev), &
              &  source_up(:,2:nregions,jlev), source_dn(:,2:nregions,jlev))
            ...
            ! No-scattering case: use simpler functions for
            ! transmission and emission
            call calc_no_scattering_transmittance_lw_CPU(ng_lw_in*2, &
                &  od_total(:,2:nregions), &
                &  planck_hl_top(:,2:nregions), planck_hl_bot(:,2:nregions), &
                &  transmittance(:,2:nregions,jlev), &
                &  source_up(:,2:nregions,jlev), source_dn(:,2:nregions,jlev))
      ...
      if (config%do_save_spectral_flux) then
        call indexed_sum_CPU(flux_up(:,1), &
             &           config%i_spec_from_reordered_g_lw, &
             &           flux%lw_up_band(:,jcol,i_cloud_top))
      end if
      ! do jlev = i_cloud_top-1,1,-1
    ...
    ! --------------------------------------------------------
      ...
      do jlev = i_cloud_top,nlev
        ...
        ! Save the spectral fluxes if required
        if (config%do_save_spectral_flux) then
          call indexed_sum_CPU(sum(flux_up,2), &
               &           config%i_spec_from_reordered_g_lw, &
               &           flux%lw_up_band(:,jcol,jlev+1))
          call indexed_sum_CPU(sum(flux_dn,2), &
               &           config%i_spec_from_reordered_g_lw, &
               &           flux%lw_dn_band(:,jcol,jlev+1))
         end if
      ...
      if (config%do_lw_derivatives) then
        ...
        ! regions first to provide a simple spectral flux upwelling
        ! from the surface
        call calc_lw_derivatives_region_CPU(ng_lw_in, nlev, nregions, jcol, transmittance, &
