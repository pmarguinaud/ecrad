...
module radiation_tripleclouds_lw_opt
...
#include "radiation_optical_depth_scaling.h"
  ...
  subroutine solver_tripleclouds_lw_opt(ng_lw_in, nlev,istartcol,iendcol, &
       &  config, cloud, & 
...
#define ng ng_lw_in
    ...
    real(jprb), dimension(ng) :: gamma1, gamma2
    ...
    integer :: jcol, jlev, jg, jreg, jreg2, ret
...
#if OPTIM_CODE==2 
        call calc_no_scattering_transmittance_lw_opt(ng*nlev, od(:,:,jcol), &
    ...
    !  ret =  gptlstart('section_4_lw_reftrans')
          ...
          if (config%do_lw_cloud_scattering) then
            call calc_reflectance_transmittance_lw_opt(ng*2, &
...
#if OPTIM_CODE==2 
            call calc_no_scattering_transmittance_lw_opt(ng*2, &
                &  od_total(:,2:nregions), &
                &  planck_hl_top(:,2:nregions), planck_hl_bot(:,2:nregions), &
     ...
     ret =  gptlstart('section_8_lw')
      ...
      do jlev = i_cloud_top,nlev
        ...
        if (nregions==3) then ! faster code
          ...
          if (is_clear_sky_layer(jlev) .and. is_clear_sky_layer(jlev+1)) then
            flux%lw_up(jcol,jlev+1) = sum(flux_up(:,1))
            flux%lw_dn(jcol,jlev+1) = sum(flux_dn(:,1))
          else
  ...
  end subroutine solver_tripleclouds_lw_opt

end module radiation_tripleclouds_lw_opt