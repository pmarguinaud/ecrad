...
!    use radiation_io, only             : nulout
    ...
    real(jprb), dimension(ng_sw_in, 2:nregions, nlev) &
    ...
    real(jprb), dimension(ng_sw_in, nregions, nlev+1) &
    ...
    real(jprb), dimension(ng_sw_in, nregions) &
    ...
    real(jprb), dimension(ng_sw_in, nregions) :: direct_dn
    ! Diffuse equivalents
    real(jprb), dimension(ng_sw_in, nregions) :: flux_dn, flux_up

    ! ...clear-sky equivalent (no distinction between "above/below")
    real(jprb), dimension(ng_sw_in) &
    ...
    real(jprb), dimension(ng_sw_in, nregions) :: inv_denom
    ...
    do jcol = istartcol, iendcol
      ...
      do while (are_clouds_below)
        ...
        allocate(od_tot_batch(ng_sw_in,2,jtop:jbot))
        allocate(ssa_tot_batch(ng_sw_in,2,jtop:jbot))
        allocate(g_tot_batch(ng_sw_in,2,jtop:jbot))

        do jlev = jtop, jbot
          do jreg = 2, nregions
            if (ng_sw_in == config%n_bands_sw) then
        ...
        call calc_ref_trans_sw_GPU(ng_sw_in*2*nlev_cloud, &
      ...
      if (config%do_save_spectral_flux) then
        ...
        call indexed_sum_GPU(sum(direct_dn,2), &
             &           config%i_spec_from_reordered_g_sw, &
        ...
        call add_indexed_sum_GPU(sum(flux_dn,2), &
             &           config%i_spec_from_reordered_g_sw, &
             &           flux%sw_dn_band(:,jcol,1), lacc=lacc)
        if (config%do_clear) then
          ...
          call add_indexed_sum_GPU(flux_dn_clear, &
               &           config%i_spec_from_reordered_g_sw, &
               &           flux%sw_dn_clear_band(:,jcol,1), lacc=lacc)
        end if
      end if
      ...
      do jlev = 1,nlev
        ...
        if (.not. (is_clear_sky_layer(jlev) &
          ...
          flux_dn = singlemat_x_vec_sw(ng_sw_in, &
               &  v_matrix(:,:,jlev+1), flux_dn)
          direct_dn = singlemat_x_vec_sw(ng_sw_in, &
...
!    use radiation_io, only             : nulout
    ...
    do jcol = istartcol, iendcol
      ...
      do while (are_clouds_below)
        ...
        call calc_ref_trans_sw_CPU(ng_sw_in*2*nlev_cloud, &
          &  mu0, od_tot_batch, ssa_tot_batch, g_tot_batch, &
          &  reflectance(:,:,jtop:jbot), transmittance(:,:,jtop:jbot), &
          &  ref_dir(:,:,jtop:jbot), trans_dir_diff(:,:,jtop:jbot), &
          &  trans_dir_dir(:,:,jtop:jbot))
      ...
      if (config%do_save_spectral_flux) then
        call indexed_sum_CPU(sum(flux_up,2), &
             &           config%i_spec_from_reordered_g_sw, &
             &           flux%sw_up_band(:,jcol,1))
        call indexed_sum_CPU(sum(direct_dn,2), &
             &           config%i_spec_from_reordered_g_sw, &
             &           flux%sw_dn_band(:,jcol,1))
        flux%sw_dn_band(:,jcol,1) = mu0 * flux%sw_dn_band(:,jcol,1)
        if (allocated(flux%sw_dn_direct_band)) then
        ...
        call add_indexed_sum_CPU(sum(flux_dn,2), &
             &           config%i_spec_from_reordered_g_sw, &
             &           flux%sw_dn_band(:,jcol,1))
        if (config%do_clear) then
