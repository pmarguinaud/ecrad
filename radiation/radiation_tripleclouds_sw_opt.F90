...
module radiation_tripleclouds_sw_opt
...
#include "radiation_optical_depth_scaling.h"
  ...
  subroutine solver_tripleclouds_sw_opt(ng_sw_in, nlev,istartcol,iendcol, &
...
!    use radiation_io, only             : nulout
    ...
    use radiation_two_stream, only     : calc_reflectance_transmittance_sw_opt, &
         & calc_reflectance_transmittance_sw, calc_two_stream_gammas_sw
#ifdef USE_TIMING
    ! Timing library
...
#define ng ng_sw_in
...
#if OPTIM_CODE==2
#else
    ! below is just for testing; in final version (OPTIM=2) gamma computations are inlined
    real(jprb), allocatable, dimension(:) :: gamma1, gamma2, gamma3
#endif
    ...
    ret =  gptlstop('overlap_matrices')
      ...
      call calc_reflectance_transmittance_sw_opt(ng*nlev, &
          ...
          &  trans_dir_dir_clear )  
#else 
      allocate(gamma1(ng*nlev),gamma2(ng*nlev),gamma3(ng*nlev))
      call calc_two_stream_gammas_sw(ng*nlev, &
        &  mu0, ssa(:,:,jcol), g(:,:,jcol), &
        &  gamma1, gamma2, gamma3)
      call calc_reflectance_transmittance_sw(ng*nlev, &
        &  mu0, od(:,:,jcol), ssa(:,:,jcol), &
        &  gamma1, gamma2, gamma3, &
        &  reflectance_clear, transmittance_clear, &
        &  ref_dir_clear, trans_dir_diff_clear, &
        &  trans_dir_dir_clear ) 
      deallocate(gamma1,gamma2,gamma3)
#endif
#ifdef USE_TIMING
...
#if OPTIM_CODE==2
        call calc_reflectance_transmittance_sw_opt(ng*2*nlev_cloud, &
...
#else
        allocate(gamma1(ng*2*nlev_cloud),gamma2(ng*2*nlev_cloud),gamma3(ng*2*nlev_cloud))

        call calc_two_stream_gammas_sw(ng*2*nlev_cloud, &
        &  mu0, ssa_tot_batch, g_tot_batch, &
        &  gamma1, gamma2, gamma3)
        call calc_reflectance_transmittance_sw(ng*2*nlev_cloud, &
        &  mu0,  od_tot_batch, ssa_tot_batch, &
        &  gamma1, gamma2, gamma3, &
        &  reflectance(:,:,jtop:jbot), transmittance(:,:,jtop:jbot), &
        &  ref_dir(:,:,jtop:jbot), trans_dir_diff(:,:,jtop:jbot), trans_dir_dir(:,:,jtop:jbot))
  ...
  end subroutine solver_tripleclouds_sw_opt

end module radiation_tripleclouds_sw_opt
