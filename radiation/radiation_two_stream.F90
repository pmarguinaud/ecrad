...
!NEC$ shortloop
  ...
  subroutine calc_reflectance_transmittance_lw_opt(ng, &
       &    od, ssa, g, &
    ...
    if (lhook) call dr_hook('radiation_two_stream:calc_reflectance_transmittance_lw_opt',0,hook_handle)
#endif
...
#ifdef DO_DR_HOOK_TWO_STREAM
    if (lhook) call dr_hook('radiation_two_stream:calc_reflectance_transmittance_lw_opt',1,hook_handle)
#endif
  
  end subroutine calc_reflectance_transmittance_lw_opt
...
!NEC$ shortloop
  ...
  ! Optimized version, improved performance at least with GCC compiler
  subroutine calc_no_scattering_transmittance_lw_opt(ng, &
    &    od, planck_top, planck_bot, transmittance, source_up, source_dn)
 ...
 use yomhook, only : lhook, dr_hook, jphook
...
#ifdef DO_DR_HOOK_TWO_STREAM
    real(jphook) :: hook_handle
    if (lhook) call dr_hook('radiation_two_stream:calc_no_scattering_transmittance_lw_opt',0,hook_handle)
#endif
    ...
    associate(od_loc=>source_up)
      ...
      do jg = 1, ng
        ...
        ! else
        !   ! Linear limit at low optical depth
        !   ! od_loc(jg) = LwDiffusivity*od(jg)
        !   ! transmittance(jg) = 1.0_jprb - od_loc(jg)
        !   source_up(jg) = od_loc(jg) * 0.5_jprb * (planck_top(jg)+planck_bot(jg))
        !   source_dn(jg) = source_up(jg)
        ! end if
      end do
    end associate
...
#ifdef DO_DR_HOOK_TWO_STREAM
 if (lhook) call dr_hook('radiation_two_stream:calc_no_scattering_transmittance_lw_opt',1,hook_handle)
#endif

  end subroutine calc_no_scattering_transmittance_lw_opt
...
!NEC$ shortloop
  ...
  subroutine calc_reflectance_transmittance_sw_opt(ng, mu0, od, ssa, &
    ...
    if (lhook) call dr_hook('radiation_two_stream:calc_reflectance_transmittance_sw_opt',0,hook_handle)
#endif
...
#ifdef DO_DR_HOOK_TWO_STREAM
    if (lhook) call dr_hook('radiation_two_stream:calc_reflectance_transmittance_sw_opt',1,hook_handle)
#endif
