...
!   2024-08-05  R. El khatib and then use array syntax to let the compilers use fast memeset.
  ...
  subroutine get_real_array4(this, var_name, var, ipermute, ld_first_touch)
    ...
    if (do_permute) then
      ...
      if (ll_first_touch) then
        !!first touch
        !$OMP PARALLEL DO PRIVATE(mm)
        do mm=1,ndimlen4
        ! array syntax in such a case lets the compiler use fast memset. REK.
          var(:,:,:,mm) = 0.0_jprb        
        end do
        !$OMP END PARALLEL DO
      end if
